...
/datum/brain_trauma/severe/split_personality/proc/get_ghost()
	...
	if(owner.stat == DEAD || !owner.mind)
		qdel(src)
		return
	var/list/mob/dead/observer/candidates = pollCandidatesForMob("Do you want to play as [owner]'s split personality?", BAN_ROLE_SPLIT_PERSONALITY, null, 7.5 SECONDS, stranger_backseat)
...
/datum/brain_trauma/severe/split_personality/brainwashing/get_ghost()
	set waitfor = FALSE
	var/list/mob/dead/observer/candidates = pollCandidatesForMob("Do you want to play as [owner]'s brainwashed mind?", BAN_ROLE_TRAITOR, null, 7.5 SECONDS, stranger_backseat, ignore_category = FALSE)
	if(LAZYLEN(candidates))
		var/mob/dead/observer/C = pick(candidates)
		stranger_backseat.key = C.key
	else
