...
/datum/ai_behavior/monkey_equip/ground/perform(delta_time, datum/ai_controller/controller, target_key)
	. = ..()
...
/datum/ai_behavior/monkey_equip/pickpocket/perform(delta_time, datum/ai_controller/controller, target_key)
	. = ..()
...
/datum/ai_behavior/monkey_attack_mob/perform(delta_time, datum/ai_controller/controller, target_key)
	...
	var/attack_results = monkey_attack(controller, target, delta_time, holding_weapon && DT_PROB(MONKEY_ATTACK_DISARM_PROB, delta_time), holding_weapon)

	if(!attack_results || controller.blackboard[BB_MONKEY_AGGRESSIVE])
		return AI_BEHAVIOR_DELAY

	var/hatred_value = controller.blackboard[BB_MONKEY_ENEMIES][target] // Do we still hate this guy?
...
/datum/ai_behavior/monkey_attack_mob/proc/monkey_attack(datum/ai_controller/controller, mob/living/target, delta_time, disarm, holding_weapon)
	...
	var/obj/item/gun/gun_to_shoot = locate() in living_pawn.held_items // We're holding a gun, SHOOOT!
...
/datum/ai_behavior/monkey_set_combat_target/perform(delta_time, datum/ai_controller/controller, set_key, enemies_key)
	...
	for(var/mob/living/possible_enemy in view(MONKEY_ENEMY_VISION, controller.pawn))
		...
		valids[possible_enemy] = ceil(100 / (get_dist(controller.pawn, possible_enemy) || 1))
