...
/datum/ai_behavior/monkey_attack_mob/perform(delta_time, datum/ai_controller/controller, target_key)
	...
	if(W && DT_PROB(MONKEY_ATTACK_DISARM_PROB, delta_time))
		perform_flags = monkey_attack(controller, target, delta_time, TRUE)
	else
		perform_flags = monkey_attack(controller, target, delta_time, FALSE)
...
/datum/ai_behavior/monkey_attack_mob/proc/monkey_attack(datum/ai_controller/controller, mob/living/target, delta_time, disarm)
	...
	if(living_pawn.CanReach(target, weapon))
		if(weapon)
			...
			living_pawn.UnarmedAttack(target, TRUE, disarm ? list(RIGHT_CLICK = TRUE) : null) // Call it a workaround call it a fix, we're faking a right click, and it's deliberately not ai_interact
		...
		else if(get_dist(living_pawn, target) >= MONKEY_THROW_MIN_DISTANCE) // We have a weapon on our hand, let's not throw it because they stepped  1 tile away
