...
/datum/ai_planning_subtree/capricious_retaliate/SelectBehaviors(datum/ai_controller/controller, delta_time)
	. = ..()
...
/datum/ai_behavior/capricious_retaliate/perform(delta_time, datum/ai_controller/controller, targeting_strategy_key, ignore_faction)
	...
	if (controller.blackboard_key_exists(BB_BASIC_MOB_RETALIATE_LIST))
		...
		if (!DT_PROB(deaggro_chance, delta_time))
	...
	if (!DT_PROB(aggro_chance, delta_time))
