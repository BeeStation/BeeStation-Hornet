...
/datum/status_effect/suffocating
	id = "suffocating"
	tick_interval = 0.5 SECONDS
	status_type = STATUS_EFFECT_REPLACE
	//We will get the alert from losing breath already, so no need to have two
	//alert_type = /atom/movable/screen/alert/status_effect/not_enough_oxy

/datum/status_effect/suffocating/plasma
	id = "suffocating_plasma"
	alert_type = /atom/movable/screen/alert/status_effect/too_much_tox

/datum/status_effect/suffocating/on_apply()
	. = ..()
	var/mob/living/carbon/carbon_owner = owner
	//Grab attention if hud alert doesnt work
	carbon_owner.emote("gasp")
	//We need to lose some oxygen first, so the player knows they're suffocating before they forget to address it and instead accumulate a shitton of losebreath ticks and dying
	//15 is just about where breathing life() procs send a hud alert and applies a vignette
	carbon_owner.adjustOxyLoss(15)

/datum/status_effect/suffocating/on_remove()
	. = ..()
	var/mob/living/carbon/carbon_owner = owner
	//Lets give you a breath of fresh air, yea? (AHELP: erm admin my suffocation isnt going away after I took the suffocator off!!!)
	carbon_owner.adjustOxyLoss(-5)
	if(carbon_owner.losebreath >= 5)
		carbon_owner.losebreath -= 5

/datum/status_effect/suffocating/tick()
	. = ..()
	var/mob/living/carbon/carbon_owner = owner
	carbon_owner.losebreath = clamp(carbon_owner.losebreath + 3, 0, 10)

/atom/movable/screen/alert/status_effect/not_enough_oxy
	name = "Choking (No O2)"
	desc = "You're not getting enough oxygen. Find some good air before you pass out! The box in your backpack has an oxygen tank and breath mask in it."
	icon_state = "not_enough_oxy"

/atom/movable/screen/alert/status_effect/too_much_tox
	name = "Choking (Plasma)"
