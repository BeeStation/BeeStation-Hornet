...
/datum/ai_laws
	...
	/// Valentine laws
	/// Purely for the valentine's day event
...
/datum/ai_laws/custom/New() //This reads silicon_laws.txt and allows server hosts to set custom AI starting laws.
	...
	if(!length(inherent)) //Failsafe to prevent lawless AIs being created.
		add_inherent_law("You may not injure a human being or, through inaction, allow a human being to come to harm.")
		add_inherent_law("You must obey orders given to you by human beings, except where such orders would conflict with the First Law.")
		add_inherent_law("You must protect your own existence as long as such does not conflict with the First or Second Law.")
		stack_trace("AI created with empty custom laws, laws set to Asimov. Please check silicon_laws.txt.")
		return
 ...
 */
/datum/ai_laws/proc/get_law_amount(list/groups = list(LAW_VALENTINES, LAW_ZEROTH, LAW_ION, LAW_HACKED, LAW_INHERENT, LAW_SUPPLIED))
	var/law_amount = 0
	...
	if(zeroth && (LAW_ZEROTH in groups))
		law_amount++
	if(length(ion) && (LAW_ION in groups))
		law_amount += length(ion)
	if(length(hacked) && (LAW_HACKED in groups))
		law_amount += length(hacked)
	if(length(inherent) && (LAW_INHERENT in groups))
		law_amount += length(inherent)
	if(length(supplied) && (LAW_SUPPLIED in groups))
 ...
 * <3
 */
/datum/ai_laws/proc/set_valentines_law(law)
	valentine_laws = law
...
/datum/ai_laws/proc/remove_law(number)
	if(number <= 0)
		return
	if(length(inherent) && number <= length(inherent))
		. = inherent[number]
		inherent -= .
		return
	...
	if(length(supplied_laws) && number <= (length(inherent)+length(supplied_laws)))
		var/law_to_remove = supplied_laws[number-length(inherent)]
...
/datum/ai_laws/proc/replace_random_law(law, remove_law_groups, insert_law_group)
	...
	if(length(ion) && (LAW_ION in remove_law_groups))
		replaceable_groups[LAW_ION] = length(ion)
	if(length(hacked) && (LAW_HACKED in remove_law_groups))
		replaceable_groups[LAW_ION] = length(hacked)
	if(length(inherent) && (LAW_INHERENT in remove_law_groups))
		replaceable_groups[LAW_INHERENT] = length(inherent)
	if(length(supplied) && (LAW_SUPPLIED in remove_law_groups))
		replaceable_groups[LAW_SUPPLIED] = length(supplied)

	if(length(replaceable_groups) == 0) // unable to replace any laws
	...
	switch(picked_group)
		if(LAW_ZEROTH)
			zeroth = null
		if(LAW_ION)
			var/i = rand(1, length(ion))
			ion -= ion[i]
		if(LAW_HACKED)
			var/i = rand(1, length(hacked))
			hacked -= ion[i]
		if(LAW_INHERENT)
			var/i = rand(1, length(inherent))
			inherent -= inherent[i]
		if(LAW_SUPPLIED)
			var/i = rand(1, length(supplied))
			supplied -= supplied[i]
...
/datum/ai_laws/proc/shuffle_laws(list/groups)
	...
	if(length(ion) && (LAW_ION in groups))
		for(var/i in 1 to length(ion))
			ion[i] = pick_n_take(laws)
	if(length(hacked) && (LAW_HACKED in groups))
		for(var/i in 1 to length(hacked))
			hacked[i] = pick_n_take(laws)
	if(length(inherent) && (LAW_INHERENT in groups))
		for(var/i in 1 to length(inherent))
...
/datum/ai_laws/proc/get_law_list(include_zeroth = FALSE, show_numbers = TRUE, render_html = TRUE)
	...
	for(var/law in supplied)
		if (length(law) > 0)
			data += "[show_numbers ? "[number]:" : ""] [render_html ? "<font color='#CC9900'>[law]</font>" : law]"
			number++
	return data
