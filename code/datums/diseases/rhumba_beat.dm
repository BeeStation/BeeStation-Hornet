	...
	switch(stage)
		...
		if(5)
			...
			if(DT_PROB(29, delta_time))
				explosion(get_turf(affected_mob), devastation_range = -1, heavy_impact_range = 0, light_impact_range = 2, flash_range = 3, adminlog = FALSE, flame_range = 2, magic = TRUE) // This is equivalent to a lvl 1 fireball
