...
/datum/symptom/radconversion/Activate(datum/disease/advance/A)
	...
	if(A.stage >= 4)
		if(M.radiation)
			...
			M.take_direct_overall_damage(BRUTE, 2)
			if(prob(5))
				if(M.stat != DEAD)
					to_chat(M, span_userdanger("A tear opens in your flesh!"))
		...
		if(M.getToxLoss() && toxheal)
			...
			M.take_direct_overall_damage(BRUTE, 1)
			if(prob(5))
				if(M.stat != DEAD)
					to_chat(M, span_userdanger("A tear opens in your flesh!"))
				M.add_splatter_floor()
		if(M.getCloneLoss() && cellheal)
			M.adjustCloneLoss(-1)
			M.take_direct_overall_damage(BURN, 2) //this uses burn, so as not to make it so you can heal brute to heal all the damage types this deals, and it isn't a no-brainer to use with Pituitary
			if(prob(5) && M.stat != DEAD)
				to_chat(M, span_userdanger("A nasty rash appears on your skin!"))
	else if(prob(2) && ((M.getCloneLoss() && cellheal) || (M.getToxLoss() && toxheal) || M.radiation) && M.stat != DEAD)
		to_chat(M, span_notice("You feel a tingling sensation"))
