...
/datum/symptom/vitiligo/Activate(datum/disease/advance/A)
	...
	if(ishuman(M))
		...
		if(H.skin_tone == "albino")
			return
		if(H.dna.features[FEATURE_MUTANT_COLOR] == COLOR_VERY_LIGHT_GRAY)
			return
		switch(A.stage)
			if(5)
				if(HAS_TRAIT(H, TRAIT_USES_SKINTONES))
					...
					H.dna.features[FEATURE_MUTANT_COLOR] = COLOR_VERY_LIGHT_GRAY
				H.regenerate_icons()
			else
...
/datum/symptom/revitiligo/Activate(datum/disease/advance/A)
	...
	if(ishuman(M))
		...
		if(H.skin_tone == "african2")
			return
		if(H.dna.features[FEATURE_MUTANT_COLOR] == COLOR_BLACK)
			return
		switch(A.stage)
			if(5)
				if(HAS_TRAIT(H, TRAIT_USES_SKINTONES))
					...
					H.dna.features[FEATURE_MUTANT_COLOR] = COLOR_BLACK
				H.regenerate_icons()
			else
