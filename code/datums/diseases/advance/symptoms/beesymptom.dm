	...
	switch(A.stage)
		...
		if(4, 5)
			...
			if(prob(15))
				if(honey)
					to_chat(M, "<span class='notice'>You can't get the taste of honey out of your mouth!.</span>")
					M.reagents.add_reagent(/datum/reagent/medicine/insulin, 10) , (/datum/reagent/consumable/honey, 5) //honey equivalent to 2u omnizine. due to how honey synthesizes 7.5 sugar per unit, the large amounts of insulin are necessary to prevent hyperglycaemic shock due to the bees
				else
					to_chat(M, "<span class='danger'>Your stomach stings painfully.</span>")				
					M.adjustToxLoss(5)
					M.updatehealth()
