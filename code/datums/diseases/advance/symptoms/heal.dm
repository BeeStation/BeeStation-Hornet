...
/datum/symptom/teleport/Activate(datum/disease/advance/A)
	...
	switch(A.stage)
		if(4, 5)
			...
			if(COOLDOWN_FINISHED(src, teleport_cooldown) && (M.bodytemperature < M.get_bodytemp_heat_damage_limit() || M.bodytemperature > M.get_bodytemp_cold_damage_limit()))
			...
			if(location_return)
				...
				else if(((M.bodytemperature > M.get_bodytemp_heat_damage_limit() + telethreshold  && !HAS_TRAIT(M, TRAIT_RESISTHEAT)) || (M.bodytemperature < M.get_bodytemp_cold_damage_limit() - telethreshold  && !HAS_TRAIT(M, TRAIT_RESISTCOLD)) || (burnheal && M.getFireLoss() > 60 + telethreshold)))
					do_sparks(5, FALSE, M)
