...
/datum/symptom/mind_restoration/Activate(var/datum/disease/advance/A)
	...
	if(A.stage >= 3)
		...
		if(purge_alcohol)
			...
			mob_reagent_holder.remove_all_type(/datum/reagent/consumable/ethanol, 3)
			if(ishuman(M))
	...
	if(A.stage >= 4)
		M.drowsyness = max(0, M.drowsyness - 2)
		var/datum/reagents/mob_reagent_holder = M.get_reagent_holder()
		if(mob_reagent_holder.has_reagent(/datum/reagent/toxin/mindbreaker))
			mob_reagent_holder.remove_reagent(/datum/reagent/toxin/mindbreaker, 5)
		if(mob_reagent_holder.has_reagent(/datum/reagent/toxin/histamine))
			mob_reagent_holder.remove_reagent(/datum/reagent/toxin/histamine, 5)
