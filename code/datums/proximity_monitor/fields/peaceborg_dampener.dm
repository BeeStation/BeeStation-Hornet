	...
	for(var/mob/living/silicon/robot/R in ohearers(current_range, get_turf(host)))
		if(R.has_buckled_mobs())
			for(var/mob/living/L in R.buckled_mobs)
				L.visible_message("<span class='warning'>[L] is knocked off of [R] by the charge in [R]'s chassis induced by the hyperkinetic dampener field!</span>")	//I know it's bad.
				L.Paralyze(10)
				R.unbuckle_mob(L)
				do_sparks(5, 0, L)
	..()
