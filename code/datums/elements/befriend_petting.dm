...
/datum/element/befriend_petting/proc/on_click(mob/living/owner, mob/living/user)
	...
	if (owner.stat == DEAD)
		var/additional_text = HAS_TRAIT(user, TRAIT_NAIVE) || HAS_MIND_TRAIT(user, TRAIT_NAIVE) ? "It looks like [owner.p_theyre()] sleeping." : "[owner.p_They()] seem[owner.p_s()] to be dead."
