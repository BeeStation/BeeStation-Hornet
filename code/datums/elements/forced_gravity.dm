...
/datum/element/forced_gravity/Attach(datum/target, gravity=1, ignore_space=FALSE)
	...
	RegisterSignal(target, COMSIG_ATOM_HAS_GRAVITY, PROC_REF(gravity_check), TRUE)
	if(isturf(target))
		RegisterSignal(target, COMSIG_TURF_HAS_GRAVITY, PROC_REF(turf_gravity_check), TRUE)
