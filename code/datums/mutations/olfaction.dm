...
/datum/mutation/olfaction/modify()
	. = ..()
...
/datum/action/cooldown/spell/olfaction/proc/on_the_trail(mob/living/caster)
	...
	if(caster_turf.get_virtual_z_level() < current_target_turf.get_virtual_z_level())
		to_chat(caster, span_warning("The trail leads... way up above you? Huh. They must be really, really far away."))
		return

	else if(caster_turf.get_virtual_z_level() > current_target_turf.get_virtual_z_level())
