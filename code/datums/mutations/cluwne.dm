...
/datum/mutation/cluwne/on_losing(mob/living/carbon/owner)
	owner.emote("scream")
	owner.visible_message(span_warning("<span class='name'>[owner]</span> faints as [owner.p_their()] cursed cluwne clothing melts away!"))
...
/mob/living/carbon/proc/cluwneify(cursed = FALSE)
	...
	emote("scream")
	regenerate_icons()
	visible_message(span_danger("<span class='name'>[src]'s</span> body glows green, the glow dissipating only to leave behind a cluwne formerly known as <span class='name'>[src]</span>!"), \
