...
/datum/mutation/race
	...
	var/orig_species = /datum/species/human

/datum/mutation/race/on_acquiring(mob/living/carbon/human/owner)
	if(..())
		return
	orig_species = owner.dna.species
	. = owner.monkeyize(TR_KEEPITEMS | TR_KEEPIMPLANTS | TR_KEEPORGANS | TR_KEEPDAMAGE | TR_KEEPVIRUS | TR_KEEPSE | TR_KEEPAI, FALSE, original_species = orig_species)

/datum/mutation/race/on_losing(mob/living/carbon/monkey/owner)
	if(owner && ismonkey(owner) && owner.stat != DEAD && !..())
		. = owner.humanize(TR_KEEPITEMS | TR_KEEPIMPLANTS | TR_KEEPORGANS | TR_KEEPDAMAGE | TR_KEEPVIRUS | TR_KEEPSE | TR_KEEPAI, original_species = orig_species)
