...
/datum/action/proc/Grant(mob/M)
	if(M)
		...
		if(M.client)
			...
			button.locked = (M.client.prefs.toggles & TOGGLE_BUTTON_LOCK) || button.id ? M.client.prefs.action_buttons_screen_locs["[name]_[button.id]"] : FALSE //even if it's not defaultly locked we should remember we locked it before
