		...
		if(isliving(target))		//target is living, thus we have preset color for him
			if(ishuman(target))
				...
				if(!target.chat_color)		//extreme case - mob doesn't have set color
					STACK_TRACE_ADV("Error: Mob did not have a chat_color. The only way this can happen is if you set it to null purposely in the thing. Don't do that please.")
...
/datum/chatmessage/balloon_alert/New(text, atom/target, mob/owner, color)
	...
	if(QDELETED(owner) || !istype(owner) || !owner.client)
		STACK_TRACE_ADV("/datum/chatmessage created with [isnull(owner) ? "null" : "invalid"] mob owner")
		qdel(src)
		return
	//handle color
	if(color)
