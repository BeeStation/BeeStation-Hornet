...
/mob/proc/should_show_chat_message(atom/movable/speaker, datum/language/message_language, is_emote = FALSE, is_heard = FALSE)
	...
	if(!(client.prefs.toggles & TOGGLE_RUNECHAT) || (!(client.prefs.toggles & TOGGLE_NON_MOB_RUNECHAT) && !ismob(speaker)))
		return CHATMESSAGE_CANNOT_HEAR
	if(!(client.prefs.toggles & TOGGLE_EMOTES_RUNECHAT) && is_emote)
		return CHATMESSAGE_CANNOT_HEAR
