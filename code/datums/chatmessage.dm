...
/datum/chatmessage/proc/generate_image(text, atom/target, list/client/hearers, datum/language/language, list/extra_classes, lifespan)
	...
	group.message.maptext = "<span style='margin-left:[CHAT_MESSAGE_MARGIN + CHAT_MESSAGE_WIDTH]px;line-height:[CHAT_MESSAGE_LINE_HEIGHT]'>[MAPTEXT(complete_text)]</span>"
		...
		for(var/datum/chatmessage/m as() in message_loc.chat_messages)
			...
			for (var/datum/chatmessage_group/group as() in m.groups)
				...
				if (length(new_group.clients))
					...
					// a single line
					new_group.message.maptext = "[new_group.message.maptext]\n<span style='margin-right:[CHAT_MESSAGE_WIDTH + CHAT_MESSAGE_MARGIN]px;line-height:[CHAT_MESSAGE_LINE_HEIGHT]'>[MAPTEXT(complete_text)]</span>"
