...
/datum/music/New(client/C, _fade_volume = 1)
	...
	if((target.prefs.toggles & PREFTOGGLE_SOUND_AMBIENCE) && (!target.playing_music || target.playing_music.priority < priority || (target.playing_music.priority == priority && target.playing_music.does_loop && !does_loop)))
		unmask()
	else if(!does_loop)
		qdel(src)
...
/datum/music/proc/unmask()
	...
	if(sound_datum || !target || !sound_file)
		return
	sound_datum = sound(sound_file, does_loop, 0, CHANNEL_AMBIENT_MUSIC, base_volume * fade_volume)
