...
/datum/dna/proc/generate_uni_identity()
	...
	switch(holder.gender)
		if(MALE)
			L[DNA_GENDER_BLOCK] = construct_block(G_MALE, GENDER_OPTION_SIZE)
		if(FEMALE)
			L[DNA_GENDER_BLOCK] = construct_block(G_FEMALE, GENDER_OPTION_SIZE)
		if(PLURAL)
			L[DNA_GENDER_BLOCK] = construct_block(G_PLURAL, GENDER_OPTION_SIZE)
		else
			L[DNA_GENDER_BLOCK] = construct_block(G_NEUTER, GENDER_OPTION_SIZE)
	if(ishuman(holder))
...
/datum/dna/proc/update_ui_block(blocknumber)
	...
	switch(blocknumber)
		...
		if(DNA_GENDER_BLOCK)
			switch(H.gender)
				if(MALE)
					setblock(uni_identity, blocknumber, construct_block(G_MALE, GENDER_OPTION_SIZE))
				if(FEMALE)
					setblock(uni_identity, blocknumber, construct_block(G_FEMALE, GENDER_OPTION_SIZE))
				if(PLURAL)
					setblock(uni_identity, blocknumber, construct_block(G_PLURAL, GENDER_OPTION_SIZE))
				else
					setblock(uni_identity, blocknumber, construct_block(G_NEUTER, GENDER_OPTION_SIZE))
...
/mob/living/carbon/proc/updateappearance(icon_update=1, mutcolor_update=0, mutations_overlay_update=0)
	if(!has_dna())
		return
	switch(deconstruct_block(getblock(dna.uni_identity, DNA_GENDER_BLOCK), GENDER_OPTION_SIZE))
		if(G_MALE)
