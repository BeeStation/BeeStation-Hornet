...
/datum/greyscale_layer/proc/ReadJsonData(list/json_data)
	...
	for(var/keyname in json_data)
		if(required_values[keyname] && optional_values[keyname])
			STACK_TRACE_ADV("Key '[keyname]' found in both required and optional lists. Make sure keys are only in one or the other.")
			continue
		if(!required_values[keyname] && !optional_values[keyname])
			STACK_TRACE_ADV("Unknown key found in json for [src]: '[keyname]'")
			continue
		if(!(keyname in vars))
			STACK_TRACE_ADV("[src] expects a value from '[keyname]' but has no var to hold the output.")
			continue
		var/datum/json_reader/reader = required_values[keyname] || optional_values[keyname]
		reader = json_readers[reader]
		if(!reader)
			STACK_TRACE_ADV("[src] has an invalid json reader type '[required_values[keyname]]' for key '[keyname]'.")
			continue
	...
	for(var/keyname in required_values)
		if(isnull(json_data[keyname]))
			STACK_TRACE_ADV("[src] is missing required json data key '[keyname]'.")
