	...
	for(var/number_string in value)
		...
		if(!isnum(new_value))
			if(!istext(number_string) || number_string[1] != "#")
				STACK_TRACE_ADV("Expected list to only contain numbers or colors but got '[number_string]'")
				continue
...
/datum/json_reader/color_matrix/ReadJson(list/value)
	...
	for(var/list/row in value)
		...
		if(!istype(row) || length(row) != 4)
			STACK_TRACE_ADV("Expected list to contain further row lists with exactly 4 entries")
			interpreted_row = list(0, 0, 0, 0)
			continue
		for(var/number in row)
			if(!isnum(number))
				STACK_TRACE_ADV("Each color matrix row must only contain numbers")
				interpreted_row += 0
			else
