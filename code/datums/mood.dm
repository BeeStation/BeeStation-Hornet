...
/datum/mood/proc/update_mood()
	...
	for(var/category in mood_events)
		...
		if(mob_parent.has_quirk(/datum/quirk/hypersensitive) && (mood_change<0))
			mood_change*=1.5
		if(mob_parent.has_quirk(/datum/quirk/apathetic) && (mood_change<0))
			mood_change*=0.5
		mood += mood_change
		if (!the_event.hidden)
...
/datum/mood/proc/check_area_mood(datum/source, area/new_area)
	...
	if (new_area.mood_check() && (!new_area.mood_trait || HAS_TRAIT(source, new_area.mood_trait)))
		add_mood_event("area", /datum/mood_event/area, new_area.mood_check(), new_area.mood_message)
	else
