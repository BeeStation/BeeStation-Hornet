...
/datum/map_generator/island_generator
	///2D list of all biomes based on heat and humidity combos.
	var/list/possible_biomes = list(
		BIOME_LOW_HEAT = /datum/biome/islandedge,
		BIOME_MEDIUM_HEAT = /datum/biome/islandlush,
		BIOME_HIGH_HEAT = /datum/biome/islandclearing
	)
	///Used to select "zoom" level into the perlin noise, higher numbers result in slower transitions
	var/perlin_zoom = 30

///Seeds the rust-g perlin noise with a random number.
/datum/map_generator/island_generator/generate_terrain(list/turfs, area/generate_in)
	. = ..()
