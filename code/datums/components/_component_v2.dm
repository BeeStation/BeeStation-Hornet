...
/datum/component_v2/New(list/raw_args)
	parent = raw_args[1]
...
/datum/component_v2/proc/_JoinParent()
	var/datum/P = parent
		...
		if(test)	//already another component of this type here
			...
			if(I == our_type)	//exact match, take priority
				...
				for(var/J in 1 to components_of_type.len)
					var/datum/component_v2/C = components_of_type[J]
...
/datum/component_v2/proc/RegisterWithParent()
	return

/datum/component_v2/proc/Initialize(...)
	return

/datum/component_v2/Destroy(force=FALSE, silent=FALSE)
	if(!force && parent)
		_RemoveFromParent()
	if(parent && !silent)
...
/datum/component_v2/proc/_RemoveFromParent()
	var/datum/P = parent
	var/list/dc = P.datum_components
...
/datum/component_v2/proc/UnregisterFromParent()
	return
...
/datum/component_v2/proc/InheritComponent(datum/component_v2/C, i_am_original)
	return

/datum/component_v2/proc/CheckDupeComponent(datum/component_v2/C, ...)
	return

/datum/component_v2/proc/PreTransfer()
	return

/datum/component_v2/proc/PostTransfer()
	return COMPONENT_INCOMPATIBLE //Do not support transfer by default as you must properly support it

/datum/component_v2/proc/_GetInverseTypeList(our_type = type)
	//we can do this one simple trick
	var/current_type = parent_type
	. = list(our_type, current_type)
	//and since most components are root level + 1, this won't even have to run
	while (current_type != /datum/component_v2)
...
/datum/proc/GetComponent(datum/component_v2/c_type)
	RETURN_TYPE(c_type)
...
/datum/proc/GetExactComponent(datum/component_v2/c_type)
	RETURN_TYPE(c_type)
	if(initial(c_type.dupe_mode) == COMPONENT_DUPE_ALLOWED || initial(c_type.dupe_mode) == COMPONENT_DUPE_SELECTIVE)
		stack_trace("GetComponent was called to get a component of which multiple copies could be on an object. This can easily break and should be changed. Type: \[[c_type]\]")
	var/list/dc = datum_components
	if(!dc)
		return null
	var/datum/component_v2/C = dc[c_type]
	if(C)
		if(length(C))
			C = C[1]
		if(C.type == c_type)
...
/datum/proc/_AddComponent(list/raw_args)
	...
	var/datum/component_v2/nt = new_type
	...
	var/datum/component_v2/old_comp
	var/datum/component_v2/new_comp

	if(ispath(nt))
		if(nt == /datum/component_v2)
	...
	if(dm != COMPONENT_DUPE_ALLOWED && dm != COMPONENT_DUPE_SELECTIVE)
		...
		for(var/datum/component_v2/existing_component as anything in GetComponents(new_type))
...
/datum/component_v2/proc/RemoveComponent()
	if(!parent)
		return
...
/datum/proc/TakeComponent(datum/component_v2/target)
...
/datum/proc/TransferComponents(datum/target)
	...
	if(!dc)
		return
	var/comps = dc[/datum/component_v2]
	if(islist(comps))
		for(var/datum/component_v2/I in comps)
			if(I.can_transfer)
				target.TakeComponent(I)
	else
		var/datum/component_v2/C = comps
		if(C.can_transfer)
			target.TakeComponent(comps)

/datum/component_v2/ui_host()
	return parent
