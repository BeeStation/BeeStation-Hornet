...
/datum/component/personal_crafting/proc/create_mob_button(mob/user, client/user_client)
	...
	RegisterSignal(craft_ui, COMSIG_CLICK, PROC_REF(component_ui_interact))
...
/datum/component/personal_crafting/proc/get_environment(atom/a, list/blacklist = null, radius_range = 1)
	...
	for(var/atom/movable/AM in range(radius_range, a))
		if(blacklist && (AM.type in blacklist))
			continue
		else if(istype(get_area(AM), /area/station/holodeck/prison)) //don't prevent crafting in the prison workshop
			. += AM
