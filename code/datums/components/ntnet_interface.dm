...
/datum/proc/ntnet_send(packet_data, target_id = null, passkey = null)
	...
	if(!istype(data)) // construct netdata from list()
		if(!islist(packet_data))
			STACK_TRACE_ADV("ntnet_send: Bad packet creation") // hard fail as its runtime fault
			return
...
/datum/component/ntnet_interface/proc/register_port(port, list/data)
	if(!port || !length(data))
		STACK_TRACE_ADV("port is null or data is empty")
		return
	if(registered_sockets[port])
		STACK_TRACE_ADV("port already regestered")
		return
	data["_updated"] = FALSE
