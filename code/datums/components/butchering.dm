...
/datum/component/butchering/proc/onItemAttack(obj/item/source, mob/living/M, mob/living/user)
	...
	if(user.a_intent == INTENT_GRAB && ishuman(M) && source.is_sharp())
		...
		if(H.has_status_effect(/datum/status_effect/neck_slice))
			user.show_message(span_danger("[H]'s neck has already been already cut, you can't make the bleeding any worse!"), 1, \
							span_danger("Their neck has already been already cut, you can't make the bleeding any worse!"))
			return COMPONENT_ITEM_NO_ATTACK
