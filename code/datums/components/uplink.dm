...
/datum/component/uplink/proc/new_ringtone(datum/source, mob/living/user, new_ring_text)
	...
	if(trim(LOWER_TEXT(new_ring_text)) != trim(lowertext(unlock_code)))
		if(failsafe_code && trim(LOWER_TEXT(new_ring_text)) == trim(lowertext(failsafe_code)))
			failsafe()
...
/datum/component/uplink/proc/radio_message(datum/source, mob/living/user, treated_message, channel, list/message_mods)
	...
	if(!findtext(LOWER_TEXT(message_to_use), lowertext(unlock_code)))
		if(failsafe_code && findtext(LOWER_TEXT(message_to_use), lowertext(failsafe_code)))
			failsafe()
		return
	locked = FALSE
	interact(null, user)
