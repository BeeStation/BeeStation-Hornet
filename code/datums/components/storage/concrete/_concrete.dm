...
/datum/component/storage/concrete/remove_from_storage(atom/movable/AM, atom/new_location)
	...
	if(new_location)
		...
		_removal_reset(AM)
		if(!ismob(new_location || !isitem(AM)))
			AM.forceMove(get_turf(new_location))
		else
			...
			target_item.move_to_mobhand(new_location, !silent && isturf(new_location.loc) && get_turf(target_item.loc))
