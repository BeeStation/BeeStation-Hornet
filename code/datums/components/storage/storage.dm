...
/datum/component/storage/proc/can_be_inserted(obj/item/I, stop_messages = FALSE, mob/M)
	...
	if(length(can_hold))
		if(!can_hold[I.type])
			...
			return FALSE
	if(length(cant_hold) && cant_hold[I.type] || HAS_TRAIT(I, TRAIT_NO_STORAGE_INSERT) || (can_hold_trait && !HAS_TRAIT(I, can_hold_trait))) //Items which this container can't hold.
		if(!stop_messages)
			host.balloon_alert(M, "It doesn't fit")
		return FALSE
