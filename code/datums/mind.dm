	...
	if (!implant)
		...
		if(!silent)
			if(uplink_loc == R)
				...
				U.unlock_text = "[employer] [employer == "You" ? "have" : "has"] cunningly disguised a Syndicate Uplink as your PDA's network card. Simply enter the code \"[U.unlock_code]\" into the ring tone selection to unlock its hidden features."
