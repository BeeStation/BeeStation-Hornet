...
/turf/Destroy(force)
	. = QDEL_HINT_IWILLGC
	if(!changing_turf)
		STACK_TRACE_ADV("Incorrect turf deletion")
	changing_turf = FALSE
	if (z_flags & Z_MIMIC_BELOW)
		cleanup_zmimic()
	if(force)
...
/turf/proc/assemble_baseturfs(turf/fake_baseturf_type)
	...
	if(fake_baseturf_type)
		...
		if(!baseturfs)
			...
			STACK_TRACE_ADV("baseturfs var was null for [type]. Failsafe activated and it has been given a new baseturfs value of [current_target].")
		else
	...
	for(var/i=0;current_target != next_target;i++)
		if(i > 100)
			...
			STACK_TRACE_ADV("A turf <[type]> created a baseturfs list over 100 members long. This is most likely an infinite loop.")
