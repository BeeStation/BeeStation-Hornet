...
/obj/structure/checkoutmachine/Destroy()
	...
	priority_announce("The credit deposit machine at [get_area(src).name] has been destroyed. Station funds have stopped draining!", sender_override = "CRAB-17 Protocol", SSstation.announcer.get_rand_alert_sound())
...
/obj/effect/dumpeetTarget/proc/startLaunch()
	...
	priority_announce("The spacecoin bubble has popped! Get to the credit deposit machine at [get_area(src).name] and cash out before you lose all of your funds!", sender_override = "CRAB-17 Protocol", SSstation.announcer.get_rand_alert_sound())
