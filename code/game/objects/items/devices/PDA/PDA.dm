...
/mob/living/silicon/proc/cmd_send_pdamesg(mob/user)
	...
	var/choice = input(user, "Please select a PDA") as null|anything in sortList(plist)
	...
	if(aicamera.stored.len)
		var/add_photo = input(user,"Do you want to attach a photo?","Photo","No") as null|anything in list("Yes","No") /// Change to TGUI List once ported
		if(add_photo == "Yes")
