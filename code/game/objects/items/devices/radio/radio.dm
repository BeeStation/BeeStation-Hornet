...
/obj/item/radio/talk_into(atom/movable/M, message, channel, list/spans, datum/language/language)
	if(HAS_TRAIT(M, TRAIT_SIGN_LANG) && istype(M, /mob/living/carbon)) //Forces Sign Language users to wear the translation gloves to speak over radios
		var/mob/living/carbon/mute = M
		var/empty_indexes = mute.get_empty_held_indexes() //How many hands the player has empty
		var/gloves = mute.get_item_by_slot(ITEM_SLOT_GLOVES)
		if(!istype(gloves, /obj/item/clothing/gloves/radio))
			return FALSE
