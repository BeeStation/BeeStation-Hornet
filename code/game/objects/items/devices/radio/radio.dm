...
/obj/item/radio/proc/talk_into_impl(atom/movable/M, message, channel, list/spans, datum/language/language, list/message_mods)
	...
	*/
	...
	for(var/obj/item/jammer/jammer in GLOB.active_jammers)
		...
		if(position?.get_z_level() == jammer_turf.get_z_level() && (get_dist(position, jammer_turf) <= jammer.range))
			return
...
/obj/item/radio/proc/backup_transmission(datum/signal/subspace/vocal/signal)
	...
	if (signal.data["done"] && (T.get_z_level() in signal.levels))
		return

	// Okay, the signal was never processed, send a mundane broadcast.
	signal.data["compression"] = 0
	signal.transmission_method = TRANSMISSION_RADIO
	signal.levels = list(T.get_z_level())
	signal.broadcast()
...
/obj/item/radio/proc/can_receive(freq, level)
	...
	if (!(0 in level))
		var/turf/position = get_turf(src)
		if(!position || !(position.get_z_level() in level))
			return FALSE
