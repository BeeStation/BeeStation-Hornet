...
/obj/item/circuitboard/proc/apply_default_parts(obj/machinery/M)
	if(LAZYLEN(M.component_parts))
		...
		STACK_TRACE_ADV("apply_defauly_parts called on machine that already had component_parts: [M]")
	...
	if(M.circuit != src)
		// This really shouldn't happen. If it somehow does, print out a stack trace and gracefully handle it.
		STACK_TRACE_ADV("apply_default_parts called from a circuit board that does not belong to machine: [M]")
