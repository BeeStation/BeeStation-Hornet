		...
		if(do_after(user, 20, target = H)) //placed on chest and short delay to shock for dramatic effect, revive time is 5sec total
			...
			if(H.stat == DEAD)
				...
				if (H.suiciding)
					...
					failed = span_warning("[req_defib ? "[defib]" : "[src]"] buzzes: Resuscitation failed - Body has decayed for too long. All further attempts at revival futile.")
				else if (!heart)
					...
					if(BR)
						if(BR.organ_flags & ORGAN_FAILING || BR.brain_death)
							failed = span_warning("[req_defib ? "[defib]" : "[src]"] buzzes: Resuscitation failed - Patient's brain has decayed too long. All further attempts at revival futile.")
