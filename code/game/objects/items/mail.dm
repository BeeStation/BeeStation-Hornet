...
/obj/item/paper/fluff/nice_argument/Initialize()
	...
	default_raw_text = "Nice argument, however there's a <i>small detail</i>...<br>IP: '[rand(0,10)].[rand(0,255)].[rand(0,255)].[rand(0,255)]'<br> Station name: '[station_name]'<br>"
...
/obj/item/paper/fluff/junkmail_redpill/Initialize()
	...
	if(!prob(nuclear_option_odds))
		default_raw_text = "<i>You need to escape the simulation. Don't forget the numbers, they help you remember:</i> '[random_code(4)]...'"
		return
	...
	for(var/obj/machinery/nuclearbomb/selfdestruct/nuke in GLOB.nuke_list)
		if(nuke)
			...
			code = null
	default_raw_text = "<i>You need to escape the simulation. Don't forget the numbers, they help you remember:</i> '[code[rand(1,5)]][code[rand(1,5)]][code[rand(1,5)]][code[rand(1,5)]]...'"
