...
/obj/item/grenade/discogrenade
	...
	var/list/message_social_anxiety = list("I want to go home...", "I don't like this song", "AHHH!!!", "I am going home now")
...
/obj/item/grenade/discogrenade/subgrenade/prime()
	...
	//Force the nearby mobs to dance
...
/obj/item/grenade/discogrenade/subgrenade/proc/forcedance(turf/T , mob/living/carbon/human/M)
	...
	if(distance > 2.5)
		return
	//Social Anxiety will have a different response to party
	if(M.has_quirk(/datum/quirk/social_anxiety))
		M.say(pick(message_social_anxiety), 0, list(), FALSE, null, TRUE)
		...
		return
	//Mindshielded people are not affected
	...
	switch(rand(0, 5))
		...
		if(3 to 4)
			M.emote("flip")
		if(5)
			M.say(pick(messages), 0, list(), FALSE, null, TRUE)
