...
/obj/item/pinpointer/update_icon()
	...
	if(here.get_virtual_z_level() != there.get_virtual_z_level())
		...
		var/datum/orbital_object/our_location = SSorbits.get_associated_level(here)
		var/datum/orbital_object/their_location = SSorbits.get_associated_level(there)
		if (our_location && their_location)
			var/angle = their_location.position.AngleFrom(our_location.position)
			switch (angle)
				if (-15 to 15)
					setDir(NORTH)
				if (75 to 105)
				...
				if (-165 to -105)
					setDir(SOUTHWEST)
			switch(our_location.position.DistanceTo(their_location.position))
				if(0 to 2000)
