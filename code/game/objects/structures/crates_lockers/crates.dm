...
/obj/structure/closet/crate/animate_door(var/closing = FALSE)
	...
	for(var/I in 0 to num_steps)
		...
		var/azimuth_angle = angle >= 90 ? 145 : 325
		var/polar_angle = abs(arcsin(cos(angle)))
		var/radius_cr = angle >= 90 ? radius_2 : 1
		var/matrix/M = get_door_transform(angle, azimuth_angle, polar_angle, isbiggersin, radius_cr)
		if(I == 0)
...
/obj/structure/closet/crate/get_door_transform(angle, azimuth_angle, polar_angle, isbiggersin, radius_cr)
		var/matrix/M = matrix()
		M.Translate(0, -door_hinge)
		M.Multiply(matrix(1, -sin(polar_angle)*sin(azimuth_angle)* radius_cr, cos(polar_angle)*radius_cr, 0, radius_cr*cos(azimuth_angle)*sin(polar_angle), 0))
		M.Translate(0, door_hinge)
		return M
