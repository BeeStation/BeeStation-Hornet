	...
	if(grill)
		...
		if(!user.combat_mode && !(used_item.item_flags & ABSTRACT))
			if(user.temporarilyRemoveItemFromInventory(used_item))
				...
				used_item.pixel_x = used_item.base_pixel_x + clamp(text2num(LAZYACCESS(modifiers, ICON_X)) - 16, -(world.ICON_SIZE_X / 2), world.ICON_SIZE_X / 2)
				used_item.pixel_y = used_item.base_pixel_y + clamp(text2num(LAZYACCESS(modifiers, ICON_Y)) - 16, -(world.ICON_SIZE_Y / 2), world.ICON_SIZE_Y / 2)
		else
			return ..()
...
/obj/structure/bonfire/proc/start_burning()
	if(burning && (needs_oxygen || !check_oxygen()))
		return
	...
	for(var/burn_target in current_location)
		if(burn_target == src)
			...
			if(grill && isitem(burned_movable))
				...
				continue
			burned_movable.fire_act(1000, 250 * delta_time)

/obj/structure/bonfire/process(delta_time)
	if(needs_oxygen && !check_oxygen())
		extinguish()
		return
