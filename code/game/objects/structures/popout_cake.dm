...
/obj/structure/popout_cake/attackby(obj/item/W, mob/user, params)
	if(W.tool_behaviour == TOOL_SCREWDRIVER && used_string == TRUE)
		...
		if(do_after(user, 2 SECONDS, target=src, timed_action_flags = IGNORE_HELD_ITEM, add_item = W))
			used_string = FALSE
	...
	if(W.is_sharp())
		...
		if(do_after(user, 6 SECONDS, src, timed_action_flags = IGNORE_HELD_ITEM, add_item = W))
			do_popout()
			if(!strong_surprise)
	...
	if(istype(W, /obj/item/grenade/flashbang))
		if(strong_surprise)
			...
			if(do_after(user, 3 SECONDS, src, timed_action_flags = IGNORE_HELD_ITEM, add_item = W))
				strong_surprise = TRUE
