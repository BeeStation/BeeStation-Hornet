		...
		if(prob(50) && anchored && !broken)
			...
			if(O.throwforce != 0)//don't want to let people spam tesla bolts, this way it will break after time
				...
				if(C)
					...
					//Zap for 1% of the amount of power. At one MegaWatt in the grid it will be as powerful as a tesla revolver
					tesla_zap(src, 3, C.newavail() * 0.01, 1e3, ZAP_MOB_DAMAGE | ZAP_OBJ_DAMAGE | ZAP_MOB_STUN | ZAP_LOW_POWER_GEN | ZAP_ALLOW_DUPLICATES)
