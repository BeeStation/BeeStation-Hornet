...
/datum/objective/download/check_completion()
	...
	var/list/datum/mind/owners = get_owners()
	for(var/datum/mind/owner as() in owners)
		if(!ismob(owner.current))
			continue
		var/mob/M = owner.current
		var/list/items = M.GetAllContents()
		for(var/obj/item/disk/tech_disk/TD in items)
...
/datum/objective/download/ninja

/datum/objective/download/ninja/update_explanation_text()
	..()
	explanation_text = "Download [target_amount] research node\s and save them on you suit."

/datum/objective/download/ninja/check_completion()
	var/datum/techweb/checking = new
