...
/datum/game_mode/traitor/bros/pre_setup()
	...
	var/list/datum/mind/possible_brothers = get_players_for_role(ROLE_KEY_BROTHER)
	...
	for(var/j = 1 to num_teams)
		...
		for(var/k = 1 to team_size)
			var/datum/mind/bro = antag_pick(possible_brothers, ROLE_KEY_BROTHER)
			possible_brothers -= bro
			antag_candidates -= bro
			team.add_member(bro)
			bro.assign_special_role(ROLE_KEY_BROTHER)
