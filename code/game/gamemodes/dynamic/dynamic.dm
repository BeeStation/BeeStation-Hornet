		...
		if(fexists(json_file))
			...
			if(configuration["Dynamic"])
				for(var/variable in configuration["Dynamic"])
					if(!vars[variable])
						STACK_TRACE_ADV("Invalid dynamic configuration variable [variable] in game mode variable changes.")
						continue
...
/datum/game_mode/dynamic/proc/roundstart(list/roundstart_rules)
	...
	while (round_start_budget_left > 0)
		...
		if (cost == 0)
			STACK_TRACE_ADV("[ruleset] cost 0, this is going to result in an infinite loop.")
...
/datum/game_mode/dynamic/proc/picking_roundstart_rule(datum/dynamic_ruleset/roundstart/ruleset, scaled_times = 0, forced = FALSE)
	...
	if(simulated || ruleset.pre_execute(roundstart_pop_ready))
		...
		STACK_TRACE_ADV("The starting rule \"[ruleset.name]\" failed to pre_execute.")
	return 0
...
/datum/game_mode/dynamic/proc/configure_ruleset(datum/dynamic_ruleset/ruleset)
	...
	for(var/variable in rule_conf)
		if(!(variable in ruleset.vars))
			STACK_TRACE_ADV("Invalid dynamic configuration variable [variable] in [ruleset.ruletype] [ruleset.name].")
			continue
