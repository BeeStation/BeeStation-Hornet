...
/datum/game_mode/proc/generate_credit_text()
	...
	len_before_addition = round_credits.len
	for(var/datum/mind/current in SSticker.mode.get_all_by_department(GLOB.civilian_positions)) // gimmicks shouldn't be here, but let's not make the code dirty
