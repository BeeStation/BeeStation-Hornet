...
/datum/action/innate/mecha/mech_toggle_internals/Activate()
	...
	to_chat(owner, "Taking air from [chassis.use_internal_tank ? "internal airtank" : "environment"]")
...
/datum/action/innate/mecha/mech_cycle_equip/Activate()
	...
	if(available_equipment.len == 0)
		to_chat(owner, "No equipment available")
		return
	if(!chassis.selected)
		chassis.selected = available_equipment[1]
		to_chat(owner, "[chassis.selected] selected")
	...
	for(var/A in available_equipment)
		...
		if(A == chassis.selected)
			if(available_equipment.len == number)
				...
				to_chat(owner, "Switched to no equipment")
				button_icon_state = "mech_cycle_equip_off"
			else
				chassis.selected = available_equipment[number+1]
				to_chat(owner, "Switched to [chassis.selected]")
				button_icon_state = "mech_cycle_equip_on"
			send_byjax(chassis.occupant,"exosuit.browser","eq_list",chassis.get_equipment_list())
			UpdateButtonIcon()
			return
...
/datum/action/innate/mecha/mech_toggle_lights/Activate()
	...
	if(chassis.lights)
		...
		chassis.set_light(-chassis.lights_power)
		button_icon_state = "mech_lights_off"
	to_chat(owner, "Toggled lights [chassis.lights?"on":"off"]")
...
/datum/action/innate/mecha/mech_overload_mode/Activate(forced_state = null)
	...
	if(chassis.leg_overload_mode)
		...
		to_chat(owner,"Leg actuators overloaded")
	else
		chassis.leg_overload_mode = 0
		chassis.step_in = initial(chassis.step_in)
		chassis.step_energy_drain = chassis.normal_step_energy_drain
		to_chat(owner, "Leg actuators reset")
	UpdateButtonIcon()
...
/datum/action/innate/mecha/mech_zoom/Activate()
	...
	if(owner.client)
		...
		to_chat(owner, "Zoom mode [chassis.zoom_mode?"enabled":"disabled"]")
...
/datum/action/innate/mecha/mech_switch_damtype/Activate()
	...
	switch(chassis.damtype)
		if("tox")
			new_damtype = "brute"
			to_chat(owner, "Your punches will now deal brute damage")
		if("brute")
			new_damtype = "fire"
			to_chat(owner, "Your punches will now deal burn damage")
		if("fire")
			new_damtype = "tox"
			to_chat(owner,"Your punches will now deal toxin damage")
...
/datum/action/innate/mecha/mech_toggle_phasing/Activate()
	...
	to_chat(owner, "[chassis.phasing ? "Enabled" : "Disabled"] phasing")
	UpdateButtonIcon()
