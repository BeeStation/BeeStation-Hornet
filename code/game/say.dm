...
/atom/movable/proc/compose_message(atom/movable/speaker, datum/language/message_language, raw_message, radio_freq, list/spans, list/message_mods = list(), face_name = FALSE)
	...
	//Speaker name
	var/namepart = "[speaker.GetVoice(speaker.name)][speaker.get_alt_name()]"
	if(ishuman(speaker))
...
/atom/movable/proc/GetVoice(var/current_name)
...
/atom/movable/virtualspeaker/Initialize(mapload, atom/movable/M, _radio)
	...
	source = M
	if(istype(M))
		name = radio.anonymize ? "Unknown" : M.GetVoice(M.name)
		verb_say = M.verb_say
		verb_ask = M.verb_ask
