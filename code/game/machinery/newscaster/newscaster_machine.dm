...
/obj/machinery/newscaster/ui_data(mob/user)
	...
	if(card?.registered_account)
		...
		if(R)
			...
			data["user"]["job"] = card.registered_account.account_job.get_title()
...
/obj/machinery/newscaster/proc/create_story(channel_name)
	...
	GLOB.news_network.submit_article("<font face=\"[PEN_FONT]\">[parsemarkdown(feed_channel_message, usr)]</font>", usr_name, current_channel.channel_name, send_photo_data(), adminMessage = FALSE, allow_comments = TRUE, author_job = issilicon(usr) ? usr.job : account.account_job.get_title(), author_account = account)
