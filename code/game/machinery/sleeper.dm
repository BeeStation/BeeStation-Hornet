	...
	if (mapload)
		...
		for (var/each_vial in roundstart_vials)
			if(created_vials >= max_vials)
				STACK_TRACE_ADV("Sleeper attempts to create roundstart chems more than [max_vials]")
				break
			if(!ispath(each_vial, /obj/item/reagent_containers))
				STACK_TRACE_ADV("Sleeper attempts to create weird item inside of it: [each_vial]")
				continue
		...
		for (var/each_chem in roundstart_chems)
			...
			if(!ispath(each_chem, /datum/reagent))
				STACK_TRACE_ADV("Sleeper attempts to create not-chemical inside of it: [each_chem]")
				continue
