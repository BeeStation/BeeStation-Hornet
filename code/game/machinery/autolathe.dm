...
/obj/machinery/autolathe/Initialize()
	AddComponent(/datum/component/material_container, list(MAT_IRON, MAT_GLASS), 0, TRUE, null, null, CALLBACK(src, .proc/AfterMaterialInsert))
	. = ..()
...
/obj/machinery/autolathe/Topic(href, href_list)
	...
	if (!busy)
		...
		if(href_list["make"])
			...
			var/power = max(2000, (iron_cost+glass_cost)*multiplier/5)

			GET_COMPONENT(materials, /datum/component/material_container)
			if((materials.amount(MAT_IRON) >= iron_cost*multiplier*coeff) && (materials.amount(MAT_GLASS) >= glass_cost*multiplier*coeff))
				busy = TRUE
				use_power(power)
				icon_state = "autolathe_n"
				var/time = is_stack ? 32 : (32 * coeff * multiplier) ** 0.8
				addtimer(CALLBACK(src, .proc/make_item, power, iron_cost, glass_cost, multiplier, coeff, is_stack), time)
...
/obj/machinery/autolathe/proc/make_item(power, iron_cost, glass_cost, multiplier, coeff, is_stack)
	GET_COMPONENT(materials, /datum/component/material_container)
	var/atom/A = drop_location()
	use_power(power)
	var/list/materials_used = list(MAT_IRON=iron_cost*coeff*multiplier, MAT_GLASS=glass_cost*coeff*multiplier)
...
/obj/machinery/autolathe/proc/category_win(mob/user,selected_category)
	...
	for(var/v in stored_research.researched_designs)
		...
		if(ispath(D.build_path, /obj/item/stack))
			GET_COMPONENT(materials, /datum/component/material_container)
			var/max_multiplier = min(D.maxstack, D.materials[MAT_IRON] ?round(materials.amount(MAT_IRON)/D.materials[MAT_IRON]):INFINITY,D.materials[MAT_GLASS]?round(materials.amount(MAT_GLASS)/D.materials[MAT_GLASS]):INFINITY)
...
/obj/machinery/autolathe/proc/get_design_cost(datum/design/D)
	...
	if(D.materials[MAT_IRON])
		dat += "[D.materials[MAT_IRON] * coeff] metal "
