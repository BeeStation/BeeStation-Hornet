...
/obj/machinery/sheetifier/Initialize(mapload)
	...
	AddComponent(/datum/component/material_container, allowed_materials, MINERAL_MATERIAL_AMOUNT * MAX_STACK_SIZE * 2, TRUE, /obj/item/food/meat/slab, CALLBACK(src, PROC_REF(CanInsertMaterials)), CALLBACK(src, PROC_REF(AfterInsertMaterials)), _breakdown_flags=BREAKDOWN_FLAGS_SHEETIFIER)
