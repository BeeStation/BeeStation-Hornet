...
/obj/machinery/server/telecomms/bus
	name = "bus mainframe"
	icon_state = "bus"
...
/obj/machinery/server/telecomms/bus/receive_information(datum/signal/subspace/signal, obj/machinery/server/telecomms/machine_from)
	...
	if(!istype(machine_from, /obj/machinery/server/telecomms/processor) && machine_from != src) // Signal must be ready (stupid assuming machine), let's send it
		// send to one linked processor unit
		if(relay_information(signal, /obj/machinery/server/telecomms/processor))
			return
	...
	// Try sending it!
	var/list/try_send = list(signal.server_type, /obj/machinery/server/telecomms/hub, /obj/machinery/server/telecomms/broadcaster)
...
/obj/machinery/server/telecomms/bus/preset_one
	id = "Bus 1"
	network = "tcommsat"
	freq_listening = list(FREQ_SCIENCE, FREQ_MEDICAL)
	autolinkers = list("processor1", "science", "medical")

/obj/machinery/server/telecomms/bus/preset_two
	id = "Bus 2"
	network = "tcommsat"
	freq_listening = list(FREQ_SUPPLY, FREQ_SERVICE)
	autolinkers = list("processor2", "supply", "service")

/obj/machinery/server/telecomms/bus/preset_three
	id = "Bus 3"
	network = "tcommsat"
	freq_listening = list(FREQ_SECURITY, FREQ_COMMAND)
	autolinkers = list("processor3", "security", "command")

/obj/machinery/server/telecomms/bus/preset_four
	id = "Bus 4"
	network = "tcommsat"
	freq_listening = list(FREQ_ENGINEERING)
	autolinkers = list("processor4", "engineering", "common", "messaging")

/obj/machinery/server/telecomms/bus/preset_four/Initialize(mapload)
	. = ..()
	for(var/i = MIN_FREQ, i <= MAX_FREQ, i += 2)
		freq_listening |= i

/obj/machinery/server/telecomms/bus/preset_one/birdstation
	name = "Bus"
	autolinkers = list("processor1", "common", "messaging")
	freq_listening = list()

/obj/machinery/server/telecomms/bus/preset_exploration
	id = "Exploration Bus"
	network = "exploration"
