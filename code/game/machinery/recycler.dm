...
/obj/machinery/recycler/proc/eat(atom/movable/morsel, sound=TRUE)
	...
	for(var/i in to_eat)
		...
		var/brain_holder = istype(AM, /obj/item/organ/internal/brain) || (istype(as_head) && locate(/obj/item/organ/internal/brain) in as_head) || (istype(as_mmi) && as_mmi.brain) || isbrain(AM) || istype(AM, /obj/item/dullahan_relay)
		if(brain_holder)
			emergency_stop(AM)
		if(isitem(AM))
