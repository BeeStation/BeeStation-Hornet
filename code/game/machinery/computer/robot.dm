...
/obj/machinery/computer/robotics/ui_act(action, datum/params/params)
	...
	switch(action)
		if("killbot")
			if(allowed(usr))
				var/mob/living/silicon/robot/R = locate(params.get_ref("ref")) in GLOB.silicon_mobs
		...
		if("killdrone")
			if(allowed(usr))
				var/mob/living/simple_animal/drone/D = locate(params.get_ref("ref")) in GLOB.mob_list
				if(D.hacked)
