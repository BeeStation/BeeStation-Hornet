	...
	if (mode == 1) // accessing crew manifest
		...
		if(length(paycheck_departments))
			for(var/datum/bank_account/B in SSeconomy.bank_accounts)
				var/datum/record/crew/R = find_record("name", B.account_holder, GLOB.manifest.general)
				dat += "<tr>"
		...
		if (authenticated && inserted_modify_id)
			...
			var/datum/record/crew/R = find_record("name", inserted_modify_id.registered_name, GLOB.manifest.general)
			if(R)
				for(var/each in available_paycheck_departments)
					if(!(SSeconomy.get_budget_acc_bitflag(each) & region_access_payment))
						continue
					if(R.rank & SSeconomy.get_budget_acc_bitflag(each))
...
/obj/machinery/computer/card/Topic(href, href_list)
	...
	switch(href_list["choice"])
				...
				if(t1 == "Custom")
					...
					if(R)
						for(var/each in B.payment_per_department)
							if(SSeconomy.is_nonstation_account(each)) // do not touch VIP/Command flag
								continue
							R.rank &= ~SSeconomy.get_budget_acc_bitflag(each) // turn off all bitflag for each department except for VIP/Command. *note: this actually shouldn't use `get_budget_acc_bitflag()` proc, because bitflags are the same but these have a different purpose.
						R.rank &= ~DEPT_BITFLAG_COM  // micromanagement2. the reason is the same. Command should be removed manually.
					...
					if(R && jobdatum) // 1-B: reseting crew manifest
						for(var/each in available_paycheck_departments)
							if(SSeconomy.is_nonstation_account(each))
								continue
							R.rank &= ~SSeconomy.get_budget_acc_bitflag(each)
						R.rank &= ~DEPT_BITFLAG_COM  // micromanagement2
						// Note: `rank = NONE` is a bad idea because you should keep VIP_BITFLAG.
		...
		if ("turn_on_off_department_manifest")
			...
			if(R.rank & target_bitflag)
				R.rank &= ~target_bitflag // turn off
			else
				R.rank |= target_bitflag // turn on
