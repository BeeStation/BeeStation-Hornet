...
/obj/machinery/computer/card/proc/get_accesses_html()
	...
	for(var/datum/department_group/each_dept in SSdepartment.sorted_department_for_access)
		if(authenticated == 1 && !(each_dept.department_bitflags & accessible_region_bitflag))
			continue
		var/list/dept_access = each_dept.get_access_list()
		if(!length(dept_access) || (!is_centcom && each_dept.access_filter))
			continue
		var/card_html = "<div class='idc-dept' style='border-top-color:[each_dept.dept_colour]'>"
		card_html += "<div class='idc-dept-head'>[each_dept.access_group_name]<span class='idc-dept-count'>[length(dept_access)]</span></div>"
		for(var/each_access in dept_access)
		...
		card_html += "</div>"
		access_cards += list(list("html" = card_html, "weight" = length(dept_access) + 2))
		...
		if ("assign")
			if (authenticated == 2)
				...
				if(t1 == "Custom")
					...
					inserted_modify_id.remove_access(SSdepartment.get_department_access(DEPARTMENT_ID_STATION_ALL), should_log = FALSE)
