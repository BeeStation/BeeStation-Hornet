...
// only_station:
// 0: respect soundtrack option
// 1: play to all
// 2: play only on station
/proc/play_soundtrack_music(var/datum/soundtrack_song/song, list/hearers = null, ignore_prefs = FALSE, play_to_lobby = FALSE, allow_deaf = TRUE, only_station = 0, is_global = TRUE)
	...
	addtimer(CALLBACK(GLOBAL_PROC, /proc/stop_soundtrack_music), initial(song.length) + 1 SECONDS)
