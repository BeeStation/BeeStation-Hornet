...
/datum/atom_hud/alternate_appearance/basic/variants/New()
	..()
	for(var/mob in  GLOB.player_list)
		if(mobShouldSee(mob))
			add_hud_to(mob)

/datum/atom_hud/alternate_appearance/basic/variants/mobShouldSee(mob/M)
	return isvariant(M) || isobserver(M)
