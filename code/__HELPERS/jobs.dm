...
/proc/get_department_by_hud(jobname)
	...
	var/static/hud_to_department_acc = list(
		// Command
		JOB_HUD_RAWCOMMAND = ACCOUNT_SEC_ID,
		JOB_HUD_CAPTAIN = ACCOUNT_SEC_ID,
		JOB_HUD_ACTINGCAPTAIN = ACCOUNT_SEC_ID,

		// Service + Civilian
		JOB_HUD_RAWSERVICE = ACCOUNT_SRV_ID,
		JOB_HUD_HEADOFPERSONNEL = ACCOUNT_SRV_ID,
		JOB_HUD_ASSISTANT = ACCOUNT_CIV_ID,
		JOB_HUD_BARTENDER = ACCOUNT_SRV_ID,
		JOB_HUD_COOK = ACCOUNT_SRV_ID,
		JOB_HUD_BOTANIST = ACCOUNT_SRV_ID,
		JOB_HUD_CURATOR = ACCOUNT_CIV_ID,
		JOB_HUD_CHAPLAIN = ACCOUNT_CIV_ID,
		JOB_HUD_JANITOR = ACCOUNT_SRV_ID,
		JOB_HUD_LAWYER = ACCOUNT_CIV_ID,
		JOB_HUD_MIME = ACCOUNT_SRV_ID,
		JOB_HUD_CLOWN = ACCOUNT_SRV_ID,
		JOB_HUD_STAGEMAGICIAN = ACCOUNT_SRV_ID,
		JOB_HUD_BARBER = ACCOUNT_CIV_ID,

		// Cargo
		JOB_HUD_RAWCARGO = ACCOUNT_CAR_ID,
		JOB_HUD_QUARTERMASTER = ACCOUNT_CAR_ID,
		JOB_HUD_CARGOTECHNICIAN = ACCOUNT_CAR_ID,
		JOB_HUD_SHAFTMINER = ACCOUNT_CAR_ID,

		// Engineering
		JOB_HUD_RAWENGINEERING = ACCOUNT_ENG_ID,
		JOB_HUD_CHIEFENGINEER = ACCOUNT_ENG_ID,
		JOB_HUD_STATIONENGINEER = ACCOUNT_ENG_ID,
		JOB_HUD_ATMOSPHERICTECHNICIAN = ACCOUNT_ENG_ID,

		// Medical
		JOB_HUD_RAWMEDICAL = ACCOUNT_MED_ID,
		JOB_HUD_CHEIFMEDICALOFFICIER = ACCOUNT_MED_ID,
		JOB_HUD_MEDICALDOCTOR = ACCOUNT_MED_ID,
		JOB_HUD_PARAMEDIC = ACCOUNT_MED_ID,
		JOB_HUD_VIROLOGIST = ACCOUNT_MED_ID,
		JOB_HUD_CHEMIST = ACCOUNT_MED_ID,
		JOB_HUD_GENETICIST = ACCOUNT_MED_ID,
		JOB_HUD_PSYCHIATRIST = ACCOUNT_MED_ID,

		// R&D
		JOB_HUD_RAWSCIENCE = ACCOUNT_SCI_ID,
		JOB_HUD_RESEARCHDIRECTOR = ACCOUNT_SCI_ID,
		JOB_HUD_SCIENTIST = ACCOUNT_SCI_ID,
		JOB_HUD_ROBOTICIST = ACCOUNT_SCI_ID,
		JOB_HUD_EXPLORATIONCREW = ACCOUNT_SCI_ID,

		// Security
		JOB_HUD_RAWSECURITY = ACCOUNT_SEC_ID,
		JOB_HUD_HEADOFSECURITY = ACCOUNT_SEC_ID,
		JOB_HUD_SECURITYOFFICER = ACCOUNT_SEC_ID,
		JOB_HUD_WARDEN = ACCOUNT_SEC_ID,
		JOB_HUD_DETECTIVE = ACCOUNT_SEC_ID,
		JOB_HUD_BRIGPHYSICIAN = ACCOUNT_SEC_ID,
		JOB_HUD_DEPUTY = ACCOUNT_SEC_ID,

		// CentCom
		JOB_HUD_RAWCENTCOM = ACCOUNT_CIV_ID,
		JOB_HUD_CENTCOM = ACCOUNT_CIV_ID,

		// ETC
		JOB_HUD_VIP = ACCOUNT_CIV_ID, // Fake VIP gets a station department
		JOB_HUD_KING = ACCOUNT_CIV_ID,
		JOB_HUD_SYNDICATE = ACCOUNT_CIV_ID,
		JOB_HUD_UNKNOWN = ACCOUNT_CIV_ID,
		JOB_HUD_PRISONER = ACCOUNT_CIV_ID
	)
	return hud_to_department_acc[jobname] || ACCOUNT_CIV_ID // default: Civ budget department
