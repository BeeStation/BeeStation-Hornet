...
/proc/baseturfs_string_list(list/values, turf/baseturf_holder)
	...
	//	return values
	if(length(values) > 10)
		stack_trace("/proc/baseturfs_string_list", "The baseturfs list of [baseturf_holder] at [baseturf_holder.x], [baseturf_holder.y], [baseturf_holder.z] is [length(values)]. Values: [values.Join(", ")]")
