...
/proc/assert_sorted(list/list, name, cmp = /proc/cmp_numeric_asc)
	...
	for (var/index in 2 to list.len)
		...
		if (call(cmp)(value, last_value) < 0)
			stack_trace("list_not_sorted", "[name] is not sorted. value at [index] ([value]) is in the wrong place compared to the previous value of [last_value] (when compared to by [cmp])")
