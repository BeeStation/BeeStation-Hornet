...
/proc/log_manifest(ckey, datum/mind/mind,mob/body, latejoin = FALSE)
	if (CONFIG_GET(flag/log_manifest))
		...
		if(!isnull(species))
			WRITE_LOG(GLOB.world_manifest_log, "[ckey] \\ [body.real_name] \\ [mind.get_display_role_for_report(TRUE)]] \\ [latejoin ? "LATEJOIN":"ROUNDSTART"] \\ [species]")
		else
			WRITE_LOG(GLOB.world_manifest_log, "[ckey] \\ [body.real_name] \\ [mind.get_display_role_for_report(TRUE)] \\ [latejoin ? "LATEJOIN":"ROUNDSTART"]")
