...
/datum/holoparasite_ability/major/frenzy/proc/on_ranged_attack(datum/_source, mob/living/target, params)
	...
	if(rush_knockback)
		...
		owner.visible_message(span_danger("[owner.color_name] violently rushes and attacks <span class='name'>[target]</span>, flinging them backwards!"), span_warning("We violently rush and attack <span class='name'>[target]</span>, flinging them backwards!"), ignored_mobs = list(target))
		...
		owner.visible_message(span_danger("[owner.color_name] violently rushes and attacks <span class='name'>[target]</span>!"), span_warning("We violently rush and attack <span class='name'>[target]</span>!"), ignored_mobs = list(target))
