...
/obj/effect/holopara_bluespace_tear/attack_hand(mob/living/carbon/user)
	...
	if(!istype(destination) || QDELETED(destination))
		...
		return
	user.visible_message(span_notice("<span class='name'>[user]</span> begins to effortlessly climb into \the [src], navigating through the tear with unnatural familarity!"), \
	...
	if(!do_after(user, 1.5 SECONDS, src, extra_checks = CALLBACK(src, PROC_REF(_bluespace_tear_crawl_check), user)))
		user.visible_message(span_warning("<span class='name'>[user]</span> backs out of \the [src]!"), span_warning("You were interrupted while trying to navigate \the [src]!"))
		return
	user.visible_message(span_warning("<span class='name'>[user]</span> fully crawls into \the [src], disappearing from view!"), \
