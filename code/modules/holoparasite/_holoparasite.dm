...
/mob/living/simple_animal/hostile/holoparasite/Initialize(_mapload, _key, _name, datum/holoparasite_theme/_theme, _accent_color, _notes, datum/mind/_summoner, datum/holoparasite_stats/_stats)
	. = ..()
	if(!istype(_summoner))
		STACK_TRACE_ADV("Holoparasite initialized without a valid summoner!")
		return INITIALIZE_HINT_QDEL
	if(!istype(_stats))
		STACK_TRACE_ADV("Holoparasite initialized without valid stats!")
		return INITIALIZE_HINT_QDEL
	if(!istype(_theme))
		STACK_TRACE_ADV("Holoparasite initialized without a valid theme!")
		return INITIALIZE_HINT_QDEL
