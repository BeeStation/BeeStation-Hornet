...
/datum/forensics
	...
	var/list/fibers
	/**
	 * List of psyphoza souls on this atom
	 *
	 * Formatting:
	 * * soul = soul
	 */
...
/datum/forensics/New(atom/parent, list/fingerprints, list/hiddenprints, list/blood_DNA, list/fibers, list/souls)
	if(!isatom(parent))
...
/datum/forensics/proc/inherit_new(list/fingerprints, list/hiddenprints, list/blood_DNA, list/fibers, list/souls) //Use of | and |= being different here is INTENTIONAL.
	if (fingerprints)
	...
	if (souls)
		src.souls = LAZY_LISTS_OR(src.souls, souls)
	add_blood_decal()
...
/datum/forensics/proc/wipe_souls(force)
	if(force)
		souls = null
		return TRUE
...
/datum/forensics/proc/add_soul_list(list/_souls)	//list(text)
	if(!length(_souls))
		return
	LAZYINITLIST(souls)
	for(var/i in _souls)
...
/datum/forensics/proc/add_soul(mob/living/carbon/human/H)
