...
/datum/xenoartifact_trait/minor/sentient/proc/setup_sentience(obj/item/xenoartifact/X, ckey)
	...
	log_game("[key_name_admin(man)] took control of the sentient [X]. [X] located at [get_area(src)]")
	man.forceMove(X)
	man.anchored = TRUE
...
/datum/xenoartifact_trait/minor/signalsend/activate(obj/item/xenoartifact/X)
	...
	log_game("[X] sent signal code [X.code] on frequency [X.frequency] at [world.time]. [X] located at [get_area(src)]")
