...
/obj/item/xenoartifact/proc/generate_traits(var/list/blacklist_traits, malf = FALSE)
    ...
    for(var/X in 2 to 4)//Minors
        while((!new_trait) || (new_trait in blacklist_traits))
            new_trait = pick(subtypesof(/datum/xenoartifact_trait/minor))
    ...
    while(!(new_trait) || (new_trait in blacklist_traits))//Major
        new_trait = pick(subtypesof(/datum/xenoartifact_trait/major))
