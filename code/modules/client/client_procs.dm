...
/client/New(TopicData)
	...
	//BEESTATION Only R_ADMIN holders are in GLOB.admins, ditto for R_MENTOR and GLOB.mentors, Others are stored in GLOB.staff
	if(holder)
		...
		if(check_rights_for(src, R_MENTOR))
			GLOB.mentors |= src
		else if(!check_rights_for(src, R_ADMIN|R_MENTOR)) //Other Staff
			GLOB.staff |= src
		holder.owner = src
