...
/datum/job/exploration_crew/equip(mob/living/carbon/human/H, visualsOnly, announce, latejoin, datum/outfit/outfit_override, client/preference_source)
	...
	switch(exploration_job_id)
		...
		if(1)
			to_chat(H, span_noticebig("You are the exploration team's <span class'sciradio'>Scientist</span>!"))
		...
		if(2)
			to_chat(H, span_noticebig("You are the exploration team's <span class'medradio'>Medical Doctor</span>!"))
		...
		if(3)
			to_chat(H, span_noticebig("You are the exploration team's <span class'engradio'>Engineer</span>!"))
