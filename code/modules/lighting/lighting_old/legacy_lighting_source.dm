...
/datum/legacy_light_source
	...
	var/list/datum/legacy_lighting_corner/effect_str     // List used to store how much we're affecting corners.
...
/datum/legacy_light_source/New(var/atom/owner, var/atom/top)
	source_atom = owner // Set our new owner.
	LAZYADD(source_atom.legacy_light_sources, src)
	top_atom = top
	if (top_atom != source_atom)
		LAZYADD(top_atom.legacy_light_sources, src)
...
/datum/legacy_light_source/Destroy(force)
	remove_lum()
	if (source_atom)
		LAZYREMOVE(source_atom.legacy_light_sources, src)

	if (top_atom)
		LAZYREMOVE(top_atom.legacy_light_sources, src)
...
/datum/legacy_light_source/proc/update(var/atom/new_top_atom)
	// This top atom is different.
	if (new_top_atom && new_top_atom != top_atom)
		if(top_atom != source_atom && top_atom.legacy_light_sources) // Remove ourselves from the light sources of that top atom.
			LAZYREMOVE(top_atom.legacy_light_sources, src)

		top_atom = new_top_atom

		if (top_atom != source_atom)
			LAZYADD(top_atom.legacy_light_sources, src) // Add ourselves to the light sources of our new top atom.

	EFFECT_UPDATE(LIGHTING_CHECK_UPDATE)

// Will force an update without checking if it's actually needed.
/datum/legacy_light_source/proc/force_update()
	EFFECT_UPDATE(LIGHTING_FORCE_UPDATE)

// Will cause the light source to recalculate turfs that were removed or added to visibility only.
/datum/legacy_light_source/proc/vis_update()
	EFFECT_UPDATE(LIGHTING_VIS_UPDATE)

// Decompile the hexadecimal colour into lumcounts of each perspective.
/datum/legacy_light_source/proc/parse_light_color()
	if (light_color)
...
/datum/legacy_light_source/proc/remove_lum()
	applied = FALSE
	var/thing
...
/datum/legacy_light_source/proc/recalc_corner(var/datum/legacy_lighting_corner/C)
...
/datum/legacy_light_source/proc/update_corners()
	var/update = FALSE
	...
	var/list/datum/legacy_lighting_corner/corners = list()
	...
	if (source_turf)
		...
		for(T in view(CEILING(light_range, 1), source_turf))
			var/area/A = T.loc
			if(!A.legacy_lighting && !T.legacy_light_sources)
				continue
