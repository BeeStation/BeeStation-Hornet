...
/obj/item/slimecross/charged/green/do_effect(mob/user)
	...
	if(!istype(H))
		...
		return
	var/racechoice = input(H, "Choose your slime subspecies.", "Slime Selection") as null|anything in sortList(subtypesof(/datum/species/oozeling), GLOBAL_PROC_REF(cmp_typepaths_asc))
	if(!racechoice)
