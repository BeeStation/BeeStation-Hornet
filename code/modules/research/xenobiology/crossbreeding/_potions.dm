...
/obj/item/slimepotion/peacepotion/attack(mob/living/peace_target, mob/user)
	...
	if(!do_after(user, 100, target = peace_target))
		return
	if(peace_target != user)
		to_chat(user, "<span class = 'notice'>You feed [peace_target] [src]!</span>")
	else
...
/obj/item/slimepotion/lovepotion/attack(mob/living/love_target, mob/user)
	...
	if(love_target.has_status_effect(/datum/status_effect/in_love))
		to_chat(user, "<span class = 'warning'>[love_target] is already lovestruck!</span>")
		return ..()
	...
	if(!do_after(user, 50, target = love_target))
		return
	to_chat(user, "<span class = 'notice'>You feed [love_target] the love potion!</span>")
	to_chat(love_target, "<span class = 'notice'>You develop feelings for [user], and anyone [user.p_they()] like[user.p_s()].</span>")
...
/obj/item/slimepotion/slime_reviver/attack(mob/living/basic/slime/revive_target, mob/user)
	...
	revive_target.visible_message("<span class = 'notice'>[revive_target] is filled with renewed vigor and blinks awake!</span>")
