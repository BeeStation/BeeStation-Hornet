...
/datum/status_effect/lovecookie/tick()
	...
	if(length(huggables))
		...
		owner.visible_message("<span class = 'notice'>[owner] hugs [hugged]!"), span_notice("You hug [hugged]!</span>")
...
/datum/status_effect/stabilized/purple/get_examine_text()
	if(healed_last_tick)
		return "<span class = 'warning'>[owner.p_they(TRUE)] [owner.p_are()] regenerating slowly, purplish goo filling in small injuries!</span>"
...
/datum/status_effect/stabilized/yellow/get_examine_text()
	return "<span class = 'warning'>Nearby electronics seem just a little more charged wherever [owner.p_they()] go[owner.p_es()].</span>"
...
/datum/status_effect/stabilized/darkpurple/tick()
	...
	if(item)
		if(IS_EDIBLE(item) && (item.microwave_act(microwaver = owner) & COMPONENT_MICROWAVE_SUCCESS))
			to_chat(owner, "<span class = 'warning'>[linked_extract] flares up brightly, and your hands alone are enough to cook [item]!</span>")
		else
...
/datum/status_effect/stabilized/darkpurple/get_examine_text()
	return "<span class = 'notice'>[owner.p_their(TRUE)] fingertips burn brightly!</span>"
...
/datum/status_effect/stabilized/green/get_examine_text()
	if(prob(50))
		return "<span class = 'warning'>[owner.p_they(TRUE)] look[owner.p_s()] a bit green and gooey...</span>"
...
/datum/status_effect/stabilized/oil/get_examine_text()
	return "<span class = 'warning'>[owner.p_they(TRUE)] smell[owner.p_s()] of sulfur and oil!</span>"
...
/datum/status_effect/stabilized/black/get_examine_text()
	...
	return "<span class = 'warning'>[owner.p_they(TRUE)] [owner.p_are()] draining health from [draining]!</span>"
...
/datum/status_effect/stabilized/black/tick()
	...
	if(drained.stat == DEAD)
		to_chat(owner, "<span class = 'warning'>[drained] is dead, you cannot drain anymore life from them!</span>")
		draining_ref = null
		return
...
/datum/status_effect/stabilized/adamantine/get_examine_text()
	return "<span class = 'warning'>[owner.p_they(TRUE)] [owner.p_have()] strange metallic coating on [owner.p_their()] skin.</span>"
