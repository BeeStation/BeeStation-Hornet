...
/obj/item/slime_cookie/attack(mob/living/M, mob/user)
	var/fed = FALSE
	if(M == user)
		M.visible_message("<span class = 'notice'>[user] eats [src]!"), span_notice("You eat [src].</span>")
		fed = TRUE
	else
		M.visible_message(span_danger("[user] tries to force [M] to eat [src]!"), span_userdanger("[user] tries to force you to eat [src]!"))
		if(do_after(user, 20, target = M))
			fed = TRUE
			M.visible_message(span_danger("[user] forces [M] to eat [src]!"), "<span class = 'warning'>[user] forces you to eat [src].</span>")
	if(fed)
