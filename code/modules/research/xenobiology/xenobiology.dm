...
/obj/item/slimepotion/genderchange/attack(mob/living/L, mob/user)
	...
	L.visible_message(span_danger("[user] starts to feed [L] a gender change potion!"),
		span_userdanger("[user] starts to feed you a gender change potion!"))

	if(!do_after(user, 50, target = L))
		return
	
	to_chat(user, span_notice("You feed [L] the gender change potion!"))
