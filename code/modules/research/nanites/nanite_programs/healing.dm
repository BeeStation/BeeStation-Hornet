...
/datum/nanite_program/defib/proc/check_revivable()
	...
	if((C.getBruteLoss() >= MAX_REVIVE_BRUTE_DAMAGE) || (C.getFireLoss() >= MAX_REVIVE_FIRE_DAMAGE) || !C.can_be_revived()) //too damaged
		return FALSE
	if(!C.get_organ_by_type(/obj/item/organ/internal/heart)) //what are we even shocking
		return FALSE
	var/obj/item/organ/internal/brain/BR = C.get_organ_by_type(/obj/item/organ/internal/brain)
