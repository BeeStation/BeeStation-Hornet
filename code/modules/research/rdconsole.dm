...
/obj/machinery/computer/rdconsole/proc/ui_protolathe_materials()		//Legacy code
	...
	for(var/mat_id in mat_container.materials)
		...
		l += "* [amount] of [M.name]: "
		if(amount >= MINERAL_MATERIAL_AMOUNT) l += "<A href='?src=[REF(src)];ejectsheet=[M.type];eject_amt=1'>Eject</A> [RDSCREEN_NOBREAK]"
		if(amount >= MINERAL_MATERIAL_AMOUNT*5) l += "<A href='?src=[REF(src)];ejectsheet=[M.type];eject_amt=5'>5x</A> [RDSCREEN_NOBREAK]"
		if(amount >= MINERAL_MATERIAL_AMOUNT) l += "<A href='?src=[REF(src)];ejectsheet=[M.type];eject_amt=50'>All</A>[RDSCREEN_NOBREAK]"
		l += ""
...
/obj/machinery/computer/rdconsole/proc/ui_circuit_materials()	//Legacy code!
	...
	for(var/mat_id in mat_container.materials)
		...
		var/amount = mat_container.materials[mat_id]
		l += "* [amount] of [M.name]: "
		if(amount >= MINERAL_MATERIAL_AMOUNT) l += "<A href='?src=[REF(src)];imprinter_ejectsheet=[M.type];eject_amt=1'>Eject</A> [RDSCREEN_NOBREAK]"
		if(amount >= MINERAL_MATERIAL_AMOUNT*5) l += "<A href='?src=[REF(src)];imprinter_ejectsheet=[M.type];eject_amt=5'>5x</A> [RDSCREEN_NOBREAK]"
		if(amount >= MINERAL_MATERIAL_AMOUNT) l += "<A href='?src=[REF(src)];imprinter_ejectsheet=[M.type];eject_amt=50'>All</A>[RDSCREEN_NOBREAK]</div>"
	return l
...
/obj/machinery/computer/rdconsole/Topic(raw, ls)
	...
	if(ls["ejectsheet"]) //Causes the protolathe to eject a sheet of material
		...
		if(!linked_lathe.materials.mat_container)
			...
			return
		linked_lathe.eject_sheets(getmaterialref(ls["ejectsheet"]), ls["eject_amt"])
	...
	if(ls["imprinter_ejectsheet"]) //Causes the imprinter to eject a sheet of material
		...
		if(!linked_imprinter.materials.mat_container)
			...
			return
		linked_imprinter.eject_sheets(getmaterialref(ls["imprinter_ejectsheet"]), ls["eject_amt"])
	if(ls["disk_slot"])
