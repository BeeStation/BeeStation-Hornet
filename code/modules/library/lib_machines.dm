	...
	if(href_list["setauthor"])
		...
		if(newauthor)
			if(length(newauthor) <= 45)
				...
				tgui_alert("Unable to set author. The field must be fewer than 45 characters.")
	if(href_list["setcategory"])
			...
			if(scanner.cache)
				...
				if(choice == "Confirm")
					if (!SSdbcore.Connect())
						...
						tgui_alert("The title and author fields must each be 45 characters or fewer. Check your entry and try again.")
						return
					else
