	...
	if(target.mind)
		...
		if(abductee?.custom_objective)
			if(istype(target.getorganslot(ORGAN_SLOT_HEART), /obj/item/organ/heart/gland))
				...
				var/datum/objective/abductee/custom/custom_objective = locate() in abductee.objectives
				if(custom_objective)
					message_admins("[ADMIN_LOOKUPFLW(user)] removed [ADMIN_LOOKUPFLW(target)]'s custom abductee objective ([custom_objective.explanation_text]) with a lobotomy.")
					log_game("[key_name(user)] removed [key_name(target)]'s custom abductee objective ([custom_objective.explanation_text]) with a lobotomy.")
					abductee.objectives -= custom_objective
					qdel(custom_objective)
					to_chat(target, "<span class='userdanger'>You're <span class='hypnophrase'>FREE</span>! The objective imprinted upon your broken psyche no longer has any hold over you, although you cannot remember any actions you took while under the influence of it...</span>")
					switch(target.stat)
						if(CONSCIOUS)
