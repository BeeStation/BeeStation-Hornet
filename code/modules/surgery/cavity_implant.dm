...
/datum/surgery_step/handle_cavity/preop(mob/user, mob/living/carbon/human/target, target_zone, obj/item/tool, datum/surgery/surgery)
	...
	item_for_cavity = CH.cavity_item
	if(tool)
		display_results(
			user,
			target,
		...
		log_combat(user, target, "searched for cavity item [item_for_cavity ? "([item_for_cavity.name])" : null] in")
...
/datum/surgery_step/handle_cavity/success(mob/user, mob/living/carbon/human/target, target_zone, obj/item/tool, datum/surgery/surgery = FALSE)
	...
	if(tool)
		...
		if(item_for_cavity)
			...
			log_combat(user, target, "extracted [item_for_cavity.name] from cavity in")
			return ..()
		else
