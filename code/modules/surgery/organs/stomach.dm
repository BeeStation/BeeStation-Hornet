...
/obj/item/organ/stomach/proc/handle_disgust(mob/living/carbon/human/H, delta_time, times_fired)
	if(H.disgust)
		...
		if(H.disgust >= DISGUST_LEVEL_GROSS)
			...
			H.adjust_timed_status_effect(-6 SECONDS, /datum/status_effect/jitter)
		if(H.disgust >= DISGUST_LEVEL_VERYGROSS)
			if(DT_PROB(pukeprob, delta_time)) //iT hAndLeS mOrE ThaN PukInG
				H.adjust_timed_status_effect(2.5 SECONDS, /datum/status_effect/confusion)
				H.adjust_timed_status_effect(2 SECONDS, /datum/status_effect/speech/stutter)
				H.vomit(10, distance = 0, toxic = 0)
