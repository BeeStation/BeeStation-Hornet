...
/obj/item/organ/internal/cyberimp/chest/thrusters/Insert(mob/living/carbon/M, special, movement_flags)
	. = ..()
	if(!ion_trail)
		ion_trail = new
	ion_trail.set_up(M)

/obj/item/organ/internal/cyberimp/chest/thrusters/Remove(mob/living/carbon/M, special, movement_flags)
	if(on)
		toggle(silent = TRUE)
	..()
