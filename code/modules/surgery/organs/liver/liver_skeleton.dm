...
/obj/item/organ/liver/bone/handle_chemical(mob/living/carbon/organ_owner, datum/reagent/chem, seconds_per_tick, times_fired)
	...
	if(istype(chem, /datum/reagent/toxin/bonehurtingjuice))
		...
		if(chem.overdosed)
			if(DT_PROB(2, seconds_per_tick)) //big oof
				...
				if(bodypart)
					playsound(organ_owner, "desecration", 50, vary = TRUE) //You just want to socialize
	...
	if(chem.type == /datum/reagent/consumable/milk)
		...
		organ_owner.heal_bodypart_damage(brute = milk_brute_healing * REM * seconds_per_tick, burn = milk_burn_healing * REM * seconds_per_tick)
