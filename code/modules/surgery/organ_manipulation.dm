...
/datum/surgery_step/manipulate_organs/preop(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery)
	...
	if(istype(tool, /obj/item/organ/brain/positron))
		...
		if(affected.status != ORGAN_ROBOTIC)
			to_chat(user, span_notice("You can't put [tool] into a meat enclosure!"))
			return -1
		if(!isipc(target))
			to_chat(user, span_notice("[target] does not have the proper connectors to interface with [tool]."))
			return -1
		if(target_zone != "chest")
			to_chat(user, span_notice("You have to install [tool] in [target]'s chest!"))
		if(target.internal_organs_slot["brain"])
			to_chat(user, span_notice("[target] already has a brain! You'd rather not find out what would happen with two in there."))
			return -1
		user.visible_message(span_notice("[user] begins to insert [tool] into [target]'s [parse_zone(target_zone)]."),
