...
/datum/surgery_step/manipulate_organs/preop(mob/user, mob/living/carbon/target, obj/item/tool, datum/surgery/surgery)
	...
	if(isorgan(tool))
		...
		if(!meatslab.useable)
			...
			return -1
		display_results(user, target, span_notice("You begin to insert [tool] into [target]'s [parse_zone(check_zone(surgery.location))]..."),
			span_notice("[user] begins to insert [tool] into [target]'s [parse_zone(check_zone(surgery.location))]."),
			span_notice("[user] begins to insert something into [target]'s [parse_zone(check_zone(surgery.location))]."))
