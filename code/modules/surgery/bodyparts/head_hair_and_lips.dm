...
/obj/item/bodypart/head/proc/get_hair_and_lips_icon(dropped)
	...
	if(!facial_hair_hidden && lip_style && (head_flags & HEAD_LIPS))
		...
		//Overlay
		var/image/lip_overlay = image('icons/mob/species/human/human_face.dmi', "lips_[lip_style]", layer = CALCULATE_MOB_OVERLAY_LAYER(BODY_LAYER))
	...
	if(!facial_hair_hidden && facial_hairstyle && (head_flags & HEAD_FACIAL_HAIR))
		...
		if(sprite_accessory)
			...
			if(facial_hair_gradient_style != "None")
				...
				var/image/facial_hair_gradient_overlay = get_gradient_overlay(sprite_accessory.icon, sprite_accessory.icon_state, CALCULATE_MOB_OVERLAY_LAYER(HAIR_LAYER), SSaccessories.facial_hair_gradients_list[facial_hair_gradient_style], facial_hair_gradient_color)
	...
	if(!(show_debrained && (head_flags & HEAD_DEBRAIN)) && !hair_hidden && hairstyle && (head_flags & HEAD_HAIR))
		...
		if(hair_sprite_accessory)
			//Overlay
			hair_overlay = image(hair_sprite_accessory.icon, hair_sprite_accessory.icon_state, CALCULATE_MOB_OVERLAY_LAYER(HAIR_LAYER))
			...
			if(hair_gradient_style != "None")
				...
				var/image/hair_gradient_overlay = get_gradient_overlay(hair_sprite_accessory.icon, hair_sprite_accessory.icon_state, CALCULATE_MOB_OVERLAY_LAYER(HAIR_LAYER), SSaccessories.hair_gradients_list[hair_gradient_style], hair_gradient_color)
