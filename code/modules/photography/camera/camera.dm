...
/obj/item/camera/proc/captureimage(atom/target, mob/user, flag, size_x = 1, size_y = 1)
	...
	for(var/turf/placeholder in block(locate(target_turf.x - size_x, target_turf.y - size_y, target_turf.get_z_level()), locate(target_turf.x + size_x, target_turf.y + size_y, target_turf.get_z_level())))
