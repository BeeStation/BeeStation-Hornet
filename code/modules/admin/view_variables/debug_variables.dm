...
/proc/debug_variable(name, value, level, datum/owner, sanitize = TRUE, display_flags = NONE) //if D is a list, name will be index, and value will be assoc value.
	...
	if(level > 0 || islist(owner)) //handling keys in assoc lists
		if(istype(name,/datum))
			name_part = "<a href='byond://?[HREF_COMMAND(var_edit)][HrefToken()];[HREF_PARAM(var_edit, Vars, REF(name))]'>[VV_HTML_ENCODE(name)] [REF(name)]</a>"
		else if(islist(name))
			var/list/list_value = name
			name_part = "<a href='byond://?[HREF_COMMAND(var_edit)][HrefToken()];[HREF_PARAM(var_edit, Vars, REF(name))]'> /list ([length(list_value)]) [REF(name)]</a>"
...
/proc/_debug_variable_value(name, datum/value, level, datum/owner, sanitize, display_flags)
	...
	if(isappearance(value)) // Reminder: Do not replace this into /image/debug_variable_value() proc. /appearance can't do that.
		return "<a href='byond://?[HREF_COMMAND(var_edit)][HrefToken()];[HREF_PARAM(var_edit, Vars, REF(value))]'>/appearance ([span_value("[get_appearance_vv_summary_name(value)]")]) [REF(value)]</a>"

	if(isimage(value))
		var/image/image = value
		return "<a href='byond://?[HREF_COMMAND(var_edit)][HrefToken()];[HREF_PARAM(var_edit, Vars, REF(value))]'>[image.type] ([span_value("[get_appearance_vv_summary_name(image)]")]) [REF(value)]</a>"
	...
	if(islist)
		...
		if(can_open_list_window)
			var/href_reference_string = \
				...
				: "Vars=[REF(value)]"
			a_open = "<a href='byond://?[HREF_COMMAND(var_edit)][HrefToken()];[href_reference_string]'>"
			a_close = "</a>"
...
/datum/proc/debug_variable_value(name, level, datum/owner, sanitize, display_flags)
	if("[src]" != "[type]") // If we have a name var, let's use it.
		return "<a href='byond://?[HREF_COMMAND(var_edit)][HrefToken()];[HREF_PARAM(var_edit, Vars, REF(src))]'>[src] [type] [REF(src)]</a>"
	else
		return "<a href='byond://?[HREF_COMMAND(var_edit)][HrefToken()];[HREF_PARAM(var_edit, Vars, REF(src))]'>[type] [REF(src)]</a>"

/datum/weakref/debug_variable_value(name, level, datum/owner, sanitize, display_flags)
	. = ..()
	return "[.] <a href='byond://?[HREF_COMMAND(var_edit)][HrefToken()];[HREF_PARAM(var_edit, Vars, reference)]'>(Resolve)</a>"
