...
/client/proc/vv_do_list(list/target, href_list)
	...
	if(check_rights(R_VAREDIT))
		...
		if(special_varname)
			var/special_list_level = GLOB.vv_special_lists[special_varname]
			if(special_list_level == VV_LIST_EDITABLE)
				log_world("### vv_do_list() called: [src] attempted to edit a special list ([special_varname]) Security-level:[special_list_level](allowed)")
				log_admin("[key_name(src)] attempted to edit  a special list ([special_varname]) Security-level:[special_list_level](allowed)")
			else // fuck you exploiters
				log_world("### vv_do_list() called: [src] attempted to edit a special list ([special_varname]), but denied due to the Security-level:[special_list_level]")
				log_admin("[key_name(src)] attempted to edit a special list ([special_varname]), but denied due to the Security-level:[special_list_level]")
				message_admins("[key_name_admin(src)] attempted to edit a special list ([special_varname]), but denied due to the Security-level:[special_list_level]. Bonk this guy.")
				return
		if(target_index)
