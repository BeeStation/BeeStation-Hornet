...
/datum/help_ticket/admin/MessageNoRecipient(msg, add_to_ticket = TRUE, sanitized = FALSE)
	...
	var/admin_msg = span_adminnotice("<span class='adminhelp'>Ticket [TicketHref("#[id]", ref_src)]</span><b>: [LinkedReplyName(ref_src)] [FullMonty(ref_src)]:</b> <span class='linkify'>[keywords_lookup(sanitized_msg)]</span>")
	...
	if(add_to_ticket)
		to_chat(initiator,
			type = message_type,
			html = span_adminnotice("PM to-<b>Admins</b>: <span class='linkify'>[sanitized_msg]</span>"))
