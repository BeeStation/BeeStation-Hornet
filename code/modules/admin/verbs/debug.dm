	...
	for(var/test_obj in subtypesof(/obj/item))
		...
		if(sprite.worn_icon)
			if(!icon_exists(sprite.worn_icon, sprite.icon_state))
				to_chat(user, span_warning("ERROR sprites for [sprite.type]. Slot Flags are [sprite.slot_flags]."))
