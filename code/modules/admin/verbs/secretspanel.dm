/client/proc/openSecretsPanel()
	set name = "Secrets"
	set category = "Admin"
	var/datum/SecretsPanel/UI  = new(usr)
	UI.ui_interact(usr)

/datum/SecretsPanel
	var/datum/admins/admin_datum

/datum/SecretsPanel/New(mob/user)
	var/client/C = user.client
	if(!C)
		return
	admin_datum = GLOB.admin_datums[C.ckey]
	if(!admin_datum)
		log_admin_private("[C.ckey] attempted to open the Secrets panel, but had no admin datum")
		message_admins("[C.ckey] attempted to open the Secrets panel, but had no admin datum")
		return

/datum/SecretsPanel/ui_interact(mob/user, ui_key = "secrets_panel", datum/tgui/ui = null, force_open = TRUE, datum/tgui/master_ui = null, datum/ui_state/state = GLOB.admin_state)
	if(!check_rights(0))
		return
...
/datum/SecretsPanel/ui_data(mob/user)
	if(!admin_datum)
		log_admin_private("[user] sent a request to interact with the Secrets panel without sufficient rights.")
		message_admins("[user] sent a request to interact with the Secrets panel without sufficient rights.")
		return
	var/list/data = list()
	var/list/category

	/*
	Each command is a list that will be read like [Name, Action(see ui_act), Color(optional)]
	...
	*/
	...
	if(check_rights(R_FUN,0))
		category = list()
		category["name"] = "Fun Secrets"
		category["commands"] = list(
			list("Trigger a Virus Outbreak", "virus"),
			list("Turn all humans into monkeys", "monkey", "red"),
			...
			list("Mass Antag (Everyone is the traitor)", "traitor_all", "red"),
			list("Summon Guns", "guns"),
			list("Summon Magic", "magic"),
			list("Summon Events (Toggle)", "events"),
			list("There can only be one!", "onlyone", "red"),
			list("There can only be one! (40-second delay)", "delayed_onlyone", "red"),
			...
			list("The floor is lava! (DANGEROUS: extremely lame)", "floorlava", "red"),
			...
			list("Change bomb cap", "changebombcap", "red"),
			list("Mass Purrbation", "masspurrbation", "red"),
	...
	if(check_rights(R_DEBUG,0))
		category = list()
		category["name"] = "Security Level Elevated"
		category["commands"] = list(
...
/datum/SecretsPanel/ui_act(action, params)
	...
	switch(action)
		...
		if("admin_log")
			var/cats = "<B>Admin Log<HR></B>"
			for(var/l in GLOB.admin_log)
				cats += "<li>[l]</li>"
			if(!GLOB.admin_log.len)
				cats += "No-one has done anything this round!"
			usr << browse(cats, "window=admin_log")

		if("mentor_log") // hippie start -- access mentor log
			admin_datum.MentorLogSecret() // hippie end

		if("show_admins")
			var/cats = "<B>Current admins:</B><HR>"
			if(GLOB.admin_datums)
				for(var/ckey in GLOB.admin_datums)
					var/datum/admins/D = GLOB.admin_datums[ckey]
					cats += "[ckey] - [D.rank.name]<br>"
				usr << browse(cats, "window=showadmins;size=600x500")
		...
		if("list_bombers")
			if(!check_rights(R_ADMIN))
				return
			var/cats = "<B>Bombing List</B><HR>"
			for(var/l in GLOB.bombers)
				cats += text("[l]<BR>")
			usr << browse(cats, "window=bombers")

		if("list_signalers")
			if(!check_rights(R_ADMIN))
				return
			var/cats = "<B>Showing last [length(GLOB.lastsignalers)] signalers.</B><HR>"
			for(var/sig in GLOB.lastsignalers)
				cats += "[sig]<BR>"
			usr << browse(cats, "window=lastsignalers;size=800x500")

		if("list_lawchanges")
			if(!check_rights(R_ADMIN))
				return
			var/cats = "<B>Showing last [length(GLOB.lawchanges)] law changes.</B><HR>"
			for(var/sig in GLOB.lawchanges)
				cats += "[sig]<BR>"
			usr << browse(cats, "window=lawchanges;size=800x500")
		...
		if("manifest")
			if(!check_rights(R_ADMIN))
				return
			var/cats = "<B>Showing Crew Manifest.</B><HR>"
			cats += "<table cellspacing=5><tr><th>Name</th><th>Position</th></tr>"
			for(var/datum/data/record/t in GLOB.data_core.general)
				cats += "<tr><td>[t.fields["name"]]</td><td>[t.fields["rank"]]</td></tr>"
			cats += "</table>"
			usr << browse(cats, "window=manifest;size=440x410")
		if("DNA")
			if(!check_rights(R_ADMIN))
				return
			var/cats = "<B>Showing DNA from blood.</B><HR>"
			cats += "<table cellspacing=5><tr><th>Name</th><th>DNA</th><th>Blood Type</th></tr>"
			for(var/mob/living/carbon/human/H in GLOB.carbon_list)
				if(H.ckey)
					cats += "<tr><td>[H]</td><td>[H.dna.unique_enzymes]</td><td>[H.dna.blood_type]</td></tr>"
			cats += "</table>"
			usr << browse(cats, "window=DNA;size=440x410")
		if("fingerprints")
			if(!check_rights(R_ADMIN))
				return
			var/cats = "<B>Showing Fingerprints.</B><HR>"
			cats += "<table cellspacing=5><tr><th>Name</th><th>Fingerprints</th></tr>"
			for(var/mob/living/carbon/human/H in GLOB.carbon_list)
				if(H.ckey)
					cats += "<tr><td>[H]</td><td>[md5(H.dna.uni_identity)]</td></tr>"
			cats += "</table>"
			usr << browse(cats, "window=fingerprints;size=440x410")
