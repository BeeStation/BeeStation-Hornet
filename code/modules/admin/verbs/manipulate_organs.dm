/client/proc/manipulate_organs(mob/living/carbon/C in world)
	...
	switch(operation)
		if("add organ")
			...
			var/obj/item/organ/organ = input("Select organ type:", "Organ Manipulation", null) as null|anything in sort_list(organs)
			if(!organ)
				return
			organ = organs[organ]
			organ = new organ
		...
		if("add implant")
			...
			var/obj/item/implant/organ = input("Select implant type:", "Organ Manipulation", null) as null|anything in sort_list(organs)
			if(!organ)
				return
			organ = organs[organ]
			organ = new organ
		...
		if("drop organ/implant", "remove organ/implant")
			...
			var/obj/item/organ = input("Select organ/implant:", "Organ Manipulation", null) as null|anything in sort_list(organs)
			if(!organ)
				return
			organ = organs[organ]
			if(!organ)
