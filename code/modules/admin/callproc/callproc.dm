...
/client/proc/callproc_blocking(list/get_retval)
	...
	var/procpath = capped_input(usr, "Proc path, eg: /proc/fake_blood","Path:", null)
	if(!procpath)
		return
	...
	if(findtextEx(trim(LOWER_TEXT(procname)), "rustg"))
		...
		message_admins(span_userdanger("[key_name_admin(src)] attempted to proc call rust-g procs([html_strip]). Inform the host <u>at once</u>."))
...
/client/proc/callproc_datum(datum/A as null|area|mob|obj|turf)
	...
	var/procname = capped_input(usr, "Proc name, eg: fake_blood","Proc:", null)
	if(!procname)
		return
