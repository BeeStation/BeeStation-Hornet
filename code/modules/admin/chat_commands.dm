...
/datum/tgs_chat_command/ircstatus/Run(datum/tgs_chat_user/sender, params)
	...
	status += "Players: [GLOB.clients_unsafe.len] (Active: [get_active_player_count(0,1,0)]). Mode: [SSticker.mode ? SSticker.mode.name : "Not started"]."
...
/datum/tgs_chat_command/irccheck/Run(datum/tgs_chat_user/sender, params)
	...
	return new /datum/tgs_message_content("[GLOB.round_id ? "Round #[GLOB.round_id]: " : ""][GLOB.clients_unsafe.len] players on [SSmapping.config.map_name], Mode: [GLOB.master_mode]; Round [SSticker.HasRoundStarted() ? (SSticker.IsRoundInProgress() ? "Active" : "Finishing") : "Starting"] -- [server ? server : "[world.internet_address]:[world.port]"]")
