...
/datum/mafia_ability/attack_player/perform_action_target(datum/mafia_controller/game, datum/mafia_role/day_target)
	...
	if(!target_role.kill(game, host_role, FALSE))
		to_chat(host_role.body, "<span class='danger'>Your attempt at killing [target_role.body.real_name] was prevented!</span.")
	else
		to_chat(target_role.body, "<span class='userdanger'>You have been [attack_action] \a [host_role.name]!</span>")
		if(honorable && (target_role.team != MAFIA_TEAM_TOWN))
			to_chat(host_role.body, "<span class='userdanger'>You have killed an innocent crewmember. You will die tomorrow night.</span>")
...
/datum/mafia_ability/attack_player/proc/internal_affairs(datum/mafia_controller/game)
	SIGNAL_HANDLER
	to_chat(host_role.body, "<span class='userdanger'>You have been killed by Nanotrasen Internal Affairs!</span>")
