/datum/unit_test/shuttle_checker/Run()
	...
	for(var/shuttle_id in SSmapping.shuttle_templates)
		...
		if (shuttle_horizontal_size > shuttle.width)
			TEST_FAIL("Shuttle [shuttle.name] has a docking port that is too large ([shuttle_horizontal_size] > [shuttle.width]) (Shuttle width/height starts from 0)")
		if (shuttle_vertical_size > shuttle.height)
			TEST_FAIL("Shuttle [shuttle.name] has a docking port that is too large ([shuttle_vertical_size] > [shuttle.height]) (Shuttle width/height starts from 0)")
