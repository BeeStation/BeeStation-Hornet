/datum/unit_test/on_mob_end_metabolize/Run()
	...
	var/datum/reagents/reagent_holder = user.get_reagent_holder()
	...
	TEST_ASSERT(reagent_holder.has_reagent(meth), "User does not have meth in their system after consuming it")
	...
	user.Life()
	TEST_ASSERT(!reagent_holder.has_reagent(meth), "User still has meth in their system when it should've finished metabolizing")
