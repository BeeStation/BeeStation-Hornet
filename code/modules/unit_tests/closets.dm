...
/datum/unit_test/closets/Run()
	...
	for (var/datum/objective_item/objective_item_path as anything in subtypesof(/datum/objective_item))
	...
	for(var/closet_type in all_closets)
		...
		for (var/obj/item/item in closet.contents - immediate_contents)
			...
			if (item.resistance_flags & INDESTRUCTIBLE)
				TEST_FAIL("[closet_type] contains the indestructible item, [item.type], in PopulateContents(). This should be in populate_contents_immediate() instead.")
