...
/datum/unit_test/mob_damage/proc/test_biotypes(mob/living/carbon/human/consistent/dummy)
	...
	if(!test_apply_damage(dummy, 1, amount_after = 2, included_types = TOXLOSS|CLONELOSS, biotypes = MOB_ORGANIC))
		TEST_FAIL("ABOVE FAILURE: plasmaman did not take damage with biotypes = MOB_ORGANIC")
	...
	if(!test_apply_damage(dummy, -2, expected = 0, amount_after = 2, included_types = TOXLOSS|CLONELOSS, biotypes = MOB_ORGANIC))
		TEST_FAIL("ABOVE FAILURE: human took damage with biotypes = MOB_ORGANIC")

	// Force heal some of the damage. When forced = TRUE the damage/healing gets applied no matter what.
	if(!test_apply_damage(dummy, -1, amount_after = 1, included_types = TOXLOSS|CLONELOSS, biotypes = MOB_ORGANIC, forced = TRUE))
		TEST_FAIL("ABOVE FAILURE: human did not get healed when biotypes = MOB_ORGANIC and forced = TRUE")
...
/datum/unit_test/mob_damage/animal/verify_damage(mob/living/testing_mob, amount, expected, included_types = ALL)
	...
	if(included_types & BRUTELOSS)
		var/expected_brute = isnull(expected) ? amount * 5 : expected
		TEST_ASSERT_EQUAL(round(testing_mob.getBruteLoss(), 1), expected_brute, \
			"[testing_mob] should have [expected_brute] brute damage, instead they have [testing_mob.getBruteLoss()]!")
	if(included_types & FIRELOSS)
