...
/datum/unit_test/mob_damage/proc/test_biotypes(mob/living/carbon/human/consistent/dummy)
	...
	if(!test_apply_damage(dummy, 1, amount_after = 2, included_types = TOXLOSS|CLONELOSS, biotypes = MOB_MINERAL))
	...
	if(!test_apply_damage(dummy, -2, expected = 0, amount_after = 2, included_types = TOXLOSS|CLONELOSS, biotypes = MOB_MINERAL))
	...
	if(!test_apply_damage(dummy, -1, amount_after = 1, included_types = TOXLOSS|CLONELOSS, biotypes = MOB_MINERAL, forced = TRUE))
