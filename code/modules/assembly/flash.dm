...
/obj/item/assembly/flash/proc/flash_carbon(mob/living/carbon/M, mob/user, power = 15, targeted = TRUE, generic_message = FALSE)
	...
	if(targeted)
		if(M.flash_act(1, 1))
			if(M.has_status_effect(STATUS_EFFECT_FLASHED))
				if(user)
					visible_message("<span class='disarm'>[user] partially blinds [M] with the flash!</span>")
					to_chat(user, "<span class='danger'>You partially blind [M] with the flash!</span>")
					to_chat(M, "<span class='userdanger'>[user] partially blinds you with the flash!</span>")
				else
					to_chat(M, "<span class='userdanger'>You are partially blinded by [src]!</span>")
				M.blind_eyes(20)
				M.blur_eyes(40)
				M.confused = (M.confused + 5)
			else
				if(user)
