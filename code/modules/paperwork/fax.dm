...
/datum/admins/proc/fax_panel(mob/living/user)
	var/html = "<A align='right' href='?src=[REF(src)];[HrefToken(TRUE)];refreshfaxpanel=1'>Refresh</A>"
	html += "<A align='right' href='?src=[REF(src)];[HrefToken(TRUE)];AdminFaxCreate=1;faxtype=Administrator'>Create Fax</A>"
	...
	for(var/datum/fax/admin/A in GLOB.adminfaxes)
		...
		if(A.sent_by)
			...
			html += "<td>[ADMIN_PP(S)]</td>"
		else
			html += "<td>Unknown</td>"
		html += "<td><A align='right' href='?src=[REF(src)];[HrefToken(TRUE)];AdminFaxView=\ref[A.message]'>View</A></td>"
		if(!A.reply_to)
			if(A.from_department == "Administrator")
				html += "<td>N/A</td>"
			else
				html += "<td><A align='right' href='?src=[REF(src)];[HrefToken(TRUE)];AdminFaxCreate=\ref[A.sent_by];originfax=\ref[A.origin];faxtype=[A.to_department];replyto=\ref[A.message]'>Reply</A></td>"
			html += "<td>N/A</td>"
		else
			html += "<td>N/A</td>"
			html += "<td><A align='right' href='?src=[REF(src)];[HrefToken(TRUE)];AdminFaxView=\ref[A.reply_to]'>Original</A></td>"
		html += "</tr>"
	html += "</table>"
	html += "</div>"
	...
	for(var/datum/fax/F in GLOB.faxes)
		...
		if(F.sent_by)
			...
			html += "<td>Unknown</td>"
		html += "<td><A align='right' href='?src=[REF(src)];[HrefToken(TRUE)];AdminFaxView=\ref[F.message]'>View</A></td>"
		html += "</tr>"
	html += "</table>"
	html += "</div>"
