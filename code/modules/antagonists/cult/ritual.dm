...
/obj/item/melee/cultblade/dagger/proc/scribe_rune(mob/living/user)
	...
	if(!src || QDELETED(src) || !Adjacent(user) || user.incapacitated() || !check_rune_turf(Turf, user))
		return
	if(ispath(rune_to_scribe, /obj/effect/rune/summon) && (!is_station_level(Turf.get_z_level()) || A.map_name == "Space"))
...
/obj/item/melee/cultblade/dagger/proc/check_rune_turf(turf/T, mob/user)
	...
	if((!is_station_level(T.get_z_level()) && !is_mining_level(T.get_z_level())) || (A && !A.blob_allowed))
