...
/obj/effect/rune/raise_dead/invoke(var/list/invokers)
	...
	if(!mob_to_revive.client || mob_to_revive.client.is_afk())
		set waitfor = FALSE
		var/list/mob/dead/observer/candidates = pollCandidatesForMob("Do you want to play as a [mob_to_revive.name], an inactive blood cultist?", ROLE_KEY_CULTIST, null, ROLE_KEY_CULTIST, 50, mob_to_revive)
...
/obj/effect/rune/manifest/invoke(var/list/invokers)
	...
	if(choice == "Summon a Cult Ghost")
		...
		for(var/mob/dead/observer/O in T)
			if(O.client && !is_banned_from(O.ckey, list(BANCHECK_ROLE_MAJOR_GHOSTSPAWN, ROLE_KEY_CULTIST)) && !QDELETED(src) && !QDELETED(O))
				ghosts_on_rune += O
