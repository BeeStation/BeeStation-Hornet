...
/obj/effect/rune/apocalypse/invoke(var/list/invokers)
	...
	if(intensity>=285) // Based on the prior formula, this means the cult makes up <15% of current players
		...
		switch(outcome)
			if(1 to 10)
				var/datum/round_event_control/disease_outbreak/disease_outbreak_event = new()
				var/datum/round_event_control/mice_migration/mice_migration_event = new()
				disease_outbreak_event.runEvent()
				mice_migration_event.runEvent()
			if(11 to 20)
				var/datum/round_event_control/radiation_storm/radiation_storm_event = new()
				radiation_storm_event.runEvent()
			if(21 to 30)
				var/datum/round_event_control/brand_intelligence/brand_intelligence_event = new()
				brand_intelligence_event.runEvent()
			if(31 to 40)
				var/datum/round_event_control/immovable_rod/immovable_rod_event = new()
				immovable_rod_event.runEvent()
				immovable_rod_event.runEvent()
				immovable_rod_event.runEvent()
			if(41 to 50)
				var/datum/round_event_control/meteor_wave/meteor_wave_event = new()
				meteor_wave_event.runEvent()
			if(51 to 60)
				var/datum/round_event_control/spider_infestation/spider_infestation_event = new()
				spider_infestation_event.runEvent()
			if(61 to 70)
				var/datum/round_event_control/anomaly/anomaly_flux/anomaly_flux_event
				var/datum/round_event_control/anomaly/anomaly_grav/anomaly_grav_event
				var/datum/round_event_control/anomaly/anomaly_pyro/anomaly_pyro_event
				var/datum/round_event_control/anomaly/anomaly_vortex/anomaly_vortex_event
				anomaly_flux_event.runEvent()
				anomaly_grav_event.runEvent()
				anomaly_pyro_event.runEvent()
				anomaly_vortex_event.runEvent()
			if(71 to 80)
				var/datum/round_event_control/spacevine/spacevine_event = new()
				var/datum/round_event_control/grey_tide/grey_tide_event = new()
				spacevine_event.runEvent()
				grey_tide_event.runEvent()
			if(81 to 100)
				var/datum/round_event_control/portal_storm_narsie/portal_storm_narsie_event = new()
				portal_storm_narsie_event.runEvent()
	qdel(src)
