...
/obj/machinery/abductor/pad/proc/Warp(mob/living/target)
	if(!target.buckled)
		do_teleport(target, get_turf(src), no_effects = TRUE, channel = TELEPORT_CHANNEL_FREE)
...
/obj/machinery/abductor/pad/proc/Send()
	...
	for(var/mob/living/target in loc)
		do_teleport(target, teleport_target, no_effects = TRUE, channel = TELEPORT_CHANNEL_FREE)
...
/obj/machinery/abductor/pad/proc/doMobToLoc(place, atom/movable/target)
	flick("alien-pad", src)
	do_teleport(target, place, no_effects = TRUE, channel = TELEPORT_CHANNEL_FREE)
	new /obj/effect/temp_visual/dir_setting/ninja(get_turf(target), target.dir)
