	...
	if(loc == H || (in_range(src, H) && isturf(loc)))
		...
		if(href_list["school"])
			if(used)
				...
				return
			var/list/candidates = pollGhostCandidates("Do you want to play as a wizard's [href_list["school"]] apprentice?", BAN_ROLE_WIZARD, null, /datum/role_preference/midround_ghost/wizard, 150)
...
/obj/item/antag_spawner/nuke_ops/attack_self(mob/user)
	...
	var/list/nuke_candidates = pollGhostCandidates("Do you want to play as a syndicate [borg_to_spawn ? "[lowertext(borg_to_spawn)] cyborg":"operative"]?", BAN_ROLE_OPERATIVE, null, /datum/role_preference/midround_ghost/nuclear_operative, 150)
...
/obj/item/antag_spawner/slaughter_demon/attack_self(mob/user)
	...
	if(used)
		return
	var/list/candidates = pollCandidatesForMob("Do you want to play as a [initial(demon_type.name)]?", BAN_ROLE_SLAUGHTER_DEMON, null, null, 50, src)
	if(LAZYLEN(candidates))
...
/obj/item/antag_spawner/gangster/attack_self(mob/user)
	...
	to_chat(user, "<span class='notice'>You activate [src] and wait for confirmation.</span>")
	var/list/candidates = pollGhostCandidates("Do you want to play as a gangster reinforcements?", BAN_ROLE_GANG, null, /datum/role_preference/antagonist/gangster, 150)
	if(LAZYLEN(candidates))
		if(QDELETED(src) || !check_usability(user))
			return
		used = TRUE
