...
/datum/antagonist/monkey/can_be_owned(datum/mind/new_owner)
	return ..() && (!monkey_only || ismonkey(new_owner.current)) && (iscarbon(new_owner.current) && !isplasmaman(new_owner.current) && !isipc(new_owner.current))//Check for races that cannot carry the infection
