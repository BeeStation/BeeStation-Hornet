...
/datum/antagonist/heretic/proc/forge_objectives()
	...
	for(var/mob/player in SSticker.mode.current_players[CURRENT_LIVING_PLAYERS])
		if(player.client && (player.mind.assigned_role in SSdepartment.get_jobs_by_dept_id(DEPARTMENT_COMMAND)))
			num_heads++
...
/datum/antagonist/heretic/proc/can_sacrifice(target)
	...
	if(major_sacc_objective && !major_sacc_objective.check_completion() && (target_mind.assigned_role in SSdepartment.get_jobs_by_dept_id(DEPARTMENT_COMMAND)))
		return TRUE
