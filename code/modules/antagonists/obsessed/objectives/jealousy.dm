...
/datum/objective/assassinate/jealous/find_target(list/dupe_search_range, list/blacklist)//returning null = free objective
	...
	if(obsession.assigned_role in SSdepartment.get_jobs_by_dept_id(DEPARTMENT_NAME_SECURITY))
		chosen_department = SSdepartment.get_jobs_by_dept_id(DEPARTMENT_NAME_SECURITY)
	else if(obsession.assigned_role in SSdepartment.get_jobs_by_dept_id(DEPARTMENT_NAME_ENGINEERING))
		chosen_department = SSdepartment.get_jobs_by_dept_id(DEPARTMENT_NAME_ENGINEERING)
	else if(obsession.assigned_role in SSdepartment.get_jobs_by_dept_id(DEPARTMENT_NAME_MEDICAL))
		chosen_department = SSdepartment.get_jobs_by_dept_id(DEPARTMENT_NAME_MEDICAL)
	else if(obsession.assigned_role in SSdepartment.get_jobs_by_dept_id(DEPARTMENT_NAME_SCIENCE))
		chosen_department = SSdepartment.get_jobs_by_dept_id(DEPARTMENT_NAME_SCIENCE)
	else if(obsession.assigned_role in SSdepartment.get_jobs_by_dept_id(DEPARTMENT_NAME_CARGO))
		chosen_department = SSdepartment.get_jobs_by_dept_id(DEPARTMENT_NAME_CARGO)
	else if(obsession.assigned_role in (SSdepartment.get_jobs_by_dept_id(DEPARTMENT_NAME_CIVILIAN)))
