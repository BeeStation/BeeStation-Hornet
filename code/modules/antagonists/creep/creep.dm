...
/datum/objective/assassinate/jealous/find_target(list/dupe_search_range, list/blacklist)//returning null = free objective
	...
	else if(obsession.assigned_role in SSdepartment.get_jobs_by_dept_id(DEPT_NAME_SUPPLY))
		chosen_department = SSdepartment.get_jobs_by_dept_id(DEPT_NAME_SUPPLY)
	else if(obsession.assigned_role in SSdepartment.get_jobs_by_dept_id(DEPT_NAME_SERVICE))
		chosen_department = SSdepartment.get_jobs_by_dept_id(DEPT_NAME_SERVICE)
	else
		set_target(null)
		update_explanation_text()
		return
