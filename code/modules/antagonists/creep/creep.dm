...
/datum/objective/assassinate/jealous/find_target(list/dupe_search_range, list/blacklist)//returning null = free objective
	...
	if(obsession.assigned_role in SSdepartment.get_joblist_by_dept_id(DEPT_NAME_SECURITY))
		chosen_department = SSdepartment.get_joblist_by_dept_id(DEPT_NAME_SECURITY)
	else if(obsession.assigned_role in SSdepartment.get_joblist_by_dept_id(DEPT_NAME_ENGINEERING))
		chosen_department = SSdepartment.get_joblist_by_dept_id(DEPT_NAME_ENGINEERING)
	else if(obsession.assigned_role in SSdepartment.get_joblist_by_dept_id(DEPT_NAME_MEDICAL))
		chosen_department = SSdepartment.get_joblist_by_dept_id(DEPT_NAME_MEDICAL)
	else if(obsession.assigned_role in SSdepartment.get_joblist_by_dept_id(DEPT_NAME_SCIENCE))
		chosen_department = SSdepartment.get_joblist_by_dept_id(DEPT_NAME_SCIENCE)
	else if(obsession.assigned_role in SSdepartment.get_joblist_by_dept_id(DEPT_NAME_SUPPLY))
		chosen_department = SSdepartment.get_joblist_by_dept_id(DEPT_NAME_SUPPLY)
	else if(obsession.assigned_role in SSdepartment.get_joblist_by_dept_id(DEPT_NAME_SERVICE))
		chosen_department = SSdepartment.get_joblist_by_dept_id(DEPT_NAME_SERVICE)
	else
		set_target(null)
		update_explanation_text()
		return
