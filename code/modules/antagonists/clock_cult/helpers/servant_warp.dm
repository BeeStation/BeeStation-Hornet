/proc/try_warp_servant(mob/living/servant, turf/target_location, bring_dragging = FALSE)
	...
	do_teleport(M, target_location, channel = TELEPORT_CHANNEL_CULT, no_effects = TRUE, teleport_mode = TELEPORT_MODE_CLOCKWORK)
	...
	if(istype(P) && bring_dragging)
		do_teleport(P, target_location, channel = TELEPORT_CHANNEL_CULT, no_effects = TRUE, teleport_mode = TELEPORT_MODE_CLOCKWORK)
		P.Paralyze(30)
