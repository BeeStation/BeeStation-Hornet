...
/datum/clockcult/scripture/slab/proc/sentinels_compromise(mob/living/caller, atom/clicked_on)
	...
	if(isliving(clicked_on) && (clicked_on in view(7, get_turf(caller))))
		...
		if(!is_servant_of_ratvar(L))
			to_chat(caller, ("<span class='inathneq'>\"[L] does not yet serve Ratvar.\"</span>"))
			return TRUE
		if(L.stat == DEAD)
			to_chat(caller, ("<span class='inathneq'>\"[L.p_theyre(TRUE)] dead. [text2ratvar("Oh, child. To have your life cut short...")]\"</span>"))
			return TRUE
		...
		if(!totaldamage && (!L.reagents || !L.reagents.has_reagent(/datum/reagent/water/holywater)))
			to_chat(caller, ("<span class='inathneq'>\"[L] is unhurt and untainted.\"</span>"))
			return TRUE
		to_chat(caller, ("<span class='brass'>You bathe [L == caller ? "yourself":"[L]"] in Inath-neq's power!</span>"))
