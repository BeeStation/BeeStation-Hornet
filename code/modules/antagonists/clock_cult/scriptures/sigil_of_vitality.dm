		...
		if(M.stat == DEAD)
			...
			if(GLOB.clockcult_vitality >= damage_healed)
				...
				if(M.mind)
					...
					var/list/mob/dead/observer/candidates = pollCandidatesForMob("Do you want to play as a [M.name], an inactive clock cultist?", BAN_ROLE_SERVANT_OF_RATVAR, /datum/role_preference/antagonist/clock_cultist, 7.5 SECONDS, M)
