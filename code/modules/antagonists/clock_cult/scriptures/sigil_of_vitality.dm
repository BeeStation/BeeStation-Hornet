		...
		if(M.stat == DEAD)
			...
			if(GLOB.clockcult_vitality >= damage_healed)
				...
				if(M.mind)
					...
					var/list/mob/dead/observer/candidates = pollCandidatesForMob("Do you want to play as a [M.name], an inactive clock cultist?", ROLE_KEY_SERVANT_OF_RATVAR, null, ROLE_KEY_SERVANT_OF_RATVAR, 50, M)
