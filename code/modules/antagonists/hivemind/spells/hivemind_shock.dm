...
/obj/effect/proc_holder/spell/target_hive/hive_shock/cast(list/targets, mob/living/user = usr)
	...
	if(do_after(user, 30, user, timed_action_flags = IGNORE_HELD_ITEM))
		if(target.mind.assigned_role in (SSdepartment.get_jobs_by_dept_id(DEPARTMENT_SECURITY) || SSdepartment.get_jobs_by_dept_id(DEPARTMENT_COMMAND))) //Doesn't work on sec or command for balance reasons
