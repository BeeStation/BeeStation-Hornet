...
/obj/effect/proc_holder/spell/target_hive/hive_shock/cast(list/targets, mob/living/user = usr)
	...
	if(do_after(user,30,0,user))
		if(target.mind.get_mind_role(JTYPE_JOB_PATH, as_basic_job=TRUE) in (GLOB.security_positions || GLOB.command_positions)) //Doesn't work on sec or command for balance reasons
