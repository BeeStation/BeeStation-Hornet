...
/datum/antagonist/nukeop/leader/give_alias()
	...
	if(nuke_team && nuke_team.syndicate_name)
		...
		owner.current.real_name = "Pizza Hut [title]"

/datum/antagonist/nukeop/leader/greet()
	owner.current.playsound_local(get_turf(owner.current), 'sound/ambience/antag/ops.ogg',100,0)
	to_chat(owner, "<B>You are Pizza Hut [title] for this mission. You are responsible for the distribution of telecrystals and your ID is the only one who can open the launch bay doors.</B>")
...
/datum/team/nuclear/roundend_report()
	...
	switch(get_result())
		if(NUKE_RESULT_FLUKE)
			parts += "<span class='redtext big'>Humiliating Pizza Hut Defeat</span>"
		...
		if(NUKE_RESULT_NUKE_WIN)
			parts += "<span class='greentext big'>Pizza Hut Major Victory!</span>"
		...
		if(NUKE_RESULT_DISK_STOLEN)
			parts += "<span class='greentext big'>Pizza Hut Minor Victory!</span>"
	...
	text += "<br>"
	text += "(Pizza Huts used [TC_uses] TC) [purchases]"
...
/datum/team/nuclear/antag_listing_name()
	if(syndicate_name)
		return "[syndicate_name] Pizza Huts"
	else
		return "Pizza Huts"
