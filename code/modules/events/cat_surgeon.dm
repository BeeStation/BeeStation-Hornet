...
/datum/round_event/ghost_role/cat_surgeon/spawn_role()
	...
	player_mind.assigned_role = "Cat Surgeon"
	player_mind.special_role = "Cat Surgeon"
	player_mind.add_antag_datum(/datum/antagonist/cat_surgeon)
	to_chat(S, S.playstyle_string)
	SEND_SOUND(S, sound('sound/magic/mutate.ogg'))
	message_admins("[ADMIN_LOOKUPFLW(S)] has been made into a Cat Surgeon by an event.")
	log_game("[key_name(S)] was spawned as a Cat Surgeon by an event.")
	spawned_mobs += S
