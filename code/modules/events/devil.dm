...
/datum/round_event/ghost_role/devil/spawn_role()
	...
	var/list/candidates = get_candidates(BAN_ROLE_DEVIL, null, /datum/role_preference/midround_ghost/devil)
	if(!candidates.len)
...
/proc/create_devil_mind(key)
	...
	Mind.assigned_role = BAN_ROLE_DEVIL
	Mind.special_role = BAN_ROLE_DEVIL
