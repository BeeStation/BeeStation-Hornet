...
/datum/round_event/wizard/imposter/start()
	for(var/datum/mind/M in SSticker.mode.wizards)
		...
		var/list/candidates = pollGhostCandidates("Would you like to be an imposter wizard?", BAN_ROLE_WIZARD, /datum/role_preference/midround_ghost/wizard, ignore_category = POLL_IGNORE_WIZARD_HELPER)
		if(!candidates)
			return //Sad Trombone
