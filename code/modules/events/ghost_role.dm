...
/datum/round_event/ghost_role/proc/get_candidates(banning_key, gametypecheck, role_preference, poll_ignore = null)
	...
	if(priority_candidates.len < minimum_required)
		regular_candidates = pollGhostCandidates("Do you wish to be considered for the special role of '[role_name]'?", banning_key, gametypecheck, role_preference, ignore_category = poll_ignore)
	else
