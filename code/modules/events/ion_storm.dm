		...
		if(M.stat != DEAD && M.see_in_dark != 0)
			...
			if(message)
				if(prob(replace_law_prob))
					...
					log_law("[M.key]/([M.name]) had an ion law added, as follows:\"[message]\". Source: [law_source].")
					var/time = time2text(world.realtime,"hh:mm:ss")
					GLOB.lawchanges.Add("[time] <B>:</B> [M.key]/([M.name]) had an ion law added, as follows:\"[message]\". Source: [law_source].")
