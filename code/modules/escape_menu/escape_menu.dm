...
/// Verb, hardcoded to Escape via the "default" macro set in [/datum/controller/subsystem/input].
/client/verb/open_escape_menu()
	set name = "Open Escape Menu"
	set hidden = TRUE

	// An open say modal gets first claim on the key, the menu opens on the next press.
	if (close_open_tgui_say())
		return
