...
/datum/hallucination/death/New(mob/living/carbon/C, forced = TRUE)
	...
	if(prob(50))
		...
		if(fakemob)
			delay = rand(20, 50)
			addtimer(CALLBACK(GLOBAL_PROC, PROC_REF(to_chat), target, span_deadsay("<b>DEAD: [fakemob.name]</b> says, \"[pick("rip","why did i just drop dead?","hey [target.first_name()]","git gud","you too?","is the AI rogue?",\
