...
/mob/living/mech_melee_attack(obj/vehicle/sealed/mecha/mecha_attacker, mob/living/user)
	if(user.combat_mode)
		...
		switch(mecha_attacker.damtype)
			if(BRUTE)
				Unconscious(20)
				take_direct_overall_damage(BRUTE, rand(mecha_attacker.force/2, mecha_attacker.force))
				playsound(src, 'sound/weapons/punch4.ogg', 50, TRUE)
			if(BURN)
				take_direct_overall_damage(BURN, rand(mecha_attacker.force * 0.5, mecha_attacker.force))
				playsound(src, 'sound/items/welder.ogg', 50, TRUE)
			if(TOX)
