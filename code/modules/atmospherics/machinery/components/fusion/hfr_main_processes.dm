...
/obj/machinery/atmospherics/components/unary/hypertorus/core/process_atmos(delta_time)
	...
	if (start_power || power_level)
		...
		process_moderator_overflow(delta_time)
...
/obj/machinery/atmospherics/components/unary/hypertorus/core/proc/fusion_process(delta_time)
	...
	if (selected_fuel)
		...
		for(var/gas_type in selected_fuel.requirements | selected_fuel.primary_products)
			var/amount = internal_fusion.moles[gas_type]
			fuel_list[gas_type] = amount
			scaled_fuel_list[gas_type] = max((amount - FUSION_MOLE_THRESHOLD) / scale_factor, 0)

	/// Store the moderators gases quantities
	var/list/moderator_list = list()
	/// Scaled down moles of gases, no less than 0
	var/list/scaled_moderator_list = list()
	for(var/gas_type, amount in moderator_internal.moles)
		moderator_list[gas_type] = amount
		scaled_moderator_list[gas_type] = max((amount - FUSION_MOLE_THRESHOLD) / scale_factor, 0)
	...
	var/gas_power = internal_fusion.fusion_power() + moderator_internal.fusion_power() * 0.75
	...
	switch(power_level)
		if(3,4)
			production_amount = clamp(heat_output / 1000, 0, fuel_consumption_rate) * delta_time
		else
			production_amount = clamp(heat_output * 2 / 10 ** (power_level+1), 0, fuel_consumption_rate) * delta_time
...
/obj/machinery/atmospherics/components/unary/hypertorus/core/proc/moderator_common_process(delta_time, scaled_production, datum/gas_mixture/internal_output, moderator_list, dirty_production_rate, heat_output, radiation_modifier)
	switch(power_level)
		...
		if(5)
			...
			if(moderator_internal.temperature < 1e7 || (moderator_list[/datum/gas/plasma] > 100 && moderator_list[/datum/gas/bz] > 50))
				internal_output.adjust_gas(/datum/gas/antinoblium, dirty_production_rate * 0.9 / 0.065 * delta_time)
		if(6)
			...
			if(moderator_list[/datum/gas/bz])
				visible_hallucination_pulse(src, HALLUCINATION_HFR(heat_output), 100 SECONDS * power_level * delta_time)
				internal_output.adjust_gas(/datum/gas/antinoblium, clamp(dirty_production_rate / 0.045, 0, 10) * delta_time)
			if(moderator_list[/datum/gas/healium] > 100)
				if(critical_threshold_proximity > 400)
					critical_threshold_proximity = max(critical_threshold_proximity - (moderator_list[/datum/gas/healium] / 100 * delta_time ), 0)
					moderator_internal.adjust_gas(/datum/gas/healium, -min(moderator_internal.moles[/datum/gas/healium], scaled_production * 20))
			internal_fusion.adjust_gas(/datum/gas/antinoblium, dirty_production_rate * 0.01 / 0.095 * delta_time)
	...
	if(internal_fusion.temperature <= FUSION_MAXIMUM_TEMPERATURE * temperature_modifier)
		internal_fusion.temperature = clamp(
			internal_fusion.temperature + heat_output * delta_time,
			TCMB,
...
/obj/machinery/atmospherics/components/unary/hypertorus/core/proc/process_damageheal(delta_time)
	...
	if(internal_fusion.total_moles() >= HYPERTORUS_HYPERCRITICAL_MOLES)
		...
		hypercritical_damage_taken = min(hypercritical_damage_taken, HYPERTORUS_HYPERCRITICAL_MAX_DAMAGE) * delta_time
	...
	if (selected_fuel)
		...
		for(var/gas_id in selected_fuel.primary_products)
			if(internal_fusion.moles[gas_id] > 0)
				internal_remove = internal_fusion.remove_specific(gas_id, internal_fusion.moles[gas_id] * (1 - (1 - 0.25) ** delta_time))
