...
/datum/gas_mixture
	...
	/// Used for analyzer feedback - not initialized until it's used (USED FOR FUSION ONLY AT THE MOMENT)
...
///assert_gas(gas_id) - used to guarantee that the gas list for this id exists in gas_mixture.moles.
...
/// Fusion power
/datum/gas_mixture/proc/fusion_power()
	return values_dot(moles, GAS_META[META_GAS_FUSION_POWER])

/// Gas rig shielding power
/datum/gas_mixture/proc/gas_rig_shielding_power()
	return values_dot(moles, GAS_META[META_GAS_RIG_SHIELDING_POWER])

/// Gas rig shielding modifier
/datum/gas_mixture/proc/gas_rig_shielding_modifier()
	return values_dot(moles, GAS_META[META_GAS_RIG_SHIELDING_MODIFIER])
