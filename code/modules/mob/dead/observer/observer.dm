...
/mob/proc/ghostize(can_reenter_corpse = TRUE, retain_sentience = FALSE )
	if(key)
...
/mob/living/verb/ghost()
	...
	if(stat == DEAD)
		...
		ghostize(FALSE,TRUE)						//0 parameter is so we can never re-enter our body, "Charlie, you can never come baaaack~" :3
