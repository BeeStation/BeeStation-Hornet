...
/obj/item/food/deadmouse/examine(mob/user)
	...
	if (reagents?.has_reagent(/datum/reagent/fuel))
		. += span_warning("[p_Theyre()] dripping with fuel and smell[p_s()] terrible.")
...
/obj/item/food/deadmouse/proc/use_lazarus(datum/source, obj/item/lazarus_injector/injector, mob/user)
	...
	INVOKE_ASYNC(revived_critter, TYPE_PROC_REF(/mob/living, lazarus_revive), user, injector.malfunctioning)
...
/obj/item/food/deadmouse/attack_atom(atom/attacked_atom, mob/living/user, params)
	...
	if(.)
		return
	if(isnull(reagents) || !attacked_atom.is_open_container())
		return FALSE

	// is_open_container will not return truthy if target.reagents doesn't exist
	var/datum/reagents/target_reagents = attacked_atom.reagents
	...
	if(target_reagents.has_reagent(/datum/reagent/fuel) && target_reagents.trans_to(src, trans_amount))
		to_chat(user, span_notice("You dip [src] into [attacked_atom]."))
		return TRUE
