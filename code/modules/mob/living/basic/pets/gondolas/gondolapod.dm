...
/mob/living/basic/pet/gondola/gondolapod/Initialize(mapload, pod)
	...
	if(!linked_pod.stay_after_drop || !linked_pod.opened)
		GRANT_ACTION(/datum/action/innate/deliver_gondola_package)
		GRANT_ACTION(/datum/action/innate/check_gondola_contents)
	return ..()
...
/datum/action/innate/deliver_gondola_package
	...
	background_icon = 'icons/hud/screen_gen.dmi'
	button_icon = 'icons/hud/screen_gen.dmi'
	button_icon_state = "arrow"
	check_flags = AB_CHECK_PHASED

/datum/action/innate/deliver_gondola_package/on_activate(trigger_flags)
	. = ..()
	if(!.)
		return
...
/datum/action/innate/check_gondola_contents
	...
	background_icon = 'icons/obj/implants.dmi'
	button_icon = 'icons/obj/implants.dmi'
	button_icon_state = "storage"
	check_flags = AB_CHECK_PHASED

/datum/action/innate/check_gondola_contents/on_activate(trigger_flags)
	. = ..()
	if(!.)
		return
