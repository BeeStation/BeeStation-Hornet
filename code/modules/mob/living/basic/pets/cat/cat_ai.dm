...
/datum/ai_planning_subtree/reside_in_home/SelectBehaviors(datum/ai_controller/controller, delta_time)
	...
	if(istype(living_pawn.loc, /obj/structure/cat_house))
		if(DT_PROB(leave_home_chance, delta_time))
			controller.set_blackboard_key(BB_CAT_HOME, living_pawn.loc)
		return SUBTREE_RETURN_FINISH_PLANNING

	if(DT_PROB(reside_chance, delta_time))
...
/datum/ai_behavior/enter_cat_home/perform(delta_time, datum/ai_controller/controller, target_key)
...
/datum/ai_planning_subtree/territorial_struggle/SelectBehaviors(datum/ai_controller/controller, delta_time)
	var/mob/living/living_pawn = controller.pawn
	if(living_pawn.gender != MALE || !DT_PROB(hostility_chance, delta_time))
		return
...
/datum/ai_behavior/territorial_struggle/perform(delta_time, datum/ai_controller/controller, target_key, cries_key)
	. = ..()
	var/mob/living/target = controller.blackboard[target_key]
...
/datum/ai_planning_subtree/find_and_hunt_target/hunt_mice/SelectBehaviors(datum/ai_controller/controller, delta_time)
	var/mob/living/living_pawn = controller.pawn
...
/datum/ai_behavior/play_with_mouse/perform(delta_time, datum/ai_controller/controller, target_key)
...
/datum/ai_planning_subtree/haul_food_to_young/SelectBehaviors(datum/ai_controller/controller, delta_time)
...
/datum/ai_behavior/deliver_food_to_kitten/perform(delta_time, datum/ai_controller/controller, target_key, food_key)
	var/mob/living/target = controller.blackboard[target_key]
