...
/mob/living/simple_animal/hostile/cat_butcherer/AttackingTarget()
	if(ishuman(target))
		...
		if(!L.get_organ_by_type(/obj/item/organ/internal/ears/cat) && L.stat) //target doesnt have cat ears
			visible_message("[src] slices off [L]'s ears, and replaces them with cat ears!", span_notice("You replace [L]'s ears with cat ears'."))
			var/obj/item/organ/internal/ears/cat/newears = new
			newears.Insert(L)
		else if(!L.get_organ_by_type(/obj/item/organ/external/tail/cat) && L.stat)
			visible_message("[src] attaches a cat tail to [L]!", span_notice("You attach a tail to [L]."))
			var/obj/item/organ/external/tail/cat/newtail = new
			newtail.Insert(L)
			return
		else if(!L.has_trauma_type(/datum/brain_trauma/severe/pacifism) && L.get_organ_by_type(/obj/item/organ/internal/ears/cat) && L.get_organ_by_type(/obj/item/organ/external/tail/cat) && L.stat) //still does damage.
...
/mob/living/simple_animal/hostile/cat_butcherer/CanAttack(atom/the_target)
	if(iscarbon(target))
		...
		if(C.get_organ_by_type(/obj/item/organ/internal/ears/cat) && C.get_organ_by_type(/obj/item/organ/external/tail/cat) && C.has_trauma_type(/datum/brain_trauma/severe/pacifism))//he wont attack his creations
