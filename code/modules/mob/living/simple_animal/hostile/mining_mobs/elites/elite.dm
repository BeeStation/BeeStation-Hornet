	...
	if(ishuman(user))
		switch(activity)
			...
			if(TUMOR_INACTIVE)
				...
				var/list/candidates = pollCandidatesForMob("Do you want to play as a lavaland elite?", ROLE_KEY_SENTIENT, null, ROLE_KEY_SENTIENT, 50, src, POLL_IGNORE_LAVALAND_ELITE)
				if(candidates.len)
