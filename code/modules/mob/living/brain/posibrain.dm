...
/obj/item/mmi/posibrain/proc/activate(mob/user)
	if(QDELETED(brainmob))
		return
	if(is_banned_from(user.ckey, BANCHECK_ROLE_MAJOR_GHOSTSPAWN, ROLE_KEY_POSIBRAIN))
...
/obj/item/mmi/posibrain/transfer_identity(mob/living/carbon/C)
	...
	brainmob.mind.set_job(JOB_KEY_CYBORG) // they're human. Don't use `assign_station_role()` or it will wipe their old displaying job title
	if(C.mind)
...
/obj/item/mmi/posibrain/proc/transfer_personality(mob/candidate)
	...
	brainmob.mind.assign_special_role(ROLE_KEY_POSIBRAIN)
	brainmob.set_stat(CONSCIOUS)
