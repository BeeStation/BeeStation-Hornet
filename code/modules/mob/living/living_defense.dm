...
/mob/living/attack_slime(mob/living/simple_animal/slime/M)
	...
	if(stat != DEAD)
		...
		visible_message(span_danger("\The [M.name] glomps [src]!"), \
				span_userdanger("\The [M.name] glomps you!"), null, COMBAT_MESSAGE_RANGE)
		return TRUE

/mob/living/attack_animal(mob/living/simple_animal/M)
	M.face_atom(src)
	if(M.melee_damage == 0)
		visible_message(span_notice("\The [M] [M.friendly] [src]!"), \
						span_notice("\The [M] [M.friendly] you!"), null, COMBAT_MESSAGE_RANGE)
		return FALSE
	if(HAS_TRAIT(M, TRAIT_PACIFISM))
		to_chat(M, span_notice("You don't want to hurt anyone!"))
		return FALSE
	...
	visible_message(span_danger("\The [M] [M.attacktext] [src]!"), \
					span_userdanger("\The [M] [M.attacktext] you!"), null, COMBAT_MESSAGE_RANGE)
	log_combat(M, src, "attacked")
	return TRUE
...
/mob/living/attack_paw(mob/living/carbon/monkey/M)
	...
	if (M.a_intent == INTENT_HARM)
		...
		visible_message(span_danger("[M.name] bites [src]!"), \
						span_userdanger("[M.name] bites you!"), null, COMBAT_MESSAGE_RANGE)
		return TRUE
	return FALSE
		...
		if("help")
			...
			if(prob(90))
				...
				visible_message(span_danger("[L.name] bites [src]!"), \
								span_userdanger("[L.name] bites you!"), null, COMBAT_MESSAGE_RANGE)
				playsound(loc, 'sound/weapons/bite.ogg', 50, 1, -1)
				return TRUE
			else
				visible_message(span_danger("[L.name]'s bite misses [src]!"), \
								span_userdanger("[L.name]'s bite misses you!"), null, COMBAT_MESSAGE_RANGE)
	return FALSE
	...
	switch(M.a_intent)
		if ("help")
			visible_message(span_notice("[M] caresses [src] with its scythe-like arm."), \
				span_notice("[M] caresses you with its scythe-like arm."))
			return FALSE
		if ("grab")
			grabbedby(M)
			return FALSE
