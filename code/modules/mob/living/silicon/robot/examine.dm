	...
	switch(stat)
		if(CONSCIOUS)
			if(shell)
				...
				. += "It appears to be in stand-by mode. A display flashes that it has been deactivated for [round(((world.time - lastclienttime) / (1 MINUTES)),1)] minutes. It may boot up soon." //afk
		if(UNCONSCIOUS)
