...
/mob/living/silicon/proc/deadchat_lawchange()
	...
	deadchat_broadcast(span_deadsay("[span_name("[src]")]'s laws were changed.") + " <a href='?src=[REF(src)]&printlawtext=[rustg_url_encode(lawtext)]'>View</a>", span_name("[src]"), follow_target=src, message_type=DEADCHAT_LAWCHANGE)
