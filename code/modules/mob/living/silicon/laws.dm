...
/mob/living/silicon/proc/deadchat_lawchange()
	...
	deadchat_broadcast("[span_deadsay("[span_name(src.name)]'s laws were changed.")] <a href='byond://?src=[REF(src)]&printlawtext=[rustg_url_encode(lawtext)]'>View</a>", span_name(src.name), follow_target=src, message_type=DEADCHAT_LAWCHANGE)
