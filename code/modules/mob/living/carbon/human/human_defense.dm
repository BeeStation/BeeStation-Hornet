...
/mob/living/carbon/human/attacked_by(obj/item/I, mob/living/user)
	...
	if(affecting)
		if(I.force && I.damtype != STAMINA && (!affecting.is_organic_limb())) // Bodpart_robotic sparks when hit, but only when it does real damage
			if(I.force >= 5)
...
/mob/living/carbon/human/check_self_for_injuries()
	...
	for(var/X in bodyparts)
		...
		if(status == "OK" || status == "no damage")
			no_damage = TRUE
		to_chat(src, "\t <span class='[no_damage ? "notice" : "warning"]'>Your [parse_zone(LB)] [HAS_TRAIT(src, TRAIT_SELF_AWARE) ? "has" : "is"] [status].</span>")
