...
//this is very slightly better than it was because you can use it more places. still can't do \his[src] though.
	...
	if(w_uniform && !(obscured & ITEM_SLOT_ICLOTHING) && !(w_uniform.item_flags & EXAMINE_SKIP))
		...
		. += span_info("[t_He] [t_is] wearing [w_uniform.get_examine_string(user)][accessory_msg].")
	//head
	if(head && !(head.item_flags & EXAMINE_SKIP))
		. += span_info("[t_He] [t_is] wearing [head.get_examine_string(user)] on [t_his] head.")
	//suit/armor
	if(wear_suit && !(wear_suit.item_flags & EXAMINE_SKIP))
		. += span_info("[t_He] [t_is] wearing [wear_suit.get_examine_string(user)].")
		//suit/armor storage
		if(s_store && !(obscured & ITEM_SLOT_SUITSTORE) && !(s_store.item_flags & EXAMINE_SKIP))
			. += span_info("[t_He] [t_is] carrying [s_store.get_examine_string(user)] on [t_his] [wear_suit.name].")
	//back
	if(back && !(back.item_flags & EXAMINE_SKIP))
		. += span_info("[t_He] [t_has] [back.get_examine_string(user)] on [t_his] back.")

	//Hands
	for(var/obj/item/I in held_items)
		if(!(I.item_flags & ABSTRACT) && !(I.item_flags & EXAMINE_SKIP))
			. += span_info("[t_He] [t_is] holding [I.get_examine_string(user)] in [t_his] [get_held_index_name(get_held_index_of_item(I))].")

	var/datum/component/forensics/FR = GetComponent(/datum/component/forensics)
	//gloves
	if(gloves && !(obscured & ITEM_SLOT_GLOVES) && !(gloves.item_flags & EXAMINE_SKIP))
		. += span_info("[t_He] [t_has] [gloves.get_examine_string(user)] on [t_his] hands.")
	...
	if(belt && !(belt.item_flags & EXAMINE_SKIP))
		. += span_info("[t_He] [t_has] [belt.get_examine_string(user)] about [t_his] waist.")

	//shoes
	if(shoes && !(obscured & ITEM_SLOT_FEET) && !(shoes.item_flags & EXAMINE_SKIP))
		. += span_info("[t_He] [t_is] wearing [shoes.get_examine_string(user)] on [t_his] feet.")

	//mask
	if(wear_mask && !(obscured & ITEM_SLOT_MASK) && !(wear_mask.item_flags & EXAMINE_SKIP))
		. += span_info("[t_He] [t_has] [wear_mask.get_examine_string(user)] on [t_his] face.")

	if(wear_neck && !(obscured & ITEM_SLOT_NECK) && !(wear_neck.item_flags & EXAMINE_SKIP))
		. += span_info("[t_He] [t_is] wearing [wear_neck.get_examine_string(user)] around [t_his] neck.")

	//eyes
	if(!(obscured & ITEM_SLOT_EYES))
		if(glasses && !(glasses.item_flags & EXAMINE_SKIP))
			. += span_info("[t_He] [t_has] [glasses.get_examine_string(user)] covering [t_his] eyes.")
		else if(eye_color == BLOODCULT_EYE && iscultist(src) && HAS_TRAIT(src, CULT_EYES))
			. += span_warning("<B>[t_His] eyes are glowing an unnatural red!</B>")

	//ears
	if(ears && !(obscured & ITEM_SLOT_EARS) && !(ears.item_flags & EXAMINE_SKIP))
		. += span_info("[t_He] [t_has] [ears.get_examine_string(user)] on [t_his] ears.")

	//ID
	if(wear_id && !(wear_id.item_flags & EXAMINE_SKIP))
		. += span_info("[t_He] [t_is] wearing [wear_id.get_examine_string(user)].")
