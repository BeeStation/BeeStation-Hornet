...
/proc/generate_acceptable_species()
	...
	for(var/species_id in no_hard_check) // warn any invalid species in the config.
		STACK_TRACE_ADV("WARNING: roundstart_no_hard_check contains invalid species ID: [species_id]")
	return base
...
/datum/species/proc/get_species_description()
	...
	STACK_TRACE_ADV("Species [name] ([type]) did not have a description set, and is a selectable roundstart race! Override get_species_description.")
...
/datum/species/proc/get_species_lore()
	...
	STACK_TRACE_ADV("Species [name] ([type]) did not have lore set, and is a selectable roundstart race! Override get_species_lore.")
...
/datum/species/proc/get_species_perks()
	...
	for(var/list/perk as anything in species_perks)
		...
		if(isnull(perks_to_return[perk_type]))
			STACK_TRACE_ADV("Invalid species perk ([perk[SPECIES_PERK_NAME]]) found for species [name]. \
