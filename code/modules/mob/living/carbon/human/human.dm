...
if(ishuman(over))
		...
		if(!src.is_busy && src.zone_selected == BODY_ZONE_PRECISE_MOUTH && get_turf(src) == get_turf(T) && !(T.mobility_flags & MOBILITY_STAND) && src.a_intent != INTENT_HELP) //all the stars align, time to tenderize nards
			...
			if (!do_mob(src,T,12) || src.zone_selected != BODY_ZONE_PRECISE_MOUTH || get_turf(src) != get_turf(T) || (T.mobility_flags & MOBILITY_STAND) || src.a_intent == INTENT_HELP) //wait 30ds and make sure the stars still align
				src.is_busy = FALSE
				return
			...
			var/obj/item/bodypart/BP = T.get_bodypart(BODY_ZONE_HEAD)
			if(BP)
				BP.receive_damage(12) //so 1 toolbox hits

			T.visible_message("<span class='warning'>[src] Hammerfists [T] in mouth!</span>", "<span class='warning'>[src]  Hammerfists you in the mouth!</span>")
