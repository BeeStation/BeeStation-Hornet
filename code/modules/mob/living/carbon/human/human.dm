	...
	if(href_list["hud"])
		...
		if(href_list["hud"] == "s")
			...
			if(href_list["add_citation"])
				...
				var/crime = GLOB.data_core.createCrimeEntry(t1, "", allowed_access, station_time_timestamp(), fine, author_ckey = H.ckey)
			...
			if(href_list["add_crime"])
				...
				if(!HAS_TRAIT(H, TRAIT_SECURITY_HUD))
					return
				var/crime = GLOB.data_core.createCrimeEntry(t1, null, allowed_access, station_time_timestamp(), author_ckey = H.ckey)
