...
/mob/living/carbon/monkey/update_hair()
	...
	if(wear_mask)
		...
		if(M.flags_inv & HIDEHAIR)
			hair_hidden = 1
	if(!hair_hidden)
		if(!get_organ_by_type(/obj/item/organ/internal/brain)) //Applies the debrained overlay if there is no brain
...
/mob/living/carbon/monkey/update_worn_undersuit()
	update_clothing_icons(UNIFORM_LAYER)

/mob/living/carbon/monkey/update_worn_head()
	update_clothing_icons(HEAD_LAYER)

/mob/living/carbon/monkey/update_worn_back()
	update_clothing_icons(BACK_LAYER)

/mob/living/carbon/monkey/update_worn_mask()
