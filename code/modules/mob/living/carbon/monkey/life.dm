	...
	if(bodytemperature > BODYTEMP_HEAT_DAMAGE_LIMIT && !HAS_TRAIT(src, TRAIT_RESISTHEAT))
		...
		switch(bodytemperature)
			if(360 to 400)
				...
				apply_damage_old(HEAT_DAMAGE_LEVEL_1, BURN)
			if(400 to 460)
				throw_alert("temp", /atom/movable/screen/alert/hot, 2)
				apply_damage_old(HEAT_DAMAGE_LEVEL_2, BURN)
			if(460 to INFINITY)
				throw_alert("temp", /atom/movable/screen/alert/hot, 3)
				if(on_fire)
					apply_damage_old(HEAT_DAMAGE_LEVEL_3, BURN)
				else
					apply_damage_old(HEAT_DAMAGE_LEVEL_2, BURN)
		...
		if(!istype(loc, /obj/machinery/atmospherics/components/unary/cryo_cell))
			...
			switch(bodytemperature)
				if(200 to BODYTEMP_COLD_DAMAGE_LIMIT)
					...
					apply_damage_old(COLD_DAMAGE_LEVEL_1, BURN)
				if(120 to 200)
					throw_alert("temp", /atom/movable/screen/alert/cold, 2)
					apply_damage_old(COLD_DAMAGE_LEVEL_2, BURN)
				if(-INFINITY to 120)
					throw_alert("temp", /atom/movable/screen/alert/cold, 3)
					apply_damage_old(COLD_DAMAGE_LEVEL_3, BURN)
		else
			clear_alert("temp")
