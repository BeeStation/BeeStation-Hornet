...
/datum/action/cooldown/alien/is_available(feedback = FALSE)
	. = ..()
	if(!.)
		return FALSE
	if(!iscarbon(owner))
...
/datum/action/cooldown/alien/pre_activate(atom/target)
...
/datum/action/cooldown/alien/make_structure/is_available(feedback = FALSE)
	. = ..()
	if(!.)
		return FALSE
...
/datum/action/cooldown/alien/make_structure/pre_activate(atom/target)
...
/datum/action/cooldown/alien/transfer/Activate(atom/target)
	...
	if(QDELETED(donation_target) || QDELETED(src) || QDELETED(owner) || !is_available(feedback = TRUE) || isnull(amount) || amount <= 0)
		return FALSE
...
/datum/action/cooldown/alien/acid/corrosion/pre_activate(atom/target)
...
/datum/action/cooldown/alien/acid/neurotoxin/is_available(feedback = FALSE)
	if(owner.is_muzzled())
		return FALSE
	if(!isturf(owner.loc))
		return FALSE
