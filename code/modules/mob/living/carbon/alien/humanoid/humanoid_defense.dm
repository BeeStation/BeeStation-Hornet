...
/mob/living/carbon/alien/humanoid/attack_hulk(mob/living/carbon/human/user, does_attack_animation = 0)
	if(user.a_intent == INTENT_HARM)
		...
		span_userdanger("[user] [hitverb] you!"), null, COMBAT_MESSAGE_RANGE)
		return 1
				...
				if (prob(90))
					...
					if ((stat != DEAD) && (damage > 9 || prob(5)))//Regular humans have a very small chance of knocking an alien down.
						...
						visible_message(span_danger("[M] knocks [src] down!"), \
								span_userdanger("[M] knocks you down!"))
					...
					visible_message(span_danger("[M]'s punch misses [src]!"), \
						span_userdanger("[M]'s punch misses you!"), null, COMBAT_MESSAGE_RANGE)
			...
			if ("disarm")
				if (!(mobility_flags & MOBILITY_STAND))
					if (prob(5))
						...
						visible_message(span_danger("[M] pushed [src] down!"), \
							span_userdanger("[M] pushed you down!"))
					else
						if (prob(50))
							...
							visible_message(span_danger("[M] fails to disarm [src]!"),\
								span_userdanger("[M] fails to disarm you!"), null, COMBAT_MESSAGE_RANGE)
