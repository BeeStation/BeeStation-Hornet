...
/mob/living/carbon/proc/crit_walk()
	if(stat == SOFT_CRIT)
		visible_message("<span class='danger'>crit walk proc ran!</span>")
		switch(health)
			if(HEALTH_THRESHOLD_FULLCRIT to -30)
				if(prob(60))
					visible_message("<span class='danger'>unconcious proc ran!</span>")
					Unconscious(rand(30, 60), ignore_canstun = TRUE)
					adjustOxyLoss(3)
				if(prob(55))
					emote("gasp")
			if(-29 to -20)
				if(prob(50))
					visible_message("<span class='danger'>unconcious proc ran!</span>")
					Unconscious(rand(20, 50), ignore_canstun = TRUE)
					adjustOxyLoss(2)
				if(prob(40))
					emote("gasp")
			if(-19 to -10)
				if(prob(35))
					visible_message("<span class='danger'>unconcious proc ran!</span>")
					Unconscious(rand(10, 30), ignore_canstun = TRUE)
					adjustOxyLoss(1.5)
				if(prob(30))
					emote("cough")
			if(-9 to crit_threshold)
				if(prob(25))
					visible_message("<span class='danger'>unconcious proc ran!</span>")
					Unconscious(rand(10, 20), ignore_canstun = TRUE)
					adjustOxyLoss(1)
				if(prob(20))
					emote("cough")
