...
/mob/living/carbon/human/get_clothing_examine_info(mob/living/user)
	...
	if(wear_id && !HAS_TRAIT(wear_id, TRAIT_EXAMINE_SKIP))
		...
		if(id && get_dist(user, src) <= ID_EXAMINE_DISTANCE)
			// Get the examine line without any links, then add the ID view link after it
			var/id_line = wear_id.get_examine_line(skip_examine_link = TRUE)
			var/id_href = "<a href='byond://?src=[REF(src)];see_id=1;id_ref=[REF(id)];id_name=[id.registered_name];examine_time=[world.time]'>\[Look at ID\]</a>"
			. += "[t_He] [t_is] wearing [id_line] [id_href]."
