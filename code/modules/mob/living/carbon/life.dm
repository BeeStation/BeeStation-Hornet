...
/mob/living/carbon/proc/natural_bodytemperature_stabilization(datum/gas_mixture/environment)
	var/areatemp = get_temperature(environment)
	var/bodytemp_difference = get_bodytemp_normal() - bodytemperature
	var/natural_change = 0

	// We are very cold, increate body temperature
	if(bodytemperature <= dna.species.bodytemp_cold_damage_limit)
		natural_change = max((bodytemp_difference * metabolism_efficiency / dna.species.bodytemp_autorecovery_divisor), \
			dna.species.bodytemp_autorecovery_min)

	// we are cold, reduce the minimum increment and do not jump over the difference
	else if(bodytemperature > dna.species.bodytemp_cold_damage_limit && bodytemperature < get_bodytemp_normal())
		natural_change = max(bodytemp_difference * metabolism_efficiency / dna.species.bodytemp_autorecovery_divisor, \
			min(bodytemp_difference, dna.species.bodytemp_autorecovery_min / 4))

	// We are hot, reduce the minimum increment and do not jump below the difference
	else if(bodytemperature > get_bodytemp_normal() && bodytemperature <= dna.species.bodytemp_heat_damage_limit)
		natural_change = min(bodytemp_difference * metabolism_efficiency / dna.species.bodytemp_autorecovery_divisor, \
			max(bodytemp_difference, -(dna.species.bodytemp_autorecovery_min / 4)))

	// We are very hot, reduce the body temperature
	else if(bodytemperature >= dna.species.bodytemp_heat_damage_limit)
		natural_change = min((bodytemp_difference / dna.species.bodytemp_autorecovery_divisor), -dna.species.bodytemp_autorecovery_min)
	...
	if(areatemp > bodytemperature) // It is hot here
		if(bodytemperature < get_bodytemp_normal())
