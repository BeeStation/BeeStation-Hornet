...
/mob/living/Bump(atom/A)
	...
	if(buckled || now_pushing)
		return
	if((confused || is_blind()) && stat <= SOFT_CRIT && (mobility_flags & MOBILITY_STAND) && m_intent == "run" && (!ismovableatom(A) || is_blocked_turf(A)) && !HAS_MOB_PROPERTY(src, PROP_CANTBUMPSLAM))  // ported from VORE, sue me
