...
/datum/tgui/New(mob/user, datum/src_object, interface, title, ui_x, ui_y)
	if(!user.client) // No client to show the TGUI to, so stop here
		return
	log_tgui(user, "new [interface] fancy [user.client.prefs.toggles & TOGGLE_FANCY_TGUI]")
	src.user = user
...
/datum/tgui/proc/open()
	...
	if(!window.is_ready())
		window.initialize(
			fancy = user.client.prefs.toggles & TOGGLE_FANCY_TGUI,
			inline_assets = list(
...
/datum/tgui/proc/get_payload(custom_data, with_data, with_static_data)
	...
	json_data["config"] = list(
		...
		"window" = list(
			...
			"size" = window_size,
			"fancy" = user.client.prefs.toggles & TOGGLE_FANCY_TGUI,
			"locked" = user.client.prefs.toggles & TOGGLE_LOCK_TGUI,
		),
		"client" = list(
