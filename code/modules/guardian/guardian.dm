...
/mob/living/simple_animal/hostile/guardian/proc/ResetMe()
	set waitfor = FALSE
	var/list/mob/dead/observer/candidates = pollGhostCandidates("Do you want to play as [summoner?.current?.name]'s [real_name]?", ROLE_KEY_HOLOPARASITE, null, FALSE, 10 SECONDS)
...
/mob/living/proc/guardian_reset()
	...
	if(LAZYLEN(guardians))
		...
		if(G)
			...
			var/list/mob/dead/observer/candidates = pollGhostCandidates("Do you want to play as [src.real_name]'s [G.real_name]?", ROLE_KEY_HOLOPARASITE, null, FALSE, 100)
			if(LAZYLEN(candidates))
				var/mob/dead/observer/C = pick(candidates)
