...
/obj/item/reagent_containers/food/drinks/afterattack(obj/target, mob/user , proximity)
	...
	if(target.is_refillable() && is_drainable()) //Something like a glass. Player probably wants to transfer TO it.
		...
		var/trans = src.reagents.trans_to(target, amount_per_transfer_from_this, transfered_by = user, methods = INGEST)
		...
		var/trans = target.reagents.trans_to(src, amount_per_transfer_from_this, transfered_by = user, methods = INGEST)
...
/obj/item/reagent_containers/food/drinks/soda_cans/suicide_act(mob/living/carbon/human/H)
	...
	reagents.trans_to(H, src.reagents.total_volume, transfered_by = H, methods = INGEST) //a big sip
	sleep(5)
