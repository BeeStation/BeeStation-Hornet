...
/obj/item/reagent_containers/food/drinks/attack(mob/living/M, mob/user, def_zone)
	...
	reagents.trans_to(mob_reagent_holder, gulp_amount, transfered_by = user)
...
/obj/item/reagent_containers/food/drinks/afterattack(obj/target, mob/user , proximity)
	...
	if(target.is_refillable() && is_drainable()) //Something like a glass. Player probably wants to transfer TO it.
		...
		if(mob_reagent_holder.holder_full())
			to_chat(user, "<span class='warning'>[target] is full.</span>")
			return

		var/refill = reagents.get_master_reagent_id()
		var/trans = src.reagents.trans_to(mob_reagent_holder, amount_per_transfer_from_this, transfered_by = user)
		...
		var/trans = mob_reagent_holder.trans_to(src, amount_per_transfer_from_this, transfered_by = user)
...
/obj/item/reagent_containers/food/drinks/soda_cans/suicide_act(mob/living/carbon/human/H)
	...
	reagents.trans_to(mob_reagent_holder, src.reagents.total_volume, transfered_by = H) //a big sip
	sleep(5)
