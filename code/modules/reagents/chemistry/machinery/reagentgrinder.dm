...
/obj/machinery/reagentgrinder/proc/grind_item(obj/item/grinded_item, mob/user) //Grind results can be found in respective object definitions
	if(!grinded_item.grind(beaker.reagents, user))
		if(isstack(grinded_item))
			to_chat(user, span_notice("[src] attempts to grind as many pieces of [grinded_item] as possible."))
		else
			to_chat(user, span_danger("[src] shorts out as it tries to grind up [grinded_item], and transfers it back to storage."))
		return
...
/obj/machinery/reagentgrinder/proc/mix_complete()
	...
	if(QDELETED(src) || !is_operational || QDELETED(beaker) || !beaker.reagents.total_volume)
		return
