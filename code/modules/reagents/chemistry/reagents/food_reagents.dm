...
/datum/reagent/consumable/frostoil/reaction_turf(turf/T, reac_volume)
	...
	if(reac_volume >= 1) // Make Freezy Foam and anti-fire grenades!
		if(isopenturf(T))
			...
			OT.air.temperature = max((OT.air.return_temperature() - MOLES_CELLSTANDARD*100*reac_volume/OT.air.heat_capacity()), TCMB) // reduces environment temperature by 5K per unit.
