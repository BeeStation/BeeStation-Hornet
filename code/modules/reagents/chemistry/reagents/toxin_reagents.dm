...
/datum/reagent/toxin/solidifying_ooze/on_mob_life(mob/living/carbon/M)
	...
	if(isoozeling(user))
		...
		if(stamina_loss < 90 && prob(90))
			user.setStaminaLoss(min(stamina_loss + rand(3, 9) * REM, 90))
		if(prob(20))
		...
		if(user.getorganslot(ORGAN_SLOT_STOMACH))
			if(prob(10))
				user.emote("cough")
			...
			if(!user.undergoing_cardiac_arrest() && user.can_heartattack())
				user.emote("gasp")
				user.set_heartattack(TRUE)
				if(user.stat == CONSCIOUS)
					user.visible_message("<span class='userdanger'>[user] clutches at [user.p_their()] chest as if [user.p_their()] heart stopped!</span>")
