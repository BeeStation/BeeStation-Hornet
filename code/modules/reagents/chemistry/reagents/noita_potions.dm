...
/datum/reagent/magic/polymorphine
	...
	var/obj/shapeshift_holder/data	
...
/datum/reagent/magic/berserkium/on_mob_metabolize(mob/living/L)
	. = ..()
	ADD_TRAIT(L, TRAIT_IGNOREDAMAGESLOWDOWN, TRAIT_REAGENT)
	ADD_TRAIT(L, TRAIT_NOSTAMCRIT, TRAIT_REAGENT)
	ADD_TRAIT(L, TRAIT_NOLIMBDISABLE, TRAIT_REAGENT)
...
/datum/reagent/magic/berserkium/on_mob_end_metabolize(mob/living/L)
	..()
	REMOVE_TRAIT(L, TRAIT_IGNOREDAMAGESLOWDOWN, TRAIT_REAGENT)
	REMOVE_TRAIT(L, TRAIT_NOSTAMCRIT, TRAIT_REAGENT)
	REMOVE_TRAIT(L, TRAIT_NOLIMBDISABLE, TRAIT_REAGENT)
...
/datum/reagent/magic/teleportarium/reaction_mob(mob/living/M, method=TOUCH, reac_volume)
	if(method == TOUCH || method == VAPOR)
		teleport_sucker(L)
	..()
...
/datum/reagent/magic/teleportarium/proc/teleport_sucker(mob/living/L)	
	var/atom/target = get_ranged_target_turf(start, L.dir, tele_range)
