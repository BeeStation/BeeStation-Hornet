...
/datum/reagent/consumable/ethanol/hooch/on_mob_life(mob/living/carbon/M)
	if(M.mind?.assigned_role == JOB_ASSISTANT)
...
/datum/reagent/consumable/ethanol/screwdrivercocktail/on_mob_life(mob/living/carbon/M)
	if(M.mind && (M.mind.assigned_role in list(JOB_STATION_ENGINEER, JOB_ATMOSPHERIC_TECHNICIAN, JOB_CHIEF_ENGINEER))) //Engineers lose radiation poisoning at a massive rate.
...
/datum/reagent/consumable/ethanol/bananahonk/on_mob_life(mob/living/carbon/M)
	if((ishuman(M) && M.job == JOB_CLOWN) || ismonkey(M))
		M.heal_bodypart_damage(1,1)
		. = 1
	return ..() || .
