...
#define METABOLITE_PENALTY(path) min(max(M.reagents.get_reagent_amount(path)/2.5, 1), 5) //Ranges from 1 to 5 depending on level of metabolites. 
...
/datum/reagent/proc/on_mob_life(mob/living/carbon/M)
	...
	if(metabolite && M.reagents.get_reagent_amount(metabolite) <= MAX_METABOLITES)
