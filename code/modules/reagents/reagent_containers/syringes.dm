		...
		if(SYRINGE_DRAW)
			...
			if(L) //living mob
				...
				if(target != user)
					...
					busy = TRUE
					if(!do_after(user, CHEM_INTERACT_DELAY(3 SECONDS, user), target = target, extra_checks=CALLBACK(L, TYPE_PROC_REF(/mob/living, can_inject), user, TRUE)))
						busy = FALSE
						return
					if(reagents.total_volume >= reagents.maximum_volume)
						return
		...
		if(SYRINGE_INJECT)
			...
			if(L) //living mob
				...
				if(L != user)
					...
					if(!do_after(user, CHEM_INTERACT_DELAY(3 SECONDS, user), target = L, extra_checks=CALLBACK(L, TYPE_PROC_REF(/mob/living, can_inject), user, TRUE)))
						return
					if(!reagents.total_volume)
						return
