...
/datum/addiction/proc/process_addiction(mob/living/carbon/affected_carbon, delta_time)
	...
	if(!on_drug_of_this_addiction && HAS_TRAIT(affected_carbon, TRAIT_ADDICTIONRESILIENT) && affected_carbon.mind.remove_addiction_points(type, addiction_loss_per_stage[withdrawal_stage + 2] * delta_time))
		return
