	...
	if(target.mind && target.client && target.stat != DEAD)
		if((!HAS_TRAIT(target, TRAIT_MINDSHIELD) || ignore_mindshield) && !istype(target.get_item_by_slot(ITEM_SLOT_HEAD), /obj/item/clothing/head/foilhat))
			if(HAS_TRAIT(target, TRAIT_MINDSHIELD) && ignore_mindshield)
				to_chat(user, span_notice("We bruteforce our way past the mental barriers of [target.name] and begin linking our minds!"))
			else
...
/obj/effect/proc_holder/spell/target_hive/hive_shock/cast(list/targets, mob/living/user = usr)
	...
	if(!hive)
		to_chat(user, span_notice("This is a bug. Error:HIVE1"))
		return
	to_chat(user, span_notice("We begin increasing the psionic bandwidth between ourself and the vessel!"))
	if(do_after(user,30,0,user))
		...
		if(power > 50 && user.get_virtual_z_level() == target.get_virtual_z_level())
			to_chat(user, span_notice("We have overloaded the vessel for a short time!"))
			target.Jitter(round(power/10))
			target.Unconscious(power)
		else
			to_chat(user, span_notice("The vessel was too far away to be affected!"))
	else
...
/obj/effect/proc_holder/spell/target_hive/hive_control/cast(list/targets, mob/living/user = usr)
	if(!active)
		...
		if(vessel && vessel.mind && backseat)
			...
			deadchat_broadcast(span_deadsay("<span class='name'>[vessel]</span> has just been mind controlled!"), vessel)
...
/obj/effect/proc_holder/spell/target_hive/hive_attack/cast(list/targets, mob/living/user = usr)
	...
	if(!user.is_real_hivehost())
		to_chat(user, span_notice("Our vessel is too weak to handle this power, we must cease our mind control beforehand."))
		revert_cast()
		return
	if(!target.undergoing_cardiac_arrest() && target.can_heartattack())
		target.set_heartattack(TRUE)
		to_chat(target, span_userdanger("You feel a sharp pain, and foreign presence in your mind!!"))
		to_chat(user, span_notice("We have overloaded the vessel's medulla! Without medical attention, they will shortly die."))
		if(target.stat == CONSCIOUS)
			target.visible_message(span_userdanger("[target] clutches at [target.p_their()] chest as if [target.p_their()] heart stopped!"))
			deadchat_broadcast(span_deadsay("<span class='name'>[target]</span> has suffered a mysterious heart attack!"), target)
	else
		to_chat(user, span_warning("We are unable to induce a heart attack!"))
	var/datum/antagonist/hivemind/hive = user.mind.has_antag_datum(/datum/antagonist/hivemind)
	if(hive)
		hive.threat_level += 4
...
/obj/effect/proc_holder/spell/target_hive/hive_warp/cast(list/targets, mob/living/user = usr)
	...
	if(target.get_virtual_z_level() != user.get_virtual_z_level())
		to_chat(user, span_notice("We are too far away from [target.name] to affect them!"))
		return
	to_chat(user, span_notice("We successfully distort reality surrounding [target.name]!"))
