...
/datum/autowiki/modsuits/generate()
	...
	for(var/theme_path in sort_list(subtypesof(/datum/mod_theme), GLOBAL_PROC_REF(cmp_typepaths_asc)))
		...
		output += include_template("Autowiki/ModsuitTheme", list(
			...
			"penetration" = armor_data.get_rating(ARMOUR_PENETRATION),
			"blunt" = armor_data.get_rating(ARMOUR_BLUNT),
			"absorption" = armor_data.get_rating(ARMOUR_ABSORPTION),
			"reflectivity" = armor_data.get_rating(ARMOUR_REFLECTIVITY),
