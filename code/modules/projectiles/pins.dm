...
/obj/item/firing_pin/paywall/pin_auth(mob/living/user)
	...
	if(ishuman(user))
		...
		if(credit_card_details && !active_prompt)
			var/license_request = tgui_alert(usr, "Do you wish to pay [payment_amount] credit[( payment_amount > 1 ) ? "s" : ""] for [( multi_payment ) ? "each shot of [gun.name]" : "usage license of [gun.name]"]?", "Weapon Purchase", list("Yes", "No"))
			active_prompt = TRUE
