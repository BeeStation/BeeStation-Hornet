...
/obj/item/gun/energy/wormhole_projector/update_ammo_types()
	...
	for(var/i in 1 to ammo_type.len)
		...
		if(istype(W))
			W.gun = WEAKREF(src)
			var/obj/projectile/laser/wormhole/WH = W.BB
			if(istype(WH))
				WH.gun = WEAKREF(src)
...
/obj/item/gun/energy/wormhole_projector/proc/create_portal(obj/projectile/laser/wormhole/W, turf/target)
	var/obj/effect/portal/P = new /obj/effect/portal(target, src, 300, null, FALSE, null, atmos_link)
	if(istype(W, /obj/projectile/laser/wormhole/orange))
		qdel(p_orange)
		p_orange = P
