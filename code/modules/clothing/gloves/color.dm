...
/obj/item/clothing/gloves/color/black/equipped(mob/user, slot)
	. = ..()
	if((slot == ITEM_SLOT_GLOVES) && (user.mind?.assigned_role.title in SSdepartment.get_jobs_by_dept_id(DEPARTMENT_NAME_SECURITY)))
...
/obj/item/clothing/gloves/color/black/dropped(mob/user)
	...
	if(carbon_user.gloves != src)
		return
	if(carbon_user.mind?.assigned_role.title in SSdepartment.get_jobs_by_dept_id(DEPARTMENT_NAME_SECURITY))
...
/obj/item/clothing/gloves/color/yellow/equipped(mob/user, slot)
	...
	if(slot == ITEM_SLOT_GLOVES)
		...
		if(user.mind?.assigned_role.title in SSdepartment.get_jobs_by_dept_id(DEPARTMENT_NAME_SECURITY))
