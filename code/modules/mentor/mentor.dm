...
/datum/mentors/New(ckey)
	if(!ckey)
		QDEL_IN(src, 0)
		STACK_TRACE_ADV("Mentor datum created without a ckey: [ckey]")
		return
	target = ckey(ckey)
	if(GLOB.mentor_datums[target])
		QDEL_IN(src, 0)
		STACK_TRACE_ADV("A second mentor datum was created for [target]!")
		return
...
/datum/mentors/proc/assign_to_client(client/C)
	...
	if(new_client_ckey != target) // what the fuck
		STACK_TRACE_ADV("Invalid client assigned to mentor datum for [target], the new client was [new_client_ckey]")
		return
	owner = C
