...
/datum/religion_rites/raise_undead/invoke_effect(mob/living/user, atom/movable/religious_tool)
	...
	var/list/candidates = pollGhostCandidates("Do you wish to be resurrected as a Holy Summoned Undead?", BAN_ROLE_HOLY_SUMMONED, null, 10 SECONDS, POLL_IGNORE_HOLYUNDEAD)
...
/datum/religion_rites/summon_carp/invoke_effect(mob/living/user, atom/movable/religious_tool)
	...
	var/list/candidates = pollGhostCandidates("Do you wish to be summoned as a Holy Carp?", BAN_ROLE_HOLY_SUMMONED, null, 10 SECONDS, POLL_IGNORE_HOLYCARP)
	if(!length(candidates))
