...
/obj/structure/altar_of_gods/attack_hand(mob/living/user)
	...
	if(pushed_mob.buckled)
		...
		return ..()
	if(pushed_mob in user.do_afters)
		to_chat(user, "<span class='warning'>You're already trying to coax [pushed_mob]!</span>")
		return ..()
	to_chat(user,"<span class='notice>You try to coax [pushed_mob] onto [src]...</span>")
	if(!do_after(user,(5 SECONDS),target = pushed_mob, show_to_target = TRUE))
		return ..()
