...
/datum/chatOutput/proc/analyzeClientData(cookie = "")
	...
	if(cookie != "none")
		...
		if(crashy_thingy.Find(cookie))
			message_admins("[key_name(src.owner)] tried to crash the server using malformed cookies. Ban them.")
			log_admin_private("[key_name(owner)] tried to crash the server using malformed cookies. Ban them.")
			return
