...
/obj/machinery/keycard_auth/proc/trigger_event(confirmer)
	...
	deadchat_broadcast(span_deadsay("[span_name("[triggerer]")] triggered [event] at [span_name("[A1.name]")]."), triggerer, message_type=DEADCHAT_ANNOUNCEMENT)

	var/area/A2 = get_area(confirmer)
	deadchat_broadcast(span_deadsay("[span_name("[confirmer]")] confirmed [event] at [span_name("[A2.name]")]."), confirmer, message_type=DEADCHAT_ANNOUNCEMENT)
	switch(event)
