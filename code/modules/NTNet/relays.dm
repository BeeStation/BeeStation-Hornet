...
/obj/machinery/ntnet_relay/process(delta_time)
	...
	if((dos_overload > dos_capacity) && !dos_failure)
		...
		ui_update()
		update_appearance()
		SSmodular_computers.add_log("DIAGNOSTICS // Quantum relay switched from normal operation mode to overload recovery mode.")
	// If the DoS buffer reaches 0 again, restart.
	if((dos_overload == 0) && dos_failure)
		set_dos_failure(FALSE)
		ui_update()
		update_appearance()
		SSmodular_computers.add_log("DIAGNOSTICS // Quantum relay switched from overload recovery mode to normal operation mode.")
	return ..()
...
/obj/machinery/ntnet_relay/ui_act(action, params)
	...
	switch(action)
		if("restart")
			...
			set_dos_failure(FALSE)
			update_appearance()
			SSmodular_computers.add_log("DIAGNOSTICS // Quantum relay manually restarted from overload recovery mode to normal operation mode.")
			return TRUE
		if("toggle")
			set_relay_enabled(!relay_enabled)
			SSmodular_computers.add_log("DIAGNOSTICS // Quantum relay manually [relay_enabled ? "enabled" : "disabled"].")
			update_appearance()
			return TRUE
