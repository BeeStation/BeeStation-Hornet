...
/obj/machinery/ntnet_relay/process(delta_time)
	...
	if((dos_overload > dos_capacity) && !dos_failure)
		...
		ui_update()
		update_icon()
		SSnetworks.add_log("DIAGNOSTICS // Quantum relay switched from normal operation mode to overload recovery mode.")
	// If the DoS buffer reaches 0 again, restart.
	if((dos_overload == 0) && dos_failure)
		set_dos_failure(FALSE)
		ui_update()
		update_icon()
		SSnetworks.add_log("DIAGNOSTICS // Quantum relay switched from overload recovery mode to normal operation mode.")
	..()
...
/obj/machinery/ntnet_relay/ui_act(action, params)
	...
	switch(action)
		if("restart")
			...
			set_dos_failure(FALSE)
			update_icon()
			SSnetworks.add_log("DIAGNOSTICS // Quantum relay manually restarted from overload recovery mode to normal operation mode.")
			return TRUE
		if("toggle")
			set_relay_enabled(!relay_enabled)
			SSnetworks.add_log("DIAGNOSTICS // Quantum relay manually [relay_enabled ? "enabled" : "disabled"].")
			update_icon()
			return TRUE
...
/obj/machinery/ntnet_relay/Initialize(mapload)
	...
	SSnetworks.add_log("DIAGNOSTICS // New quantum relay activated. Current amount of linked relays: [SSmodular_computers.ntnet_relays.len]")
	return ..()

/obj/machinery/ntnet_relay/Destroy()
	SSmodular_computers.ntnet_relays.Remove(src)
	SSnetworks.add_log("DIAGNOSTICS // Quantum relay connection severed. Current amount of linked relays: [SSmodular_computers.ntnet_relays.len]")
