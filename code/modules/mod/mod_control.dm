...
/obj/item/mod/control
	...
	armor = list(MELEE = 0, BULLET = 0, LASER = 0, ENERGY = 0, BOMB = 0, BIO = 100, FIRE = 25, ACID = 25, BLEED = 10)
	actions_types = list(
...
/obj/item/mod/control/examine(mob/user)
	. = ..()
	if(active)
		. += "<span class='notice'>Cell power: [cell ? "[round(cell.percent(), 1)]%" : "No cell"].</span>"
	...
	if(!open && !active)
		...
		. += "<span class='notice'>You could update the access with an <b>ID</b>.</span>"
		. += "<span class='notice'>You could access the wire panel with a <b>wire configuring tool</b>.</span>"
		if(cell)
			. += "<span class='notice'>You could remove the cell with an <b>empty hand</b>.</span>"
		else
			. += "<span class='notice'>You could use a <b>cell</b> on it to install one.</span>"
		if(ai)
