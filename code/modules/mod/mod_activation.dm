...
/obj/item/mod/control/proc/quick_deploy(mob/user)
	...
	for(var/obj/item/part as anything in mod_parts)
		if(deploy && part.loc == src)
			...
			conceal(null, part)
	wearer.visible_message("<span class='notice'>[wearer]'s [src] [deploy ? "deploys" : "retracts"] its' pieces with a mechanical hiss.</span>",
		"<span class='notice'>[src] [deploy ? "deploys" : "retracts"] its' pieces with a mechanical hiss.</span>",
...
/obj/item/mod/control/proc/deploy(mob/user, part)
	...
	if(wearer.equip_to_slot_if_possible(piece, piece.slot_flags, qdel_on_fail = FALSE, disable_warning = TRUE))
		...
		if(!user)
			return TRUE
		wearer.visible_message("<span class='notice'>[wearer]'s [piece.name] deploy[piece.p_s()] with a mechanical hiss.</span>",
...
/obj/item/mod/control/proc/conceal(mob/user, part)
	...
	if(!user)
		return
	wearer.visible_message("<span class='notice'>[wearer.name]'s [piece] retract[piece.p_s()] back into [src] with a mechanical hiss.",
		"<span class='notice'>[piece] retract[piece.p_s()] back into [src] with a mechanical hiss.",
		"<span class='hear'>You hear a mechanical hiss.</span>")
	playsound(src, 'sound/mecha/mechmove03.ogg', 25, TRUE, SHORT_RANGE_SOUND_EXTRARANGE)
