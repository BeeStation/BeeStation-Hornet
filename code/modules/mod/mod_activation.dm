...
/obj/item/mod/control/proc/deploy(mob/user, part)
	...
	if(wearer.equip_to_slot_if_possible(piece, piece.slot_flags, qdel_on_fail = FALSE, disable_warning = TRUE))
		...
		if(!user)
			return TRUE
		wearer.visible_message("<span class='notice'>[wearer]'s [piece] deploy[piece.p_s()] with a mechanical hiss.</span>",
...
/obj/item/mod/control/proc/conceal(mob/user, part)
	...
	if(!user)
		return
	wearer.visible_message("<span class='notice'>[wearer]'s [piece] retract[piece.p_s()] back into [src] with a mechanical hiss.",
