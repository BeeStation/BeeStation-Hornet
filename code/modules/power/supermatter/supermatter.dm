...
/proc/supermatter_anomaly_gen(turf/anomalycenter, type = ANOMALY_FLUX, anomalyrange = 5, has_weak_lifespan = TRUE)
	...
	switch(type)
		if(ANOMALY_BIOSCRAMBLER)
			new /obj/effect/anomaly/bioscrambler(local_turf, null, 1)
		if(ANOMALY_FLUX)
			var/explosive = has_weak_lifespan ? ANOMALY_FLUX_NO_EXPLOSION : ANOMALY_FLUX_LOW_EXPLOSIVE
			new /obj/effect/anomaly/flux(local_turf, has_weak_lifespan ? rand(250, 300) : null, TRUE, explosive, 1)
		if(ANOMALY_GRAVITATIONAL)
			new /obj/effect/anomaly/grav(local_turf, has_weak_lifespan ? rand(200, 300) : null, 1)
		if(ANOMALY_HALLUCINATION)
			new /obj/effect/anomaly/hallucination(local_turf, has_weak_lifespan ? rand(150, 250) : null, 1)
		if(ANOMALY_PYRO)
			new /obj/effect/anomaly/pyro(local_turf, has_weak_lifespan ? rand(150, 250) : null, 1)
		if(ANOMALY_VORTEX)
			new /obj/effect/anomaly/bhole(local_turf, 20, 1)
