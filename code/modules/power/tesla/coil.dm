...
/obj/machinery/power/tesla_coil/research/tesla_act(power, tesla_flags, shocked_things)
	if(anchored && !panel_open)
		...
		var/datum/bank_account/D = SSeconomy.get_budget_account(ACCOUNT_ENG_ID)
		if(D)
			D.adjust_money(min(power_produced, 3)*10)
		if(istype(linked_techweb))
			linked_techweb.add_point_type(TECHWEB_POINT_TYPE_DEFAULT, min(power_produced, 3)*10)
			linked_techweb.add_point_type(TECHWEB_POINT_TYPE_DISCOVERY, min(power_produced, 3)*10) // x4 coils with a pulse per second or so = ~7200/m point bonus for R&D
		addtimer(CALLBACK(src, .proc/reset_shocked), 10)
		tesla_buckle_check(power)
	else
		..()
