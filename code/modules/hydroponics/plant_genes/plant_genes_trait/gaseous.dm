...
/datum/plant_gene/trait/smoke/proc/activate_effect(obj/item/reagent_containers/food/snacks/grown/G, atom/target, p_method)
	...
	S.start()
	log_admin_private("a plant with gas trait has made a gas containing [G.reagents.log_list()] at [AREACOORD(T)]. Last pickup ckey: [G.fingerprintslast]. #botany.")
	message_admins("a plant with gas trait has made a gas containing [G.reagents.log_list()] at [ADMIN_VERBOSEJMP(T)]. Last pickup ckey: [G.fingerprintslast]")
	G.investigate_log(" has created a smoke containing [G.reagents.log_list()] at [AREACOORD(T)]. Last pickup ckey: [G.fingerprintslast]. #botany.", INVESTIGATE_BOTANY)
