...
/datum/plant_gene/trait/squash/on_slip(obj/item/reagent_containers/food/snacks/grown/G, mob/living/carbon/C)
	...
	log_botany("[key_name(C)] slipped on [G.seed] causing it to squash at [AREACOORD(C)][get_plant_stats(G.seed, G)]")
	G.squash(C)
...
/datum/plant_gene/trait/slip/proc/handle_slip(obj/item/reagent_containers/food/snacks/grown/G, mob/M)
	log_botany("[M] slipped on [G] at [AREACOORD(M)][get_plant_stats(G.seed, G)]")
...
/datum/plant_gene/trait/cell_charge/on_slip(obj/item/reagent_containers/food/snacks/grown/G, mob/living/carbon/C)
	...
	if(prob(power))
		log_botany("[key_name(C)] was electrocuted by slipping on [G.seed] at [AREACOORD(C)][get_plant_stats(G.seed, G)]")
...
/datum/plant_gene/trait/cell_charge/on_squash(obj/item/reagent_containers/food/snacks/grown/G, atom/target)
	if(iscarbon(target))
		...
		if(prob(power))
			C.electrocute_act(round(power), G, 1, 1)
			log_botany("[key_name(C)] was electrocuted by squashing [G.seed] at [AREACOORD(C)][get_plant_stats(G.seed, G)]")
...
/datum/plant_gene/trait/teleport/on_squash(obj/item/reagent_containers/food/snacks/grown/G, atom/target)
	if(isliving(target))
		...
		log_botany("[key_name(target)] teleported from [AREACOORD(T)] to [AREACOORD(target)] by squashing [G.seed][get_plant_stats(G.seed, G)]")
...
/datum/plant_gene/trait/teleport/on_slip(obj/item/reagent_containers/food/snacks/grown/G, mob/living/carbon/C)
	...
	log_botany("[key_name(C)] teleported from [AREACOORD(T)] to [AREACOORD(C)] by slipping on [G.seed][get_plant_stats(G.seed, G)]")
	if(prob(50))
...
/datum/plant_gene/trait/noreact/on_squashreact(obj/item/reagent_containers/food/snacks/grown/G, atom/target)
	...
	log_botany("[G.seed] reagents [G.reagents.get_reagents()] reacted because of squashing at [AREACOORD(target)] (Separated Chemicals)[get_plant_stats(G.seed, G)]")
...
/datum/plant_gene/trait/stinging/on_throw_impact(obj/item/reagent_containers/food/snacks/grown/G, atom/target)
	if(isliving(target) && G.reagents && G.reagents.total_volume)
		...
		if(L.reagents && L.can_inject(null, 0))
			...
			log_botany("[key_name(target)] was pricked by [G.seed] injecting them with [injecting_amount] units of the mixture {[G.reagents.get_reagents()]} at [AREACOORD(target)][get_plant_stats(G.seed, G)]")
...
/datum/plant_gene/trait/smoke/on_squash(obj/item/reagent_containers/food/snacks/grown/G, atom/target)
	...
	S.start()
	log_botany("Smoke containing [G.reagents] created at [AREACOORD(target)] from plant [G.seed][get_plant_stats(G.seed, G)]")
