...
/obj/item/clothing/glasses/spy/proc/show_to_user(mob/user)//this is the meat of it. most of the map_popup usage is in this.
	if(!user)
		return
	if(!user.client)
		return
...
/obj/item/clothing/glasses/spy/equipped(mob/user, slot)
	. = ..()
	if(slot != ITEM_SLOT_EYES)
		user.client.close_popup("spypopup")

/obj/item/clothing/glasses/spy/dropped(mob/user)
	. = ..()
	user.client.close_popup("spypopup")

/obj/item/clothing/glasses/spy/ui_action_click(mob/user)
	show_to_user(user)

/obj/item/clothing/glasses/spy/item_action_slot_check(slot)
	if(slot == ITEM_SLOT_EYES)
		return TRUE

/obj/item/clothing/glasses/spy/Destroy()
	if(linked_bug)
...
/obj/item/spy_bug
	...
	var/obj/item/clothing/glasses/spy/linked_glasses
...
/obj/item/storage/box/rxglasses/spyglasskit/PopulateContents()
	...
	var/obj/item/clothing/glasses/spy/newglasses = new(src)
