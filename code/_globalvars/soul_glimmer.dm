/*
    Colours and assignments for psychic sense
    colour
    color
    :trolled:
*/

//ALL the psychic soul colours
GLOBAL_LIST_INIT(SOUL_GLIMMER_COLORS, list("Azure" = "#0091ff", "Vermilion" = "#ff3a1c", "Sage" = "#1fff5e", "Teal" = "#00ffe5", "Crimson" = "#ff7c94", "Celadon" = "#b8ffa6"))

//Low pop colours
#define SOUL_GLIMMER_LOWER_POP 10
GLOBAL_LIST_INIT(SOUL_GLIMMER_COLORS_LOW, list("Azure" = "#0091ff", "Vermilion" = "#ff3a1c", "Sage" = "#1fff5e"))
//mid pop colours
#define SOUL_GLIMMER_MID_POP 20
GLOBAL_LIST_INIT(SOUL_GLIMMER_COLORS_MID, list("Teal" = "#00ffe5", "Crimson" = "#ff002f", "Celadon" = "#b8ffa6"))
