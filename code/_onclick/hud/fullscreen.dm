...
/atom/movable/screen/fullscreen/tweak
	icon_state = "tweaking"
	alpha = 255
	plane = LIGHTING_PLANE
	layer = BYOND_LIGHTING_PLANE + 1

/atom/movable/screen/fullscreen/tweak/update_for_view(client_view)
...
/atom/movable/screen/fullscreen/tweak/proc/start_hue_rotation()
	animate(src, color = color_matrix_rotate_hue(1), loop = -1, time = 2 SECONDS) //Start the loop.
	var/step_precision = 18 //Larger is more precise rotations.
	for(var/current_step in 1 to step_precision - 1) //We do the -1 here because 360 == 0 when it comes to angles.
		animate(
...
/atom/movable/screen/fullscreen/glaggle
	icon_state = "glaggle"
	alpha = 255
	plane = LIGHTING_PLANE
	layer = BYOND_LIGHTING_PLANE + 1

/atom/movable/screen/fullscreen/glaggle/update_for_view(client_view)
...
/atom/movable/screen/fullscreen/glaggle/proc/start_hue_rotation()
	animate(src, color = color_matrix_rotate_hue(1), loop = -1, time = 2 SECONDS) //Start the loop.
	var/step_precision = 18 //Larger is more precise rotations.
	for(var/current_step in 1 to step_precision - 1) //We do the -1 here because 360 == 0 when it comes to angles.
		animate(
