...
/client/proc/can_take_ghost_spawner(banning_key = BAN_ROLE_ALL_ANTAGONISTS, use_cooldown = TRUE, is_ghost_role = FALSE, is_admin_spawned = FALSE)
	...
	if(!src.should_include_for_role(
		...
		return FALSE
	if(use_cooldown && src.data.next_ghost_role_tick > world.time)
		to_chat(src, "<span class='warning'>You have died recently, you must wait [(src.data.next_ghost_role_tick - world.time)/10] seconds until you can use a ghost spawner.</span>")
		return FALSE
	return TRUE
