...
#define ACCOUNT_COM "Command" // dummy for card manipulation system. there's no command account.

// If you
#define ACCOUNT_CIV_FLAG (1<<0)
#define ACCOUNT_SRV_FLAG (1<<1)
#define ACCOUNT_CAR_FLAG (1<<2)
#define ACCOUNT_ENG_FLAG (1<<3)
#define ACCOUNT_SCI_FLAG (1<<4)
#define ACCOUNT_MED_FLAG (1<<5)
#define ACCOUNT_SEC_FLAG (1<<6)
#define ACCOUNT_COM_FLAG (1<<7) // for Commander only vender items (i.e. HoP cartridge vendor)
