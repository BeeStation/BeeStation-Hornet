...
#define STANDARDISE_ARMOUR(val) round(CALCULATE_ARMOUR(val, ARMOUR_DROPOFF_START, ARMOUR_MAXIMUM - ARMOUR_DROPOFF_START))
...
#define CALCULATE_ARMOUR(val, armour_dropoff_start, armour_diff) (val > armour_dropoff_start ? ((arctan((x - armour_dropoff_start) / (0.6 * armour_diff)) / 90) * armour_diff + armour_dropoff_start) : val)
