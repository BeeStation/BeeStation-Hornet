...
#define IS_BLUNT				0	//Can only remove limbs if they're easy to remove
#define IS_SHARP				1	//Can only remove limbs if target is dead
#define IS_SHARP_DISMEMBER		2	//Can only remove limbs if the limb is already disabled
#define IS_SHARP_DISMEMBER_EASY	3	//Has a chance equal to weapon force to remove limb on every attack, in some cases taking them off in one swing
