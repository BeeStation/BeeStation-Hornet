...
//General helpers
#define VV_HREF_TARGET_INTERNAL(target, href_key) "?"+HREF_COMMAND(var_edit)+"[HrefToken()];[href_key]=TRUE;[VV_HK_TARGET]=[REF(target)]"
#define VV_HREF_TARGETREF_INTERNAL(targetref, href_key) "?"+HREF_COMMAND(var_edit)+"[HrefToken()];[href_key]=TRUE;[VV_HK_TARGET]=[targetref]"
...
#define VV_HREF_SPECIAL(dmlist_origin_ref, href_action, text, list_index, dmlist_varname) "<a href='byond://?"+HREF_COMMAND(var_edit)+"[HrefToken()];[href_action]=TRUE;dmlist_origin_ref=[dmlist_origin_ref];dmlist_varname=[dmlist_varname];[VV_HK_VARNAME]=[list_index]'>[text]</a>"
#define VV_HREF_SPECIAL_MENU(dmlist_origin_ref, href_action, dmlist_varname) "byond://?"+HREF_COMMAND(var_edit)+"[HrefToken()];[href_action]=TRUE;[VV_HK_DO_LIST_EDIT]=TRUE;dmlist_origin_ref=[dmlist_origin_ref];dmlist_varname=[dmlist_varname]"
...
#define VV_TOPIC_LINK(datum, href_key, text) "<a href='byond://?"+HREF_COMMAND(var_edit)+"[HrefToken()];[href_key]=TRUE;target=[REF(datum)]'>text</a>"

//Helpers for vv_get_dropdown()
#define VV_DROPDOWN_OPTION(href_key, name) . += "<option value='byond://?"+HREF_COMMAND(var_edit)+"[HrefToken()];[href_key]=TRUE;target=[REF(src)]'>[name]</option>"
//Same with VV_DROPDOWN_OPTION, but global proc doesn't have src
#define VV_DROPDOWN_OPTION_APPEARANCE(thing, href_key, name) . += "<option value='byond://?"+HREF_COMMAND(var_edit)+"[HrefToken()];[href_key]=TRUE;target=[REF(thing)]'>[name]</option>"
