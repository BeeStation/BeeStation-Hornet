...
#define DECLARE_HEAP_TYPE(typepath, stored_type, compare_value) ##typepath/var/list/elements;\
##typepath/New(...) {\
		elements = args.Copy();\
	}\
...
##typepath/proc/swim(index) {\
		...
		var##stored_type/left = elements[index];\
		var##stored_type/right = elements[parent];\
		while(parent > 0 && (left.##compare_value - right.##compare_value > 0)) {\
...
##typepath/proc/sink(index) {\
		...
		var##stored_type/left = elements[index];\
		var##stored_type/right = elements[g_child];\
		while(g_child > 0 && (left.##compare_value - right.##compare_value < 0)) {\
