...
#define ADMIN_JMP(src) "(<a href='?_src_=holder;[HrefToken(TRUE)];adminplayerobservecoodjump=1;X=[src.x];Y=[src.y];Z=[src.get_z_level()]'>JMP</a>)"
#define COORD(src) "[src ? "([src.x],[src.y],[src.get_z_level()])" : "nonexistent location"]"
#define AREACOORD(src) "[src ? "[get_area_name(src, TRUE)] ([src.x], [src.y], [src.get_z_level()])" : "nonexistent location"]"
