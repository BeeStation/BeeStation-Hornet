...
/// Sent from /datum/action/cooldown/spell/set_click_ability() to the caster: (datum/action/spell/spell)
...
// Lichdom
/// Sent from /datum/action/cooldown/spell/lichdom/cast(), to the item being imbued: (datum/action/spell/spell, mob/user)
#define COMSIG_ITEM_IMBUE_SOUL "item_imbue_soul"
	/// Return to stop the cast and prevent the soul imbue
	#define COMPONENT_BLOCK_IMBUE (1 << 0)

/// Sent from /datum/action/cooldown/spell/aoe/knock/cast(), to every nearby turf (for connect loc): (datum/action/spell/aoe/knock/spell, mob/living/caster)
#define COMSIG_ATOM_MAGICALLY_UNLOCKED "atom_magic_unlock"

// Instant Summons
/// Sent from /datum/action/cooldown/spell/summonitem/cast(), to the item being marked for recall: (datum/action/spell/spell, mob/user)
#define COMSIG_ITEM_MARK_RETRIEVAL "item_mark_retrieval"
	/// Return to stop the cast and prevent the item from being marked
	#define COMPONENT_BLOCK_MARK_RETRIEVAL (1 << 0)

// Charge
/// Sent from /datum/action/cooldown/spell/charge/cast(), to the item in hand being charged: (datum/action/spell/spell, mob/user)
