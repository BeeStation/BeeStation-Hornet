...
#define STOPSPRESSUREDAMAGE		(1<<3)	//! SUIT and HEAD items which stop pressure damage. To stop you taking all pressure damage you must have both a suit and head item with this flag. Items should only have 1 pressure protection flag.
...
#define SNUG_FIT                (1<<11)  //! prevents hat throwing from knocking this hat off
#define EFFECT_HAT              (1<<12) //! For hats with an effect that shouldn't get knocked off ie finfoil
#define SCAN_REAGENTS           (1<<13) //! Allows helmets and glasses to scan reagents.
#define MASKEXTENDRANGE			(1<<14) //! For masks, allows you to breathe from internals on adjecent tiles
#define NOTCONSUMABLE			(1<<15) //! Moths cannot eat clothing with that flag
