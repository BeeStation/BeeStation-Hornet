...
/datum/controller/subsystem/lag_switch/proc/change_slowmode_cooldown(length)
	...
	if(measures[SLOWMODE_SAY])
		to_chat(world, "<span class='bold'>Slowmode timer has been changed to [length] seconds by an admin.</span>")
	return TRUE
	...
	switch(measure_key)
		if(DISABLE_DEAD_KEYLOOP)
			if(state)
				...
				deadchat_broadcast("<span class='big'>To increase performance Observer freelook is now disabled. Please use Orbit, Teleport, and Jump to look around.</span>")
			else
				GLOB.keyloop_list |= GLOB.player_list
				deadchat_broadcast("Observer freelook has been re-enabled. Enjoy your wooshing.")
