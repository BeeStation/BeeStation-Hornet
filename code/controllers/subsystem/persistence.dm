...
/datum/controller/subsystem/persistence/proc/LoadPaintings()
	...
	if(paintings && length(paintings["library"]) && !paintings["library"][1]["price"] &&  paintings == paintings[length(paintings)] ) //this should only ever happen once - adding owner and price values to legacy paintings
