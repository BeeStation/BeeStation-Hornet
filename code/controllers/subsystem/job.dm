...
/datum/controller/subsystem/job/proc/GiveRandomJob(mob/dead/new_player/authenticated/player)
	...
	. = FALSE
	if(player.client?.prefs?.has_quirk_trait(TRAIT_STOWAWAY))
...
/datum/controller/subsystem/job/proc/is_valid_job(mob/dead/new_player/authenticated/player, datum/job/job, required_priority)
	if(!job || !(job.job_flags & JOB_NEW_PLAYER_JOINABLE))
		return FALSE
	if(player.client?.prefs?.has_quirk_trait(TRAIT_STOWAWAY) && job.title != overflow_role)
		return FALSE
	if(is_banned_from(player.ckey, job.title))
...
/mob/living/carbon/human/on_job_equipping(datum/job/job, joined_late, client/player_client)
	if(player_client.mob.mind.account_id)
		player_client.mob.add_memory("Your account ID is [player_client.mob.mind.account_id].")
	return dress_up_as_job(job, FALSE, player_client, joined_late)
...
/mob/living/carbon/human/dress_up_as_job(datum/job/job, visual_only = FALSE, client/player_client, joined_late = FALSE)
	return job.equip(src, visual_only, TRUE, joined_late, null, player_client)
