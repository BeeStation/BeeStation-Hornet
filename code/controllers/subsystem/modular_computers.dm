...
/datum/controller/subsystem/modular_computers/proc/check_function(specific_action = NONE, signal_level)
	// No relays found. NTNet is down
	if(!length(ntnet_relays))
		return FALSE
	// Check all relays. If we have at least one working relay, network is up.
	if(signal_level != (SIGNAL_NO_RELAY || SIGNAL_HACKED) && !check_relay_operation())
		return FALSE
