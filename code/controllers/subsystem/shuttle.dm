...
/datum/controller/subsystem/shuttle/proc/requestEvac(mob/user, call_reason)
	...
	if(user)
		...
		deadchat_broadcast(span_deadsay("[span_name("[user.real_name]")] has called the shuttle at [span_name("[A.name]")]."), user, message_type=DEADCHAT_ANNOUNCEMENT)
	if(call_reason)
...
/datum/controller/subsystem/shuttle/proc/cancelEvac(mob/user)
	if(canRecall())
		...
		deadchat_broadcast(span_deadsay("[span_name("[user.real_name]")] has recalled the shuttle from [span_name("[get_area_name(user, TRUE)]")]."), user, message_type=DEADCHAT_ANNOUNCEMENT)
		return 1
