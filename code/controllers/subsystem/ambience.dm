...
/datum/controller/subsystem/ambience/fire(resumed)
	for(var/client/client_iterator as anything in ambience_listening_clients)
		...
		//SEND_SOUND(client_iterator.mob, sound(soundin, repeat = 0, wait = 0, volume = 25, channel = CHANNEL_AMBIENCE))
...
/datum/controller/subsystem/ambience/play_buzz(mob/M, area/A)
	if (A.ambient_buzz && (M.client.prefs.toggles & SOUND_SHIP_AMBIENCE) && M.can_hear_ambience())
		if (!M.client.buzz_playing || (A.ambient_buzz != M.client.buzz_playing))
			SEND_SOUND(M, sound(A.ambient_buzz, repeat = 1, wait = 0, volume = 35, channel = CHANNEL_BUZZ))
...
/datum/controller/subsystem/ambience/proc/play_ambience_music(mob/M, _ambi_music, area/A) // Background music, the more OOC ambience, like eerie space music
	if(A.ambientmusic && !M.client?.channel_in_use(CHANNEL_AMBIENT_MUSIC)) // 1/80 chance to play every second, only play while another one is not playing
		SEND_SOUND(M, sound(_ambi_music, repeat = 0, wait = 0, volume = 25, channel = CHANNEL_AMBIENT_MUSIC))
...
/datum/controller/subsystem/ambience/proc/play_ambience_effects(mob/M, _ambi_fx, area/A) // Effect, random sounds that will play at random times, IC (requires the user to be able to hear)
	if (length(A.ambientsounds) && M.can_hear_ambience() && !M.client?.channel_in_use(CHANNEL_AMBIENT_EFFECTS)) // 1/20 chance to play every second after cooldown
		SEND_SOUND(M, sound(_ambi_fx, repeat = 0, wait = 0, volume = 25, channel = CHANNEL_AMBIENT_EFFECTS))
