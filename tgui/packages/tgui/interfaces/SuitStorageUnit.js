...
export const SuitStorageUnit = (props, context) => {
  ...
  const { locked, open, safeties, uv_active, occupied, suit, helmet, mask, storage } = data;
  return (
    <Window width={400} height={305}>
      <Window.Content>
        {!!(occupied && safeties) && (
          <NoticeBox>Biological entity detected in suit chamber. Please remove before continuing with operation.</NoticeBox>
        )}
        {(uv_active && <NoticeBox>Contents are currently being decontaminated. Please wait.</NoticeBox>) || (
          <Section
            title="Storage"
            minHeight="260px"
            buttons={
              <Fragment>
                {!open && (
                  <Button icon={locked ? 'unlock' : 'lock'} content={locked ? 'Unlock' : 'Lock'} onClick={() => act('lock')} />
                )}
                {!locked && (
                  <Button
