        ...
        <Flex direction={'column'} height="100%">
          <Flex.Item mb={1}>
            <Section title="CORE.STAT" width="100%">
              <Box my={1}>Internal ID :: {data.name}</Box>
              <ProgressBar
                value={
                  data.max_charge ? (data.charge / data.max_charge) * 100 : 0
                }
                minValue={0}
                maxValue={100}
                ranges={{
                ...
                <Flex.Item grow>
                  ...
                  >
                    ...
                    >
                      <Flex.Item mb={1}>
                        IO_PORT_AUX{' '}
                        {data.control_circuit ? 'OCCUPIED' : 'EMPTY'}
                      </Flex.Item>
                      <Flex.Item
                        mb={1}
                        fontSize="20px"
                        color={data.control_circuit ? 'green' : 'red'}
                      >
                        {data.control_circuit
...
export const UpgradeStats = (props: UpgradeProps) => {
  ...
  return (
    <Flex direction={'column'} height="100%" justify="space-evenly">
      ...
      <Flex.Item
        textJustify="center"
        color={passive ? 'green' : props.upgrade.active ? 'green' : 'red'}
      >
