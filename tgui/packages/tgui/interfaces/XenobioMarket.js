    ...
    <Window width={900} height={(tabIndex === 1 && 412) || 600}>
      <Window.Content>
        <Tabs style={{ 'border-radius': '5px' }}>
          <Tabs.Tab key={1} selected={tabIndex === 1} icon="flask" onClick={() => setTabIndex(1)}>
            Slime Market
          </Tabs.Tab>
          <Tabs.Tab key={2} selected={tabIndex === 2} icon="flask" onClick={() => setTabIndex(2)}>
            Active Requests
          </Tabs.Tab>
          <Tabs.Tab key={3} selected={tabIndex === 3} icon="sack-dollar" onClick={() => setTabIndex(3)}>
            View Shop
          </Tabs.Tab>
            ...
            <Table.Cell width="25%" key={slime_price.key}>
              {!!slime_price.price && (
                <Section style={{ 'border-radius': '5px' }} mb="6px">
                  <Stack fill>
                    <Stack.Item>
                      <Box className={classes(['xenobio_market32x32', slime_price.icon])} />
                    </Stack.Item>
                    <Stack.Item mt="10px">Currect price: {toFixed(slime_price.price, 0)} points.</Stack.Item>
                  </Stack>
                </Section>
              )}
            </Table.Cell>
      ...
      {requests.map((request) => (
        <Section style={{ 'border-radius': '5px' }} mb="6px" key={request.name}>
          <Stack fill>
            ...
            <Stack.Item mt="10px">
              | Payout: {toFixed(request.payout, 0)} credits. | Xenobiology Points: {toFixed(request.payout * 3, 0)}
            </Stack.Item>
            <Stack.Item mt="10px">| {toFixed(request.amount - request.amount_give, 0)} extracts left.</Stack.Item>
          </Stack>
        </Section>
      ))}
    </Table>
      ...
      {shop_items.map((item) => (
        <Section style={{ 'border-radius': '5px' }} mb="6px" key={item.name}>
          <Stack fill>
            ...
            <Stack.Item>
              <Button ml="10px" mt="7px" icon={'question'} tooltip={item.desc} />
            </Stack.Item>
            <Stack.Item>
              <Button ml="5px" mt="7px" icon={'sack-dollar'} color="green" onClick={() => act('buy', { path: item.item_path })}>
