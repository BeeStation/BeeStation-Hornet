                ...
                <LabeledList.Item label="Possible Mutations">
                  <Collapsible title={slime.slime_color + ' Mutations'}>
                    {slime.possible_mutations.map((mutation) => (
                      <Section
                        key={mutation.color}
                        backgroundColor={!mutation.mobs_needed & !mutation.items_needed ? 'green' : ''}>
                        <Stack>
                          <Box>{mutation.color + ' Slime'}</Box>
                          <Button ml="10px" icon={'spider'} disabled={!mutation.mobs_needed} tooltip={mutation.mobs_needed} />
                          <Button
                            ml="10px"
                            icon={'drumstick-bite'}
                    ...
                    {slime.traits.map((trait) => (
                      <Section key={trait.name}>
                        <Stack>
                          ...
                          <Button ml="5px" icon={'question'} disabled={!trait.desc} tooltip={trait.desc} />
                        </Stack>
                      </Section>
                    ))}
                  </Collapsible>
...
const StoreViewer = (_, context) => {
  ...
  const { buyable_upgrades, corral_upgrades, reagent_data, reagent_amount } = data;
        ...
        {reagent_data.map((reagent) => (
          <LabeledList.Item key={reagent.name} label={reagent.name}>
            <ProgressBar
              ...
              minValue={0}
              maxValue={100}>
              {toFixed((reagent.amount / reagent_amount) * 100, 0.1) + ' % (' + toFixed(reagent.amount, 0.1) + ' units)'}
            </ProgressBar>
          </LabeledList.Item>
        ))}
      </LabeledList>
