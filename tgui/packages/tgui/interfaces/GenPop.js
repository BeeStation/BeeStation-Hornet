...
export const GenPop = (props, context) => {
  ...
  const [
    crimeName,
    setCrimeName,
  ] = useSharedState(context, 'crimeName', 'No crime');

  // Local state for the name of the crime details
  const [
    crimeDetails,
    setCrimeDetails,
  ] = useSharedState(context, 'crimeDetails', 'No details provided');

  // Local state for the current category that we are browsing
  const [
    crimeCategory,
    setCrimeCategory,
  ] = useSharedState(context, 'crimeCategory', (crime_list && crime_list.length > 0) ? Object.keys(crime_list)[0] : null);
    ...
    <Window resizable width={625} height={400}>
      <Window.Content scrollable>
          ...
          buttons={
            <Fragment>
              ...
              <Button
                icon="print"
                content="Finalize ID"
                tooltip={(attemptedMod && time <= 3000) ? "Attempted minor crimes must be met with fines!" : (crimeName === 'No crime' ? "You have not selected a crime." : (!data.desired_name ? "The prisoner requires an identifier for their card." : null))}
                color="good"
        ...
        <Section title="Preview">
          ...
          <Section title="Crime details" buttons={<Button content="Edit" icon="pen" onClick={() => act('edit_details')} />}>{desired_details}</Section>
        </Section>
