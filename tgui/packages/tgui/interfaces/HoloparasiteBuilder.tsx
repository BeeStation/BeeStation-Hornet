...
const InputValidity = (
  props: { field: string; validity: Validity },
  context,
) => {
  const {
    data: { themed_name },
...
const BasicSection = (_props, context) => {
  ...
  const [_unused_points_dialog, set_unused_points_dialog] =
    useLocalState<boolean>(context, 'unused_points_dialog', false);
...
const AbilitiesTabs = (_props, context) => {
  const [tab, set_tab] = useLocalState<AbilityTab>(
    context,
    'tab',
    AbilityTab.Major,
  );
...
export const HoloparasiteBuilder = (_props, context) => {
  const [color_select] = useLocalState<HsvaColor | null>(
    context,
    'color_select',
    null,
  );
