...
#define NEW_QUERY(thepkey, theckey, thevalue) queries += GLOB.dbcon.NewQuery("INSERT INTO SS13_preferences (ckey, preference_tag, preference_value) VALUES ([sanitizeSQL(theckey)], [sanitizeSQL(thepkey)], [sanitizeSQL(thevalue)])")
...
/proc/parse_savefile(owning_ckey, savefile/S)
	...
	*/
	...
	for(var/DBQuery/query in queries)
		query.Execute()
